 ...
 * states that inadvertantly trigger a watchdog reset.
 * 
...
#define WDG_INST_DEF() const iec60730_WatchDog_t iec60730_WdogInst_arr[IEC60730_WDOGINST_NUMB] = { \
...
#define IEC60730_RST_WDOG_CHECK(numb) (IEC60730_RSTCAUSE & iec60730_WdogInst_arr[numb].rst) == \
...
#ifdef _SILICON_LABS_32B_SERIES_2_CONFIG_3
#define IEC60730_WDOG_EN_WAIT_DISABLING(numb) while ((iec60730_WdogInst_arr[numb].wdog->EN & WDOG_EN_DISABLING) != 0U ) {}
#else
...
#define IEC60730_SETWATCHDOGTIMEOUTMIN(numb) \
  do { \
    while ( (iec60730_WdogInst_arr[numb].wdog->SYNCBUSY & WDOG_SYNCBUSY_CTRL) != 0U ) { \
    } \
    iec60730_WdogInst_arr[numb].wdog->CTRL &= ~(uint32_t)_WDOG_CTRL_PERSEL_MASK; \
  } while(0)
#else // Series 2 devices
#ifdef WDOG_HAS_SET_CLEAR
#define IEC60730_SETWATCHDOGTIMEOUTMIN(numb) \
...
#else
#define IEC60730_SETWATCHDOGTIMEOUTMIN(numb) do { \
    iec60730_WdogInst_arr[numb].wdog->EN &= ~_WDOG_EN_MASK; \
    IEC60730_WDOG_EN_WAIT_DISABLING(numb) \
    iec60730_WdogInst_arr[numb].wdog->CFG &= ~(uint32_t)_WDOG_CFG_PERSEL_MASK; \
...
extern volatile SI_SEGMENT_VARIABLE(iec60730_WatchdogState, iec60730_TestWatchdog_t, SI_SEG_DATA_RETAINED_NO_CLEAR);

/**************************************************************************//**
 * public IEC60730 Watchdog Power On Self Test
 *
 * @returns #iec60730_TestResult_t If a non-watchdog reset occurred, 
 ...
 * Planned watchdog resets return #iec60730_TestPassed.
 * 
