 ...
 * 
 * The variable iec60730_systemClockToTestClockFrequency holds the ratio value between system clock timer interrupt period and
 * test clock timer interrupt period, this value is used to compare with the system clock counter. The ratio value is devided by slow 
 ...
 * 
 * Finally, the OEM is responsible for setting iec60730_timerTestControl to iec60730_TimerTestDisable
 * within safety-critical code. This value is only used in cases where the system
 * is executing non-safety-critical code, where the BIST routine is also not being called. 
 ...
 * and set iec60730_timerTestControl to iec60730_TimerTestEnable.
 * 
...
/// Incremented when iec60730_SystemClockTick() is called, 
/// cleared in iec60730_TestClockTick.
extern SI_SEGMENT_VARIABLE(iec60730_systemClockCounter, uint8_t, SI_SEG_DATA);
