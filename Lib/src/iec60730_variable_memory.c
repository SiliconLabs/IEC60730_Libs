...
extern bool Vmc_MarchC_Step(uint32_t *start, uint32_t *end, uint32_t background, uint32_t *backup);
extern bool Vmc_MarchXC_Step(uint32_t *start, uint32_t *backup, uint32_t background);
...
iec60730_TestResult_t iec60730_VmcPost(vmcParams_t *params) {
  if (NULL == params) return iec60730_TestFailed;
  ...
  while (rtCheck < params->end) {
    if (!CHECK_INTEGRITY(uint32_t, rtCheck)) goto CHECK_DONE;
    if (!Vmc_MarchC_Step(rtCheck,
                         (uint32_t*)((uint8_t*)(rtCheck + BLOCKSIZE) - 1),
                         BACKGROUND,
                         bkBuf)) goto CHECK_DONE;
    rtCheck += BLOCKSIZE;
...
VMC_BIST_DONE:
  ...
  if ((result == iec60730_TestInProgress)
      || (result == iec60730_TestPassed)) {
