...
iec60730_TestResult_t iec60730_ImcPost(imcParams_t *params) {
  ...
  if ((uint32_t) ROM_START >= (uint32_t) ROM_END) return iec60730_TestFailed;

  refCrc = 0;
  LABEL_DEF(IEC60730_IMC_POST_START_BKPT);

  // Calculate CRC
  refCrc = calCrc(IMC_INIT_VALUE, (uint8_t*) ROM_START, ROM_SIZE) ^ IMC_XOROUTPUT;
...
iec60730_TestResult_t iec60730_updateCRCWithDataBuffer(updateCrcParams_t *params,
  ...
  if ((NULL == params)
      || (NULL == crc)
      || (NULL == buffer)) return iec60730_TestFailed;
...
iec60730_TestResult_t iec60730_ImcPost(imcParams_t *params) {
  if ((NULL == params)
      || (NULL == params->gpcrc)
      || ((uint32_t) ROM_START >= (uint32_t) ROM_END)) return iec60730_TestFailed;
...
iec60730_TestResult_t iec60730_ImcBist(imcParams_t *params) {
  ...
  if ((NULL == params)
      || (NULL == params->gpcrc)
      || ((uint32_t) ROM_START >= (uint32_t) ROM_END)) return result;
  ...
  #else /* !USE_CRC_32 */
          curCrc = (crc_t) CRC_DATAREADBITREVERSED(params->gpcrc) ^ IMC_XOROUTPUT;
  #endif /* USE_CRC_32 */
...
{
  if ((NULL == params)
      || (NULL == params->hal.gpcrc)) return iec60730_TestFailed;
