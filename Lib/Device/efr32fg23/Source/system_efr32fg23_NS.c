...
uint32_t SystemHFRCODPLLClockGet(void)
{
  ...
  switch ((HFRCO0_NS->CAL & _HFRCO_CAL_FREQRANGE_MASK)
          >> _HFRCO_CAL_FREQRANGE_SHIFT) {
    case 0:
      switch (HFRCO0_NS->CAL & _HFRCO_CAL_CLKDIV_MASK) {
...
uint32_t SystemSYSCLKGet(void)
{
  ...
  /* Find clock source */
  switch (CMU_NS->SYSCLKCTRL & _CMU_SYSCLKCTRL_CLKSEL_MASK) {
...
uint32_t SystemHCLKGet(void)
{
  ...
  presc = (CMU_NS->SYSCLKCTRL & _CMU_SYSCLKCTRL_HCLKPRESC_MASK)
...
void SystemHFXOClockSet(uint32_t freq)
{
  ...
  if ((CMU_NS->SYSCLKCTRL & _CMU_SYSCLKCTRL_CLKSEL_MASK)
...
uint32_t SystemHFRCOEM23ClockGet(void)
{
  ...
  /* Get oscillator frequency band */
  switch ((HFRCOEM23_NS->CAL & _HFRCO_CAL_FREQRANGE_MASK)
          >> _HFRCO_CAL_FREQRANGE_SHIFT) {
    case 0:
      switch (HFRCOEM23_NS->CAL & _HFRCO_CAL_CLKDIV_MASK) {
        case HFRCO_CAL_CLKDIV_DIV1:
          ret = 4000000UL;
          break;
