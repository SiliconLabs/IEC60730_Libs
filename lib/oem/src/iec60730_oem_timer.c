...
void iec60730_Adj_Pres_Timer10ms(void)
{
  ...
  // Calculate prescaler of the timer to generate 100hz
  uint32_t press = freq / 100;

  // Calculate TOP value of the timer
  uint32_t top_val = press / (Timer_10ms_cfg.prescale + 1);

  TIMER_TopSet(TIMER_10MS, top_val);

  freq = CMU_ClockFreqGet(cmuClock_LETIMER0);

  // Calculate prescaler of the timer to generate 10hz
  top_val = freq / 10;
