...
void CMU_ClockEnable(CMU_Clock_TypeDef clock, bool enable)
{
  ...
  if (((unsigned)clock >> CMU_EN_REG_POS) == CMU_NO_EN_REG) {
    ...
    reg = &IEC_P_INST(CMU)->CLKEN0;
  } else if (((unsigned)clock >> CMU_EN_REG_POS) == CMU_CLKEN1_EN_REG) {
    reg = &IEC_P_INST(CMU)->CLKEN1;
  } else {
#if defined(CRYPTOACC_PRESENT)
    reg = &IEC_P_INST(CMU)->CRYPTOACCCLKCTRL;
#else
...
static void em01GrpbClkGet(uint32_t *freq, CMU_Select_TypeDef *sel)
{
  ...
  switch (IEC_P_INST(CMU)->EM01GRPBCLKCTRL & _CMU_EM01GRPBCLKCTRL_CLKSEL_MASK) {
...
static void euart0ClkGet(uint32_t *freq, CMU_Select_TypeDef *sel)
{
  switch (IEC_P_INST(CMU)->EUART0CLKCTRL & _CMU_EUART0CLKCTRL_CLKSEL_MASK) {
...
static void eusart0ClkGet(uint32_t *freq, CMU_Select_TypeDef *sel)
{
  ...
  switch (IEC_P_INST(CMU)->EUSART0CLKCTRL & _CMU_EUSART0CLKCTRL_CLKSEL_MASK) {
...
static void em01GrpcClkGet(uint32_t *freq, CMU_Select_TypeDef *sel)
{
  ...
  switch (IEC_P_INST(CMU)->EM01GRPCCLKCTRL & _CMU_EM01GRPCCLKCTRL_CLKSEL_MASK) {
...
static void lcdClkGet(uint32_t *freq, CMU_Select_TypeDef *sel)
{
  ...
  switch (IEC_P_INST(CMU)->LCDCLKCTRL & _CMU_LCDCLKCTRL_CLKSEL_MASK) {
...
static void vdac0ClkGet(uint32_t *freq, CMU_Select_TypeDef *sel)
{
  ...
  switch (IEC_P_INST(CMU)->VDAC0CLKCTRL & _CMU_VDAC0CLKCTRL_CLKSEL_MASK) {
...
static void pcnt0ClkGet(uint32_t *freq, CMU_Select_TypeDef *sel)
{
  ...
  switch (IEC_P_INST(CMU)->PCNT0CLKCTRL & _CMU_PCNT0CLKCTRL_CLKSEL_MASK) {
...
static void lesenseHFClkGet(uint32_t *freq, CMU_Select_TypeDef *sel)
{
  ...
  switch (IEC_P_INST(CMU)->LESENSEHFCLKCTRL & _CMU_LESENSEHFCLKCTRL_CLKSEL_MASK) {
...
static void rhclkPrescMax(void)
{
  ...
  IEC_P_INST(CMU)->SYSCLKCTRL_SET = CMU_SYSCLKCTRL_RHCLKPRESC;
}
...
static void rhclkPrescOptimize(void)
{
  if (CMU_ClockFreqGet(cmuClock_SYSCLK) <= CMU_MAX_RHCLK_FREQ) {
    ...
    IEC_P_INST(CMU)->SYSCLKCTRL_CLR = CMU_SYSCLKCTRL_RHCLKPRESC;
  }
}
...
void CMU_HFRCOEM23BandSet(CMU_HFRCOEM23Freq_TypeDef freq)
{
  ...
  IEC_P_INST(CMU)->CLKEN0_SET = CMU_CLKEN0_HFRCOEM23;
#endif
...
static void traceClkGet(uint32_t *freq, CMU_Select_TypeDef *sel)
{
  ...
  switch (IEC_P_INST(CMU)->TRACECLKCTRL & _CMU_TRACECLKCTRL_CLKSEL_MASK) {
...
static void wdog1ClkGet(uint32_t *freq, CMU_Select_TypeDef *sel)
{
  ...
  switch (IEC_P_INST(CMU)->WDOG1CLKCTRL & _CMU_WDOG1CLKCTRL_CLKSEL_MASK) {
                         ...
                         CMU_Select_TypeDef upSel)
{
  ...
  uint32_t calCtrl = IEC_P_INST(CMU)->CALCTRL
  ...
  IEC_P_INST(CMU)->CALCTRL = calCtrl;
}
 ...
 *    write operation to IEC_P_INST(CMU)->CALCNT.
 ******************************************************************************/
uint32_t CMU_CalibrateCountGet(void)
{
  // Wait until calibration completes, UNLESS continuous calibration mode is on
  if ((IEC_P_INST(CMU)->CALCTRL & CMU_CALCTRL_CONT) == 0UL) {
    // Wait until calibration completes
    while ((IEC_P_INST(CMU)->STATUS & CMU_STATUS_CALRDY) == 0UL) {
    }
  }
  return IEC_P_INST(CMU)->CALCNT;
}
                         ...
                         unsigned int       pin)
{
  ...
  IEC_P_INST(CMU)->EXPORTCLKCTRL = (IEC_P_INST(CMU)->EXPORTCLKCTRL & ~mask) | tmp;
...
CMU_ClkDiv_TypeDef CMU_ClockDivGet(CMU_Clock_TypeDef clock)
{
  ...
  switch (clock) {
    case cmuClock_HCLK:
    case cmuClock_CORE:
      ret = (IEC_P_INST(CMU)->SYSCLKCTRL & _CMU_SYSCLKCTRL_HCLKPRESC_MASK)
    ...
    case cmuClock_TRACECLK:
      ret = (IEC_P_INST(CMU)->TRACECLKCTRL & _CMU_TRACECLKCTRL_PRESC_MASK)
    ...
    case cmuClock_EXPCLK:
      ret = (IEC_P_INST(CMU)->EXPORTCLKCTRL & _CMU_EXPORTCLKCTRL_PRESC_MASK)
            >> _CMU_EXPORTCLKCTRL_PRESC_SHIFT;
      break;

    case cmuClock_PCLK:
      ret = (IEC_P_INST(CMU)->SYSCLKCTRL & _CMU_SYSCLKCTRL_PCLKPRESC_MASK)
...
void CMU_ClockDivSet(CMU_Clock_TypeDef clock, CMU_ClkDiv_TypeDef div)
{
  switch (clock) {
    ...
    case cmuClock_CORE:
      ...
      // Set new divisor
      IEC_P_INST(CMU)->SYSCLKCTRL = (IEC_P_INST(CMU)->SYSCLKCTRL & ~_CMU_SYSCLKCTRL_HCLKPRESC_MASK)
    ...
    case cmuClock_TRACECLK:
      EFM_ASSERT((div == 1U) || (div == 2U) || (div == 4U));
      IEC_P_INST(CMU)->TRACECLKCTRL = (IEC_P_INST(CMU)->TRACECLKCTRL & ~_CMU_TRACECLKCTRL_PRESC_MASK)
    ...
    case cmuClock_EXPCLK:
      ...
      IEC_P_INST(CMU)->EXPORTCLKCTRL = (IEC_P_INST(CMU)->EXPORTCLKCTRL & ~_CMU_EXPORTCLKCTRL_PRESC_MASK)
                           | ((div - 1U) << _CMU_EXPORTCLKCTRL_PRESC_SHIFT);
      break;

    case cmuClock_PCLK:
      EFM_ASSERT((div == 1U) || (div == 2U));
      IEC_P_INST(CMU)->SYSCLKCTRL = (IEC_P_INST(CMU)->SYSCLKCTRL & ~_CMU_SYSCLKCTRL_PCLKPRESC_MASK)
...
// -----------------------------------------------------------------------------
    case cmuClock_SYSCLK:
      switch (IEC_P_INST(CMU)->SYSCLKCTRL & _CMU_SYSCLKCTRL_CLKSEL_MASK) {
    ...
    case cmuClock_SYSCLK:
      switch (ref) {
        ...
        case cmuSelect_HFXO:
          ...
          IEC_P_INST(CMU)->CLKEN0_SET = CMU_CLKEN0_HFXO0;
#endif
#endif
      ...
      IEC_P_INST(CMU)->SYSCLKCTRL = (IEC_P_INST(CMU)->SYSCLKCTRL & ~_CMU_SYSCLKCTRL_CLKSEL_MASK) | tmp;
...
// -----------------------------------------------------------------------------
    ...
    case cmuClock_IADCCLK:
      ...
      IEC_P_INST(CMU)->IADCCLKCTRL = (IEC_P_INST(CMU)->IADCCLKCTRL & ~_CMU_IADCCLKCTRL_CLKSEL_MASK)
                         | tmp;
      break;
...
// -----------------------------------------------------------------------------
    ...
    case cmuClock_EM01GRPACLK:
      ...
      IEC_P_INST(CMU)->EM01GRPACLKCTRL = (IEC_P_INST(CMU)->EM01GRPACLKCTRL
...
// -----------------------------------------------------------------------------
    ...
    case cmuClock_LESENSELFCLK:
      ...
      IEC_P_INST(CMU)->EM23GRPACLKCTRL = (IEC_P_INST(CMU)->EM23GRPACLKCTRL
...
// -----------------------------------------------------------------------------
    ...
    case cmuClock_EM4GRPACLK:
      ...
      IEC_P_INST(CMU)->EM4GRPACLKCTRL = (IEC_P_INST(CMU)->EM4GRPACLKCTRL
...
// -----------------------------------------------------------------------------
    ...
    case cmuClock_EM01GRPBCLK:
      ...
      IEC_P_INST(CMU)->EM01GRPBCLKCTRL = (IEC_P_INST(CMU)->EM01GRPBCLKCTRL
...
// -----------------------------------------------------------------------------
    ...
    case cmuClock_WDOG0CLK:
      ...
      IEC_P_INST(CMU)->WDOG0CLKCTRL = (IEC_P_INST(CMU)->WDOG0CLKCTRL & ~_CMU_WDOG0CLKCTRL_CLKSEL_MASK)
                          | tmp;
      break;
...
// -----------------------------------------------------------------------------
    ...
    case cmuClock_WDOG1CLK:
      ...
      IEC_P_INST(CMU)->WDOG1CLKCTRL = (IEC_P_INST(CMU)->WDOG1CLKCTRL & ~_CMU_WDOG1CLKCTRL_CLKSEL_MASK)
                          | tmp;
      break;
#endif
...
// -----------------------------------------------------------------------------
    case cmuClock_DPLLREFCLK:
      ...
      IEC_P_INST(CMU)->DPLLREFCLKCTRL = (IEC_P_INST(CMU)->DPLLREFCLKCTRL
...
// -----------------------------------------------------------------------------
    case cmuClock_TRACECLK:
      ...
      IEC_P_INST(CMU)->TRACECLKCTRL = (IEC_P_INST(CMU)->TRACECLKCTRL & ~_CMU_TRACECLKCTRL_CLKSEL_MASK)
                          | tmp;
      break;
#endif
...
// -----------------------------------------------------------------------------
    ...
    case cmuClock_EUART0CLK:
      ...
      IEC_P_INST(CMU)->EUART0CLKCTRL = (IEC_P_INST(CMU)->EUART0CLKCTRL & ~_CMU_EUART0CLKCTRL_CLKSEL_MASK)
                           | tmp;
      break;
#elif defined(EUSART_PRESENT)
    case cmuClock_EUSART0:
    case cmuClock_EUSART0CLK:
      ...
      IEC_P_INST(CMU)->EUSART0CLKCTRL = (IEC_P_INST(CMU)->EUSART0CLKCTRL & ~_CMU_EUSART0CLKCTRL_CLKSEL_MASK)
                            | tmp;
      break;
    ...
    case cmuClock_EM01GRPCCLK:
      ...
      IEC_P_INST(CMU)->EM01GRPCCLKCTRL = (IEC_P_INST(CMU)->EM01GRPCCLKCTRL & ~_CMU_EM01GRPCCLKCTRL_CLKSEL_MASK)
                             | tmp;
      break;
#endif
...
// -----------------------------------------------------------------------------
    ...
    case cmuClock_RTCCCLK:
      ...
      IEC_P_INST(CMU)->RTCCCLKCTRL = (IEC_P_INST(CMU)->RTCCCLKCTRL & ~_CMU_RTCCCLKCTRL_CLKSEL_MASK)
                         | tmp;
      break;
#endif
// -----------------------------------------------------------------------------
    ...
    case cmuClock_SYSRTCCLK:
      ...
      IEC_P_INST(CMU)->SYSRTC0CLKCTRL = (IEC_P_INST(CMU)->SYSRTC0CLKCTRL & ~_CMU_SYSRTC0CLKCTRL_CLKSEL_MASK)
                            | tmp;
      break;
#endif
// -----------------------------------------------------------------------------
    ...
    case cmuClock_LCDCLK:
      ...
      IEC_P_INST(CMU)->LCDCLKCTRL =  (IEC_P_INST(CMU)->LCDCLKCTRL  & ~_CMU_LCDCLKCTRL_CLKSEL_MASK)
                        | tmp;
      break;
#endif
// -----------------------------------------------------------------------------
    ...
    case cmuClock_VDAC0CLK:
      ...
      IEC_P_INST(CMU)->VDAC0CLKCTRL = (IEC_P_INST(CMU)->VDAC0CLKCTRL & ~_CMU_VDAC0CLKCTRL_CLKSEL_MASK)
                          | tmp;
      break;
#endif
// -----------------------------------------------------------------------------
    ...
    case cmuClock_PCNT0CLK:
      ...
      IEC_P_INST(CMU)->PCNT0CLKCTRL = (IEC_P_INST(CMU)->PCNT0CLKCTRL & ~_CMU_PCNT0CLKCTRL_CLKSEL_MASK)
                          | tmp;
      break;
#endif
// -----------------------------------------------------------------------------
    ...
    case cmuClock_LESENSEHFCLK:
      ...
      IEC_P_INST(CMU)->LESENSEHFCLKCTRL = (IEC_P_INST(CMU)->LESENSEHFCLKCTRL & ~_CMU_LESENSEHFCLKCTRL_CLKSEL_MASK)
                              | tmp;
      break;
#endif
    default:
...
uint16_t CMU_LF_ClockPrecisionGet(CMU_Clock_TypeDef clock)
{
  ...
  switch (src) {
    ...
    case cmuSelect_LFRCO:
      IEC_P_INST(CMU)->CLKEN0_SET = CMU_CLKEN0_LFRCO;
...
bool CMU_DPLLLock(const CMU_DPLLInit_TypeDef *init)
{
  ...
  IEC_P_INST(CMU)->CLKEN0_SET = CMU_CLKEN0_DPLL0 | CMU_CLKEN0_HFRCO0;
#endif
...
void CMU_LFXOInit(const CMU_LFXOInit_TypeDef *lfxoInit)
{
  ...
  IEC_P_INST(CMU)->CLKEN0_SET = CMU_CLKEN0_LFXO;
#endif
...
uint32_t CMU_OscillatorTuningGet(CMU_Osc_TypeDef osc)
{
  ...
  switch (osc) {
    ...
    case cmuOsc_HFRCODPLL:
      ...
      IEC_P_INST(CMU)->CLKEN0_SET = CMU_CLKEN0_HFRCO0;
#endif
...
static void dpllRefClkGet(uint32_t *freq, CMU_Select_TypeDef *sel)
{
  ...
  switch (IEC_P_INST(CMU)->DPLLREFCLKCTRL & _CMU_DPLLREFCLKCTRL_CLKSEL_MASK) {
...
static void em01GrpaClkGet(uint32_t *freq, CMU_Select_TypeDef *sel)
{
  ...
  switch (IEC_P_INST(CMU)->EM01GRPACLKCTRL & _CMU_EM01GRPACLKCTRL_CLKSEL_MASK) {
...
static void em23GrpaClkGet(uint32_t *freq, CMU_Select_TypeDef *sel)
{
  ...
  switch (IEC_P_INST(CMU)->EM23GRPACLKCTRL & _CMU_EM23GRPACLKCTRL_CLKSEL_MASK) {
...
static void em4GrpaClkGet(uint32_t *freq, CMU_Select_TypeDef *sel)
{
  ...
  switch (IEC_P_INST(CMU)->EM4GRPACLKCTRL & _CMU_EM4GRPACLKCTRL_CLKSEL_MASK) {
...
static void flashWaitStateControl(uint32_t coreFreq, int vscale)
{
  ...
  IEC_P_INST(CMU)->CLKEN1_SET = CMU_CLKEN1_MSC;
#endif
...
static void iadcClkGet(uint32_t *freq, CMU_Select_TypeDef *sel)
{
  ...
  switch (IEC_P_INST(CMU)->IADCCLKCTRL & _CMU_IADCCLKCTRL_CLKSEL_MASK) {
...
static void rtccClkGet(uint32_t *freq, CMU_Select_TypeDef *sel)
{
  ...
  switch (IEC_P_INST(CMU)->RTCCCLKCTRL & _CMU_RTCCCLKCTRL_CLKSEL_MASK) {
...
static void sysrtcClkGet(uint32_t *freq, CMU_Select_TypeDef *sel)
{
  ...
  switch (IEC_P_INST(CMU)->SYSRTC0CLKCTRL & _CMU_SYSRTC0CLKCTRL_CLKSEL_MASK) {
...
static void wdog0ClkGet(uint32_t *freq, CMU_Select_TypeDef *sel)
{
  ...
  switch (IEC_P_INST(CMU)->WDOG0CLKCTRL & _CMU_WDOG0CLKCTRL_CLKSEL_MASK) {
...
#if defined(PCNT_PRESENT)
  if (external) {
    IEC_P_INST(CMU)->PCNT0CLKCTRL = CMU_PCNT0CLKCTRL_CLKSEL_PCNTS0;
  } else {
    IEC_P_INST(CMU)->PCNT0CLKCTRL = CMU_PCNT0CLKCTRL_CLKSEL_EM23GRPACLK;
  }
#else
  (void)external;
#endif
...
#define GENERIC_HFLE_PRESC_REG          IEC_P_INST(CMU)->HFCORECLKDIV
...
#define GENERIC_HFLE_PRESC_REG          IEC_P_INST(CMU)->HFPRESC
...
static void setHfLeConfig(uint32_t hfFreq)
{
  ...
  BUS_RegBitWrite(&IEC_P_INST(CMU)->CTRL, _GENERIC_HFLE_WS_SHIFT, hfleWs);
...
{
  uint32_t ws = BUS_RegBitRead(&IEC_P_INST(CMU)->CTRL, _GENERIC_HFLE_WS_SHIFT);
  return ws;
}
#endif
#endif
...
static uint32_t auxClkGet(void)
{
  ...
  switch (IEC_P_INST(CMU)->AUXHFRCOCTRL & _CMU_AUXHFRCOCTRL_BAND_MASK) {
...
static uint32_t hfSrcClkGet(void)
{
  ...
  return ret * (1U + ((IEC_P_INST(CMU)->HFPRESC & _CMU_HFPRESC_PRESC_MASK)
...
__STATIC_INLINE uint32_t getHfxoTuningMode(void)
{
  ...
  return (IEC_P_INST(CMU)->HFXOCTRL & _CMU_HFXOCTRL_PEAKDETSHUNTOPTMODE_MASK)
         >> _CMU_HFXOCTRL_PEAKDETSHUNTOPTMODE_SHIFT;
#else
  return (IEC_P_INST(CMU)->HFXOCTRL & _CMU_HFXOCTRL_PEAKDETMODE_MASK)
...
{
#if defined(_CMU_HFXOCTRL_PEAKDETSHUNTOPTMODE_MASK)
  IEC_P_INST(CMU)->HFXOCTRL = (IEC_P_INST(CMU)->HFXOCTRL & ~_CMU_HFXOCTRL_PEAKDETSHUNTOPTMODE_MASK)
                  | (mode << _CMU_HFXOCTRL_PEAKDETSHUNTOPTMODE_SHIFT);
#else
  IEC_P_INST(CMU)->HFXOCTRL = (IEC_P_INST(CMU)->HFXOCTRL & ~_CMU_HFXOCTRL_PEAKDETMODE_MASK)
...
static uint32_t lfClkGet(CMU_Clock_TypeDef lfClkBranch)
{
  ...
  switch (lfClkBranch) {
    case cmuClock_LFA:
      ...
      sel = (IEC_P_INST(CMU)->LFCLKSEL & _CMU_LFCLKSEL_LFA_MASK) >> _CMU_LFCLKSEL_LFA_SHIFT;
#elif defined(_CMU_LFACLKSEL_MASK)
      sel = (IEC_P_INST(CMU)->LFACLKSEL & _CMU_LFACLKSEL_LFA_MASK) >> _CMU_LFACLKSEL_LFA_SHIFT;
#else
      EFM_ASSERT(false);
#endif
      break;
    ...
    case cmuClock_LFB:
#if defined(_CMU_LFCLKSEL_MASK)
      sel = (IEC_P_INST(CMU)->LFCLKSEL & _CMU_LFCLKSEL_LFB_MASK) >> _CMU_LFCLKSEL_LFB_SHIFT;
#elif defined(_CMU_LFBCLKSEL_MASK)
      sel = (IEC_P_INST(CMU)->LFBCLKSEL & _CMU_LFBCLKSEL_LFB_MASK) >> _CMU_LFBCLKSEL_LFB_SHIFT;
#else
      EFM_ASSERT(false);
#endif
      break;
    ...
    case cmuClock_LFC:
      ...
      sel = (IEC_P_INST(CMU)->LFCLKSEL & _CMU_LFCLKSEL_LFC_MASK) >> _CMU_LFCLKSEL_LFC_SHIFT;
#elif defined(_CMU_LFCCLKSEL_LFC_MASK)
      sel = (IEC_P_INST(CMU)->LFCCLKSEL & _CMU_LFCCLKSEL_LFC_MASK) >> _CMU_LFCCLKSEL_LFC_SHIFT;
#else
      EFM_ASSERT(false);
#endif
      break;
...
#if defined(_CMU_LFECLKSEL_MASK)
    case cmuClock_LFE:
      sel = (IEC_P_INST(CMU)->LFECLKSEL & _CMU_LFECLKSEL_LFE_MASK) >> _CMU_LFECLKSEL_LFE_SHIFT;
      break;
#endif
  ...
  switch (sel) {
    ...
    case _CMU_LFCLKSEL_LFA_DISABLED:
      ...
      if ((lfClkBranch == cmuClock_LFA) || (lfClkBranch == cmuClock_LFB)) {
        if (IEC_P_INST(CMU)->LFCLKSEL >> (lfClkBranch == cmuClock_LFA
  ...
  switch (sel) {
    ...
    case _CMU_LFBCLKSEL_LFB_HFCLKLE:
      ret = SystemHFClockGet()
            / SL_Log2ToDiv(((IEC_P_INST(CMU)->HFPRESC & _CMU_HFPRESC_HFCLKLEPRESC_MASK)
...
__STATIC_INLINE void syncReg(uint32_t mask)
{
  ...
  /* activated. */
  if ((IEC_P_INST(CMU)->FREEZE & CMU_FREEZE_REGFREEZE) != 0UL) {
    return;
  }

  /* Wait for any pending previous write operation to complete */
  /* in low-frequency domain. */
  while ((IEC_P_INST(CMU)->SYNCBUSY & mask) != 0UL) {
  }
}
...
{
  return (CMU_AUXHFRCOBand_TypeDef)((IEC_P_INST(CMU)->AUXHFRCOCTRL
...
void CMU_AUXHFRCOBandSet(CMU_AUXHFRCOBand_TypeDef band)
{
  ...
  /* Set band/tuning. */
  IEC_P_INST(CMU)->AUXHFRCOCTRL = (IEC_P_INST(CMU)->AUXHFRCOCTRL
                       & ~(_CMU_AUXHFRCOCTRL_BAND_MASK
...
void CMU_AUXHFRCOBandSet(CMU_AUXHFRCOFreq_TypeDef setFreq)
{
  ...
  while (BUS_RegBitRead(&IEC_P_INST(CMU)->SYNCBUSY,
  ...
  IEC_P_INST(CMU)->AUXHFRCOCTRL = freqCal;
}
#endif /* _CMU_AUXHFRCOCTRL_FREQRANGE_MASK */
...
uint32_t CMU_Calibrate(uint32_t HFCycles, CMU_Osc_TypeDef reference)
{
  ...
  switch (reference) {
    case cmuOsc_LFXO:
      IEC_P_INST(CMU)->CALCTRL = CMU_CALCTRL_UPSEL_LFXO;
      break;

    case cmuOsc_LFRCO:
      IEC_P_INST(CMU)->CALCTRL = CMU_CALCTRL_UPSEL_LFRCO;
      break;

#if defined(PLFRCO_PRESENT)
    case cmuOsc_PLFRCO:
      IEC_P_INST(CMU)->CALCTRL = CMU_CALCTRL_UPSEL_PLFRCO;
      break;
#endif

    case cmuOsc_HFXO:
      IEC_P_INST(CMU)->CALCTRL = CMU_CALCTRL_UPSEL_HFXO;
      break;

    case cmuOsc_HFRCO:
      IEC_P_INST(CMU)->CALCTRL = CMU_CALCTRL_UPSEL_HFRCO;
      break;

    case cmuOsc_AUXHFRCO:
      IEC_P_INST(CMU)->CALCTRL = CMU_CALCTRL_UPSEL_AUXHFRCO;
      break;

#if defined (_CMU_USHFRCOCTRL_MASK)
    case cmuOsc_USHFRCO:
      IEC_P_INST(CMU)->CALCTRL = CMU_CALCTRL_UPSEL_USHFRCO;
      break;
#endif
  ...
  /* Set the top value. */
  IEC_P_INST(CMU)->CALCNT = HFCycles;

  /* Start the calibration. */
  IEC_P_INST(CMU)->CMD = CMU_CMD_CALSTART;

#if defined(CMU_STATUS_CALRDY)
  /* Wait until calibration completes. */
  while (BUS_RegBitRead(&IEC_P_INST(CMU)->STATUS, _CMU_STATUS_CALRDY_SHIFT) == 0UL) {
  }
#else
  /* Wait until calibration completes. */
  while (BUS_RegBitRead(&IEC_P_INST(CMU)->STATUS, _CMU_STATUS_CALBSY_SHIFT) != 0UL) {
  }
#endif
                         ...
                         CMU_Osc_TypeDef upSel)
{
  ...
  IEC_P_INST(CMU)->CALCNT = downCycles;
...
uint32_t CMU_CalibrateCountGet(void)
{
  ...
  if (BUS_RegBitRead(&IEC_P_INST(CMU)->CALCTRL, _CMU_CALCTRL_CONT_SHIFT) == 0UL) {
#if defined(CMU_STATUS_CALRDY)
    /* Wait until calibration completes */
    while (BUS_RegBitRead(&IEC_P_INST(CMU)->STATUS, _CMU_STATUS_CALRDY_SHIFT) == 0UL) {
    }
...
CMU_ClkDiv_TypeDef CMU_ClockDivGet(CMU_Clock_TypeDef clock)
{
  ...
  switch (divReg) {
    ...
    case CMU_HFCLKDIV_REG:
      ret = 1 + ((IEC_P_INST(CMU)->CTRL & _CMU_CTRL_HFCLKDIV_MASK)
                 >> _CMU_CTRL_HFCLKDIV_SHIFT);
      break;
#endif

    case CMU_HFPERCLKDIV_REG:
      ret = (CMU_ClkDiv_TypeDef)((IEC_P_INST(CMU)->HFPERCLKDIV
    ...
    case CMU_HFCORECLKDIV_REG:
      ret = (CMU_ClkDiv_TypeDef)((IEC_P_INST(CMU)->HFCORECLKDIV
    ...
    case CMU_LFAPRESC0_REG:
      switch (clock) {
        case cmuClock_RTC:
          ret = (CMU_ClkDiv_TypeDef)((IEC_P_INST(CMU)->LFAPRESC0 & _CMU_LFAPRESC0_RTC_MASK)
        ...
        case cmuClock_LETIMER0:
          ret = (CMU_ClkDiv_TypeDef)((IEC_P_INST(CMU)->LFAPRESC0 & _CMU_LFAPRESC0_LETIMER0_MASK)
        ...
        case cmuClock_LCDpre:
          ret = (CMU_ClkDiv_TypeDef)(((IEC_P_INST(CMU)->LFAPRESC0 & _CMU_LFAPRESC0_LCD_MASK)
        ...
        case cmuClock_LESENSE:
          ret = (CMU_ClkDiv_TypeDef)((IEC_P_INST(CMU)->LFAPRESC0 & _CMU_LFAPRESC0_LESENSE_MASK)
    ...
    case CMU_LFBPRESC0_REG:
      switch (clock) {
        ...
        case cmuClock_LEUART0:
          ret = (CMU_ClkDiv_TypeDef)((IEC_P_INST(CMU)->LFBPRESC0 & _CMU_LFBPRESC0_LEUART0_MASK)
        ...
        case cmuClock_LEUART1:
          ret = (CMU_ClkDiv_TypeDef)((IEC_P_INST(CMU)->LFBPRESC0 & _CMU_LFBPRESC0_LEUART1_MASK)
...
void CMU_ClockDivSet(CMU_Clock_TypeDef clock, CMU_ClkDiv_TypeDef div)
{
  ...
  switch (divReg) {
    ...
    case CMU_HFCLKDIV_REG:
      ...
      /* Set the divider. */
      IEC_P_INST(CMU)->CTRL = (IEC_P_INST(CMU)->CTRL & ~_CMU_CTRL_HFCLKDIV_MASK)
    ...
    case CMU_HFPERCLKDIV_REG:
      ...
      IEC_P_INST(CMU)->HFPERCLKDIV = (IEC_P_INST(CMU)->HFPERCLKDIV & ~_CMU_HFPERCLKDIV_HFPERCLKDIV_MASK)
    ...
    case CMU_HFCORECLKLEDIV_REG:
      ...
      IEC_P_INST(CMU)->HFCORECLKDIV = (IEC_P_INST(CMU)->HFCORECLKDIV
                           & ~_CMU_HFCORECLKDIV_HFCORECLKLEDIV_MASK)
    ...
    case CMU_LFAPRESC0_REG:
      switch (clock) {
        case cmuClock_RTC:
          ...
          IEC_P_INST(CMU)->LFAPRESC0 = (IEC_P_INST(CMU)->LFAPRESC0 & ~_CMU_LFAPRESC0_RTC_MASK)
        ...
        case cmuClock_LETIMER0:
          ...
          IEC_P_INST(CMU)->LFAPRESC0 = (IEC_P_INST(CMU)->LFAPRESC0 & ~_CMU_LFAPRESC0_LETIMER0_MASK)
        ...
        case cmuClock_LCDpre:
          ...
          IEC_P_INST(CMU)->LFAPRESC0 = (IEC_P_INST(CMU)->LFAPRESC0 & ~_CMU_LFAPRESC0_LCD_MASK)
        ...
        case cmuClock_LESENSE:
          ...
          IEC_P_INST(CMU)->LFAPRESC0 = (IEC_P_INST(CMU)->LFAPRESC0 & ~_CMU_LFAPRESC0_LESENSE_MASK)
    ...
    case CMU_LFBPRESC0_REG:
      switch (clock) {
        ...
        case cmuClock_LEUART0:
          ...
          IEC_P_INST(CMU)->LFBPRESC0 = (IEC_P_INST(CMU)->LFBPRESC0 & ~_CMU_LFBPRESC0_LEUART0_MASK)
        ...
        case cmuClock_LEUART1:
          ...
          IEC_P_INST(CMU)->LFBPRESC0 = (IEC_P_INST(CMU)->LFBPRESC0 & ~_CMU_LFBPRESC0_LEUART1_MASK)
...
void CMU_ClockEnable(CMU_Clock_TypeDef clock, bool enable)
{
  ...
  switch (((unsigned)clock >> CMU_EN_REG_POS) & CMU_EN_REG_MASK) {
    ...
    case CMU_QSPI0REF_EN_REG:
      reg = &IEC_P_INST(CMU)->QSPICTRL;
      enable = !enable;
      break;
#endif
#if defined(_CMU_USBCTRL_MASK)
    case CMU_USBRCLK_EN_REG:
      reg = &IEC_P_INST(CMU)->USBCTRL;
      break;
#endif
#if defined(_CMU_PDMCTRL_MASK)
    case CMU_PDMREF_EN_REG:
      reg = &IEC_P_INST(CMU)->PDMCTRL;
      break;
#endif

    case CMU_PCNT_EN_REG:
      reg = &IEC_P_INST(CMU)->PCNTCTRL;
      break;
...
uint32_t CMU_ClockFreqGet(CMU_Clock_TypeDef clock)
{
  ...
  switch ((unsigned)clock & (CMU_CLK_BRANCH_MASK << CMU_CLK_BRANCH_POS)) {
    ...
    case (CMU_HFPER_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ...
      ret >>= (IEC_P_INST(CMU)->HFPERCLKDIV & _CMU_HFPERCLKDIV_HFPERCLKDIV_MASK)
              >> _CMU_HFPERCLKDIV_HFPERCLKDIV_SHIFT;
#endif
#if defined(_CMU_HFPERPRESC_PRESC_MASK)
      ret /= 1U + ((IEC_P_INST(CMU)->HFPERPRESC & _CMU_HFPERPRESC_PRESC_MASK)
    ...
    case (CMU_HFPERB_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ...
      ret /= 1U + ((IEC_P_INST(CMU)->HFPERPRESCB & _CMU_HFPERPRESCB_PRESC_MASK)
    ...
    case (CMU_HFPERC_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ...
      ret /= 1U + ((IEC_P_INST(CMU)->HFPERPRESCC & _CMU_HFPERPRESCC_PRESC_MASK)
    ...
    case (CMU_HFBUS_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ...
      ret /= 1U + ((IEC_P_INST(CMU)->HFBUSPRESC & _CMU_HFBUSPRESC_MASK)
    ...
    case (CMU_HFCORE_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ret = SystemHFClockGet();
      ret /= 1U + ((IEC_P_INST(CMU)->HFCOREPRESC & _CMU_HFCOREPRESC_PRESC_MASK)
                   >> _CMU_HFCOREPRESC_PRESC_SHIFT);
      break;

    case (CMU_HFEXP_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ret = SystemHFClockGet();
      ret /= 1U + ((IEC_P_INST(CMU)->HFEXPPRESC & _CMU_HFEXPPRESC_PRESC_MASK)
    ...
    case (CMU_RTC_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ret = lfClkGet(cmuClock_LFA);
      ret >>= (IEC_P_INST(CMU)->LFAPRESC0 & _CMU_LFAPRESC0_RTC_MASK)
              >> _CMU_LFAPRESC0_RTC_SHIFT;
      break;
#endif
    ...
    case (CMU_RTCC_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ...
      ret >>= (IEC_P_INST(CMU)->LFEPRESC0 & _CMU_LFEPRESC0_RTCC_MASK)
    ...
    case (CMU_LETIMER0_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ret = lfClkGet(cmuClock_LFA);
#if defined(_SILICON_LABS_32B_SERIES_0)
      ret >>= (IEC_P_INST(CMU)->LFAPRESC0 & _CMU_LFAPRESC0_LETIMER0_MASK)
              >> _CMU_LFAPRESC0_LETIMER0_SHIFT;
#else
      ret /= SL_Log2ToDiv((IEC_P_INST(CMU)->LFAPRESC0 & _CMU_LFAPRESC0_LETIMER0_MASK)
                          >> _CMU_LFAPRESC0_LETIMER0_SHIFT);
#endif
      break;
#endif
    ...
    case (CMU_LETIMER1_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ret = lfClkGet(cmuClock_LFA);
#if defined(_SILICON_LABS_32B_SERIES_0)
      ret >>= (IEC_P_INST(CMU)->LFAPRESC0 & _CMU_LFAPRESC0_LETIMER1_MASK)
              >> _CMU_LFAPRESC0_LETIMER1_SHIFT;
#else
      ret /= SL_Log2ToDiv((IEC_P_INST(CMU)->LFAPRESC0 & _CMU_LFAPRESC0_LETIMER1_MASK)
                          >> _CMU_LFAPRESC0_LETIMER1_SHIFT);
#endif
      break;
#endif
    ...
    case (CMU_LCDPRE_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ret = lfClkGet(cmuClock_LFA);
#if defined(_SILICON_LABS_32B_SERIES_0)
      ret >>= ((IEC_P_INST(CMU)->LFAPRESC0 & _CMU_LFAPRESC0_LCD_MASK)
               >> _CMU_LFAPRESC0_LCD_SHIFT)
              + CMU_DivToLog2(cmuClkDiv_16);
#else
      ret /= SL_Log2ToDiv((IEC_P_INST(CMU)->LFAPRESC0 & _CMU_LFAPRESC0_LCD_MASK)
                          >> _CMU_LFAPRESC0_LCD_SHIFT);
#endif
      break;
    ...
    case (CMU_LCD_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ...
      ret >>= (IEC_P_INST(CMU)->LFAPRESC0 & _CMU_LFAPRESC0_LCD_MASK)
              >> _CMU_LFAPRESC0_LCD_SHIFT;
      ret /= 1U + ((IEC_P_INST(CMU)->LCDCTRL & _CMU_LCDCTRL_FDIV_MASK)
    ...
    case (CMU_LESENSE_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ret = lfClkGet(cmuClock_LFA);
      ret >>= (IEC_P_INST(CMU)->LFAPRESC0 & _CMU_LFAPRESC0_LESENSE_MASK)
              >> _CMU_LFAPRESC0_LESENSE_SHIFT;
      break;
#endif
    ...
    case (CMU_LEUART0_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ret = lfClkGet(cmuClock_LFB);
#if defined(_SILICON_LABS_32B_SERIES_0)
      ret >>= (IEC_P_INST(CMU)->LFBPRESC0 & _CMU_LFBPRESC0_LEUART0_MASK)
              >> _CMU_LFBPRESC0_LEUART0_SHIFT;
#else
      ret /= SL_Log2ToDiv((IEC_P_INST(CMU)->LFBPRESC0 & _CMU_LFBPRESC0_LEUART0_MASK)
                          >> _CMU_LFBPRESC0_LEUART0_SHIFT);
#endif
      break;
#endif
    ...
    case (CMU_LEUART1_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ret = lfClkGet(cmuClock_LFB);
#if defined(_SILICON_LABS_32B_SERIES_0)
      ret >>= (IEC_P_INST(CMU)->LFBPRESC0 & _CMU_LFBPRESC0_LEUART1_MASK)
              >> _CMU_LFBPRESC0_LEUART1_SHIFT;
#else
      ret /= SL_Log2ToDiv((IEC_P_INST(CMU)->LFBPRESC0 & _CMU_LFBPRESC0_LEUART1_MASK)
                          >> _CMU_LFBPRESC0_LEUART1_SHIFT);
#endif
      break;
#endif
    ...
    case (CMU_CSEN_LF_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ret = lfClkGet(cmuClock_LFB);
      ret /= SL_Log2ToDiv(((IEC_P_INST(CMU)->LFBPRESC0 & _CMU_LFBPRESC0_CSEN_MASK)
    ...
    case (CMU_ADC0ASYNC_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ...
      ret /= 1U + ((IEC_P_INST(CMU)->ADCCTRL & _CMU_ADCCTRL_ADC0CLKDIV_MASK)
    ...
    case (CMU_ADC1ASYNC_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ...
      ret /= 1U + ((IEC_P_INST(CMU)->ADCCTRL & _CMU_ADCCTRL_ADC1CLKDIV_MASK)
...
uint32_t CMU_ClockPrescGet(CMU_Clock_TypeDef clock)
{
  ...
  switch (prescReg) {
    case CMU_HFPRESC_REG:
      ret = (IEC_P_INST(CMU)->HFPRESC & _CMU_HFPRESC_PRESC_MASK)
            >> _CMU_HFPRESC_PRESC_SHIFT;
      break;

    case CMU_HFEXPPRESC_REG:
      ret = (IEC_P_INST(CMU)->HFEXPPRESC & _CMU_HFEXPPRESC_PRESC_MASK)
            >> _CMU_HFEXPPRESC_PRESC_SHIFT;
      break;

    case CMU_HFCLKLEPRESC_REG:
      ret = (IEC_P_INST(CMU)->HFPRESC & _CMU_HFPRESC_HFCLKLEPRESC_MASK)
            >> _CMU_HFPRESC_HFCLKLEPRESC_SHIFT;
      ret = SL_Log2ToDiv(ret + 1U) - 1U;
      break;

    case CMU_HFPERPRESC_REG:
      ret = (IEC_P_INST(CMU)->HFPERPRESC & _CMU_HFPERPRESC_PRESC_MASK)
            >> _CMU_HFPERPRESC_PRESC_SHIFT;
      break;

#if defined(_CMU_HFPERPRESCB_MASK)
    case CMU_HFPERPRESCB_REG:
      ret = (IEC_P_INST(CMU)->HFPERPRESCB & _CMU_HFPERPRESCB_PRESC_MASK)
            >> _CMU_HFPERPRESCB_PRESC_SHIFT;
      break;
#endif
    ...
    case CMU_HFPERPRESCC_REG:
      ret = (IEC_P_INST(CMU)->HFPERPRESCC & _CMU_HFPERPRESCC_PRESC_MASK)
            >> _CMU_HFPERPRESCC_PRESC_SHIFT;
      break;
#endif

    case CMU_HFCOREPRESC_REG:
      ret = (IEC_P_INST(CMU)->HFCOREPRESC & _CMU_HFCOREPRESC_PRESC_MASK)
            >> _CMU_HFCOREPRESC_PRESC_SHIFT;
      break;
    ...
    case CMU_LFBPRESC0_REG:
      switch (clock) {
        ...
        case cmuClock_CSEN_LF:
          ret = (IEC_P_INST(CMU)->LFBPRESC0 & _CMU_LFBPRESC0_CSEN_MASK)
    ...
    case CMU_ADCASYNCDIV_REG:
      switch (clock) {
        ...
        case cmuClock_ADC0ASYNC:
          ret = (IEC_P_INST(CMU)->ADCCTRL & _CMU_ADCCTRL_ADC0CLKDIV_MASK)
                >> _CMU_ADCCTRL_ADC0CLKDIV_SHIFT;
          break;
#endif
#if defined(_CMU_ADCCTRL_ADC1CLKDIV_MASK)
        case cmuClock_ADC1ASYNC:
          ret = (IEC_P_INST(CMU)->ADCCTRL & _CMU_ADCCTRL_ADC1CLKDIV_MASK)
                >> _CMU_ADCCTRL_ADC1CLKDIV_SHIFT;
          break;
#endif
        default:
    ...
    case CMU_HFBUSPRESC_REG:
      ret = (IEC_P_INST(CMU)->HFBUSPRESC & _CMU_HFBUSPRESC_MASK)
            >> _CMU_HFBUSPRESC_PRESC_SHIFT;
      break;
#endif
    default:
...
void CMU_ClockPrescSet(CMU_Clock_TypeDef clock, CMU_ClkPresc_TypeDef presc)
{
  ...
  switch (prescReg) {
    case CMU_HFPRESC_REG:
      ...
      IEC_P_INST(CMU)->HFPRESC = (IEC_P_INST(CMU)->HFPRESC & ~_CMU_HFPRESC_PRESC_MASK)
    ...
    case CMU_HFEXPPRESC_REG:
      ...
      IEC_P_INST(CMU)->HFEXPPRESC = (IEC_P_INST(CMU)->HFEXPPRESC & ~_CMU_HFEXPPRESC_PRESC_MASK)
                        | (presc << _CMU_HFEXPPRESC_PRESC_SHIFT);
      break;

    case CMU_HFCLKLEPRESC_REG:
      presc = CMU_DivToLog2(presc);
      IEC_P_INST(CMU)->HFPRESC = (IEC_P_INST(CMU)->HFPRESC & ~_CMU_HFPRESC_HFCLKLEPRESC_MASK)
                     | (presc << _CMU_HFPRESC_HFCLKLEPRESC_SHIFT);
      break;

    case CMU_HFPERPRESC_REG:
      EFM_ASSERT(presc < 512U);
      IEC_P_INST(CMU)->HFPERPRESC = (IEC_P_INST(CMU)->HFPERPRESC & ~_CMU_HFPERPRESC_PRESC_MASK)
    ...
    case CMU_HFPERPRESCB_REG:
      EFM_ASSERT(presc < 512U);
      IEC_P_INST(CMU)->HFPERPRESCB = (IEC_P_INST(CMU)->HFPERPRESCB & ~_CMU_HFPERPRESCB_PRESC_MASK)
    ...
    case CMU_HFPERPRESCC_REG:
      EFM_ASSERT(presc < 512U);
      IEC_P_INST(CMU)->HFPERPRESCC = (IEC_P_INST(CMU)->HFPERPRESCC & ~_CMU_HFPERPRESCC_PRESC_MASK)
    ...
    case CMU_HFCOREPRESC_REG:
      ...
      IEC_P_INST(CMU)->HFCOREPRESC = (IEC_P_INST(CMU)->HFCOREPRESC & ~_CMU_HFCOREPRESC_PRESC_MASK)
    ...
    case CMU_LFAPRESC0_REG:
      switch (clock) {
        ...
        case cmuClock_RTCC:
          ...
          IEC_P_INST(CMU)->LFEPRESC0 = (IEC_P_INST(CMU)->LFEPRESC0 & ~_CMU_LFEPRESC0_RTCC_MASK)
          ...
          IEC_P_INST(CMU)->LFAPRESC0 = (IEC_P_INST(CMU)->LFAPRESC0 & ~_CMU_LFAPRESC0_RTCC_MASK)
        ...
        case cmuClock_LETIMER1:
          ...
          IEC_P_INST(CMU)->LFAPRESC0 = (IEC_P_INST(CMU)->LFAPRESC0 & ~_CMU_LFAPRESC0_LETIMER1_MASK)
    ...
    case CMU_LFBPRESC0_REG:
      switch (clock) {
        ...
        case cmuClock_CSEN_LF:
          ...
          IEC_P_INST(CMU)->LFBPRESC0 = (IEC_P_INST(CMU)->LFBPRESC0 & ~_CMU_LFBPRESC0_CSEN_MASK)
    ...
    case CMU_ADCASYNCDIV_REG:
      switch (clock) {
        ...
        case cmuClock_ADC0ASYNC:
          EFM_ASSERT(presc <= 3);
          IEC_P_INST(CMU)->ADCCTRL = (IEC_P_INST(CMU)->ADCCTRL & ~_CMU_ADCCTRL_ADC0CLKDIV_MASK)
        ...
        case cmuClock_ADC1ASYNC:
          EFM_ASSERT(presc <= 3);
          IEC_P_INST(CMU)->ADCCTRL = (IEC_P_INST(CMU)->ADCCTRL & ~_CMU_ADCCTRL_ADC1CLKDIV_MASK)
    ...
    case CMU_HFBUSPRESC_REG:
      ...
      IEC_P_INST(CMU)->HFBUSPRESC = (IEC_P_INST(CMU)->HFBUSPRESC & ~_CMU_HFBUSPRESC_MASK)
...
CMU_Select_TypeDef CMU_ClockSelectGet(CMU_Clock_TypeDef clock)
{
  ...
  switch (selReg) {
    case CMU_HFCLKSEL_REG:
      ...
      switch (IEC_P_INST(CMU)->HFCLKSTATUS & _CMU_HFCLKSTATUS_SELECTED_MASK) {
...
#if defined(_CMU_LFCLKSEL_MASK)
      switch (IEC_P_INST(CMU)->LFCLKSEL & _CMU_LFCLKSEL_LFA_MASK) {
        ...
        default:
#if defined(CMU_LFCLKSEL_LFAE)
          if (IEC_P_INST(CMU)->LFCLKSEL & _CMU_LFCLKSEL_LFAE_MASK) {
...
#elif defined(_CMU_LFACLKSEL_MASK)
      switch (IEC_P_INST(CMU)->LFACLKSEL & _CMU_LFACLKSEL_LFA_MASK) {
...
#if defined(_CMU_LFCLKSEL_MASK)
      switch (IEC_P_INST(CMU)->LFCLKSEL & _CMU_LFCLKSEL_LFB_MASK) {
        ...
        default:
          ...
          if (IEC_P_INST(CMU)->LFCLKSEL & _CMU_LFCLKSEL_LFBE_MASK) {
            ret = cmuSelect_ULFRCO;
            break;
          }
#else
...
#elif defined(_CMU_LFBCLKSEL_MASK)
      switch (IEC_P_INST(CMU)->LFBCLKSEL & _CMU_LFBCLKSEL_LFB_MASK) {
    ...
    case CMU_LFCCLKSEL_REG:
      switch (IEC_P_INST(CMU)->LFCLKSEL & _CMU_LFCLKSEL_LFC_MASK) {
    ...
    case CMU_LFECLKSEL_REG:
      switch (IEC_P_INST(CMU)->LFECLKSEL & _CMU_LFECLKSEL_LFE_MASK) {
    ...
    case CMU_LFCCLKSEL_REG:
      switch (IEC_P_INST(CMU)->LFCCLKSEL & _CMU_LFCCLKSEL_LFC_MASK) {
    ...
    case CMU_DBGCLKSEL_REG:
      ...
      switch (IEC_P_INST(CMU)->DBGCLKSEL & _CMU_DBGCLKSEL_DBG_MASK) {
      ...
      switch (IEC_P_INST(CMU)->CTRL & _CMU_CTRL_DBGCLK_MASK) {
    ...
    case CMU_ADC0ASYNCSEL_REG:
      switch (IEC_P_INST(CMU)->ADCCTRL & _CMU_ADCCTRL_ADC0CLKSEL_MASK) {
    ...
    case CMU_ADC1ASYNCSEL_REG:
      switch (IEC_P_INST(CMU)->ADCCTRL & _CMU_ADCCTRL_ADC1CLKSEL_MASK) {
    ...
    case CMU_SDIOREFSEL_REG:
      switch (IEC_P_INST(CMU)->SDIOCTRL & _CMU_SDIOCTRL_SDIOCLKSEL_MASK) {
    ...
    case CMU_QSPI0REFSEL_REG:
      switch (IEC_P_INST(CMU)->QSPICTRL & _CMU_QSPICTRL_QSPI0CLKSEL_MASK) {
    ...
    case CMU_USBRCLKSEL_REG:
      switch (IEC_P_INST(CMU)->USBCTRL & _CMU_USBCTRL_USBCLKSEL_MASK) {
    ...
    case CMU_PDMREFSEL_REG:
      switch (IEC_P_INST(CMU)->PDMCTRL & _CMU_PDMCTRL_PDMCLKSEL_MASK) {
...
void CMU_ClockSelectSet(CMU_Clock_TypeDef clock, CMU_Select_TypeDef ref)
{
  ...
  switch (selRegId) {
    case CMU_HFCLKSEL_REG:
      switch (ref) {
        ...
        case cmuSelect_HFXO:
          ...
          if (SystemHFXOClockGet() > 32000000) {
            IEC_P_INST(CMU)->CTRL = (IEC_P_INST(CMU)->CTRL & ~_CMU_CTRL_HFXOBUFCUR_MASK)
      ...
      IEC_P_INST(CMU)->HFCLKSEL = select;
#else
      IEC_P_INST(CMU)->CMD = select;
#endif
      /* Update the CMSIS core clock variable. */
      /* (The function will update the global variable). */
      freq = SystemCoreClockGet();
...
#if defined(_SILICON_LABS_32B_SERIES_1)
    case CMU_LFACLKSEL_REG:
      selReg = &IEC_P_INST(CMU)->LFACLKSEL;
    ...
    case CMU_LFCCLKSEL_REG:
      selReg = (selReg == NULL) ? &IEC_P_INST(CMU)->LFCCLKSEL : selReg;
    ...
    case CMU_LFECLKSEL_REG:
      selReg = (selReg == NULL) ? &IEC_P_INST(CMU)->LFECLKSEL : selReg;
    ...
    case CMU_LFBCLKSEL_REG:
      selReg = (selReg == NULL) ? &IEC_P_INST(CMU)->LFBCLKSEL : selReg;
      switch (ref) {
        case cmuSelect_Disabled:
        ...
        case cmuSelect_HFCLKLE:
          ...
          setHfLeConfig(SystemHFClockGet());
          BUS_RegBitWrite(&IEC_P_INST(CMU)->HFBUSCLKEN0, _CMU_HFBUSCLKEN0_LE_SHIFT, 1);
    ...
    case CMU_LFBCLKSEL_REG:
      switch (ref) {
        ...
        case cmuSelect_HFCLKLE:
          ...
          BUS_RegBitWrite(&IEC_P_INST(CMU)->HFCORECLKEN0, _CMU_HFCORECLKEN0_LE_SHIFT, 1);
      ...
      if (selRegId == CMU_LFACLKSEL_REG) {
        ...
        IEC_P_INST(CMU)->LFCLKSEL = (IEC_P_INST(CMU)->LFCLKSEL
                         & ~(_CMU_LFCLKSEL_LFA_MASK | _CMU_LFCLKSEL_LFAE_MASK))
                        | (tmp << _CMU_LFCLKSEL_LFA_SHIFT)
                        | (lfExtended << _CMU_LFCLKSEL_LFAE_SHIFT);
#else
        IEC_P_INST(CMU)->LFCLKSEL = (IEC_P_INST(CMU)->LFCLKSEL & ~_CMU_LFCLKSEL_LFA_MASK)
                        | (tmp << _CMU_LFCLKSEL_LFA_SHIFT);
#endif
      } else {
...
#else
        IEC_P_INST(CMU)->LFCLKSEL = (IEC_P_INST(CMU)->LFCLKSEL & ~_CMU_LFCLKSEL_LFB_MASK)
                        | (tmp << _CMU_LFCLKSEL_LFB_SHIFT);
#endif
      }
      break;
    ...
    case CMU_LFCCLKSEL_REG:
      ...
      /* Apply select. */
      IEC_P_INST(CMU)->LFCLKSEL = (IEC_P_INST(CMU)->LFCLKSEL & ~_CMU_LFCLKSEL_LFC_MASK)
    ...
    case CMU_DBGCLKSEL_REG:
      switch (ref) {
        ...
        case cmuSelect_AUXHFRCO:
          ...
          IEC_P_INST(CMU)->DBGCLKSEL = CMU_DBGCLKSEL_DBG_AUXHFRCO;
          break;

        case cmuSelect_HFCLK:
          /* Select divided HFCLK as a debug clock. */
          IEC_P_INST(CMU)->DBGCLKSEL = CMU_DBGCLKSEL_DBG_HFCLK;
          break;
#endif
        ...
        case cmuSelect_AUXHFRCO:
          /* Select AUXHFRCO as a debug clock. */
          IEC_P_INST(CMU)->CTRL = (IEC_P_INST(CMU)->CTRL & ~(_CMU_CTRL_DBGCLK_MASK))
    ...
    case CMU_USBCCLKSEL_REG:
      switch (ref) {
        case cmuSelect_LFXO:
          ...
          IEC_P_INST(CMU)->CMD = CMU_CMD_USBCCLKSEL_LFXO;

          /* Wait until the clock is activated. */
          while ((IEC_P_INST(CMU)->STATUS & CMU_STATUS_USBCLFXOSEL) == 0) {
          }
          break;
        ...
        case cmuSelect_LFRCO:
          ...
          /* Switch the oscillator. */
          IEC_P_INST(CMU)->CMD = CMU_CMD_USBCCLKSEL_LFRCO;

          /* Wait until the clock is activated. */
          while ((IEC_P_INST(CMU)->STATUS & CMU_STATUS_USBCLFRCOSEL) == 0) {
          }
          break;
        ...
        case cmuSelect_HFCLK:
          ...
          IEC_P_INST(CMU)->CMD = CMU_CMD_USBCCLKSEL_HFCLKNODIV;
          /* Wait until the clock is activated. */
          while ((IEC_P_INST(CMU)->STATUS & CMU_STATUS_USBCHFCLKSEL) == 0) {
          }
          break;
#endif
        ...
        case cmuSelect_USHFRCO:
          ...
          /* Switch the oscillator. */
          IEC_P_INST(CMU)->CMD = CMU_CMD_USBCCLKSEL_USHFRCO;

          /* Wait until the clock is activated. */
          while ((IEC_P_INST(CMU)->STATUS & CMU_STATUS_USBCUSHFRCOSEL) == 0) {
          }
          break;
#endif
    ...
    case CMU_ADC0ASYNCSEL_REG:
      ...
      /* Apply select. */
      IEC_P_INST(CMU)->ADCCTRL = (IEC_P_INST(CMU)->ADCCTRL & ~_CMU_ADCCTRL_ADC0CLKSEL_MASK)
    ...
    case CMU_ADC1ASYNCSEL_REG:
      ...
      /* Apply select. */
      IEC_P_INST(CMU)->ADCCTRL = (IEC_P_INST(CMU)->ADCCTRL & ~_CMU_ADCCTRL_ADC1CLKSEL_MASK)
    ...
    case CMU_SDIOREFSEL_REG:
      ...
      /* Apply select. */
      IEC_P_INST(CMU)->SDIOCTRL = (IEC_P_INST(CMU)->SDIOCTRL & ~_CMU_SDIOCTRL_SDIOCLKSEL_MASK)
    ...
    case CMU_QSPI0REFSEL_REG:
      ...
      /* Apply select. */
      IEC_P_INST(CMU)->QSPICTRL = (IEC_P_INST(CMU)->QSPICTRL & ~_CMU_QSPICTRL_QSPI0CLKSEL_MASK)
    ...
    case CMU_USBRCLKSEL_REG:
      switch (ref) {
        ...
        case cmuSelect_HFXOX2:
          ...
          /* Enable HFXO X2. */
          IEC_P_INST(CMU)->HFXOCTRL |= CMU_HFXOCTRL_HFXOX2EN;
      ...
      /* Apply select. */
      IEC_P_INST(CMU)->USBCTRL = (IEC_P_INST(CMU)->USBCTRL & ~_CMU_USBCTRL_USBCLKSEL_MASK)
    ...
    case CMU_PDMREFSEL_REG:
      ...
      /* Apply select. */
      IEC_P_INST(CMU)->PDMCTRL = (IEC_P_INST(CMU)->PDMCTRL & ~_CMU_PDMCTRL_PDMCLKSEL_MASK)
...
bool CMU_DPLLLock(const CMU_DPLLInit_TypeDef *init)
{
  ...
  if (CMU_ClockSelectGet(cmuClock_HF) == cmuSelect_HFRCO) {
    ...
    IEC_P_INST(CMU)->HFCLKSEL = CMU_HFCLKSEL_HF_HFRCODIV2;
  }

  IEC_P_INST(CMU)->OSCENCMD  = CMU_OSCENCMD_DPLLDIS;
  while ((IEC_P_INST(CMU)->STATUS & (CMU_STATUS_DPLLENS | CMU_STATUS_DPLLRDY)) != 0UL) {
  }
  IEC_P_INST(CMU)->IFC       = CMU_IFC_DPLLRDY | CMU_IFC_DPLLLOCKFAILLOW
                   | CMU_IFC_DPLLLOCKFAILHIGH;
  IEC_P_INST(CMU)->DPLLCTRL1 = ((uint32_t)init->n   << _CMU_DPLLCTRL1_N_SHIFT)
                   | ((uint32_t)init->m << _CMU_DPLLCTRL1_M_SHIFT);
  IEC_P_INST(CMU)->HFRCOCTRL = hfrcoCtrlVal;
  IEC_P_INST(CMU)->DPLLCTRL  = ((uint32_t)init->refClk << _CMU_DPLLCTRL_REFSEL_SHIFT)
  ...
  IEC_P_INST(CMU)->OSCENCMD  = CMU_OSCENCMD_DPLLEN;
  while ((lockStatus = (IEC_P_INST(CMU)->IF & (CMU_IF_DPLLRDY
  ...
  if ((CMU_ClockSelectGet(cmuClock_HF) == cmuSelect_HFRCODIV2)
      && (hfrcoDiv2override == true)) {
    IEC_P_INST(CMU)->HFCLKSEL = CMU_HFCLKSEL_HF_HFRCO;
  }
...
void CMU_FreezeEnable(bool enable)
{
  if (enable) {
    ...
    /* avoided.                                                             */
    while (IEC_P_INST(CMU)->SYNCBUSY != 0UL) {
    }

    IEC_P_INST(CMU)->FREEZE = CMU_FREEZE_REGFREEZE;
  } else {
    IEC_P_INST(CMU)->FREEZE = 0;
  }
}
...
{
  return (CMU_HFRCOBand_TypeDef)((IEC_P_INST(CMU)->HFRCOCTRL & _CMU_HFRCOCTRL_BAND_MASK)
...
void CMU_HFRCOBandSet(CMU_HFRCOBand_TypeDef band)
{
  ...
  /* Set band/tuning. */
  IEC_P_INST(CMU)->HFRCOCTRL = (IEC_P_INST(CMU)->HFRCOCTRL
...
void CMU_HFRCOBandSet(CMU_HFRCOFreq_TypeDef setFreq)
{
  ...
  /* Wait for any previous sync to complete and set calibration data
     for the selected frequency.  */
  while (BUS_RegBitRead(&IEC_P_INST(CMU)->SYNCBUSY, _CMU_SYNCBUSY_HFRCOBSY_SHIFT) != 0UL) {
  }
  ...
  IEC_P_INST(CMU)->HFRCOCTRL = freqCal;
...
{
  return (IEC_P_INST(CMU)->HFRCOCTRL & _CMU_HFRCOCTRL_SUDELAY_MASK)
...
void CMU_HFRCOStartupDelaySet(uint32_t delay)
{
  ...
  IEC_P_INST(CMU)->HFRCOCTRL = (IEC_P_INST(CMU)->HFRCOCTRL & ~(_CMU_HFRCOCTRL_SUDELAY_MASK))
...
void CMU_USHFRCOBandSet(CMU_USHFRCOFreq_TypeDef setFreq)
{
  ...
  /* Wait for any previous sync to complete and set calibration data
     for the selected frequency.  */
  while (BUS_RegBitRead(&IEC_P_INST(CMU)->SYNCBUSY, _CMU_SYNCBUSY_USHFRCOBSY_SHIFT)) ;

  IEC_P_INST(CMU)->USHFRCOCTRL = freqCal;
}
                             ...
                             bool enEM0EM1StartSel)
{
  ...
  hfxoCtrl = IEC_P_INST(CMU)->HFXOCTRL & ~(userSel
  ...
  IEC_P_INST(CMU)->HFXOCTRL = hfxoCtrl;
}
#endif
...
void CMU_HFXOInit(const CMU_HFXOInit_TypeDef *hfxoInit)
{
  ...
  IEC_P_INST(CMU)->HFXOCTRL = (IEC_P_INST(CMU)->HFXOCTRL & ~_CMU_HFXOCTRL_MODE_MASK) | tmp;
  ...
  IEC_P_INST(CMU)->HFXOCTRL = (IEC_P_INST(CMU)->HFXOCTRL & ~_CMU_HFXOCTRL_HFXOX2EN_MASK) | tmp;
#endif

  /* Set tuning for startup and steady state. */
  IEC_P_INST(CMU)->HFXOSTARTUPCTRL = (hfxoInit->ctuneStartup
                          << _CMU_HFXOSTARTUPCTRL_CTUNE_SHIFT)
                         | (hfxoInit->xoCoreBiasTrimStartup
                            << _CMU_HFXOSTARTUPCTRL_IBTRIMXOCORE_SHIFT);

  IEC_P_INST(CMU)->HFXOSTEADYSTATECTRL = (IEC_P_INST(CMU)->HFXOSTEADYSTATECTRL
  ...
  /* Set timeouts */
  IEC_P_INST(CMU)->HFXOTIMEOUTCTRL = (hfxoInit->timeoutPeakDetect
  ...
  IEC_P_INST(CMU)->HFXOCTRL = (IEC_P_INST(CMU)->HFXOCTRL & ~_CMU_HFXOCTRL_MODE_MASK)
                  | tmp;
  BUS_RegBitWrite(&IEC_P_INST(CMU)->HFXOCTRL,
  ...
  IEC_P_INST(CMU)->HFXOCTRL1 = (IEC_P_INST(CMU)->HFXOCTRL1 & ~_CMU_HFXOCTRL1_PEAKDETTHR_MASK)
                   | (hfxoInit->thresholdPeakDetect
                      << _CMU_HFXOCTRL1_PEAKDETTHR_SHIFT);
#endif
  /* Set tuning for startup and steady state. */
  IEC_P_INST(CMU)->HFXOSTARTUPCTRL = ((uint32_t)hfxoInit->ctuneStartup
                          << _CMU_HFXOSTARTUPCTRL_CTUNE_SHIFT)
  ...
  /* Set timeouts. */
  IEC_P_INST(CMU)->HFXOTIMEOUTCTRL = ((uint32_t)hfxoInit->timeoutPeakDetect
                          << _CMU_HFXOTIMEOUTCTRL_PEAKDETTIMEOUT_SHIFT)
...
#else
  IEC_P_INST(CMU)->CTRL = (IEC_P_INST(CMU)->CTRL & ~(_CMU_CTRL_HFXOTIMEOUT_MASK
...
uint32_t CMU_LCDClkFDIVGet(void)
{
  ...
  return (IEC_P_INST(CMU)->LCDCTRL & _CMU_LCDCTRL_FDIV_MASK) >> _CMU_LCDCTRL_FDIV_SHIFT;
#else
  return 0;
#endif /* defined(LCD_PRESENT) */
}
...
void CMU_LCDClkFDIVSet(uint32_t div)
{
  ...
  if (IEC_P_INST(CMU)->LFACLKEN0 & CMU_LFACLKEN0_LCD) {
    return;
  }

  div        <<= _CMU_LCDCTRL_FDIV_SHIFT;
  div         &= _CMU_LCDCTRL_FDIV_MASK;
  IEC_P_INST(CMU)->LCDCTRL = (IEC_P_INST(CMU)->LCDCTRL & ~_CMU_LCDCTRL_FDIV_MASK) | div;
#else
...
void CMU_LFXOInit(const CMU_LFXOInit_TypeDef *lfxoInit)
{
...
#if defined(_SILICON_LABS_32B_SERIES_1)
  uint32_t reg = (IEC_P_INST(CMU)->LFXOCTRL & ~(_CMU_LFXOCTRL_TUNING_MASK
  ...
  if (reg != IEC_P_INST(CMU)->LFXOCTRL) {
    CMU_OscillatorEnable(cmuOsc_LFXO, false, true);
    IEC_P_INST(CMU)->LFXOCTRL = reg;
  }
#elif defined(_SILICON_LABS_32B_SERIES_0)
  ...
  BUS_RegMaskedWrite(&IEC_P_INST(CMU)->CTRL,
  ...
  if (enable) {
    ...
    if (osc == cmuOsc_HFXO) {
      if (getHfxoTuningMode() == HFXO_TUNING_MODE_AUTO) {
        ...
        if ((IEC_P_INST(CMU)->HFXOCTRL & (_CMU_HFXOCTRL_MODE_MASK)) == CMU_HFXOCTRL_MODE_DIGEXTCLK) {
          setHfxoTuningMode(HFXO_TUNING_MODE_CMD);
        }
      }
    }
#endif
    IEC_P_INST(CMU)->OSCENCMD = enBit;

#if defined(_SILICON_LABS_32B_SERIES_1)
    /* Always wait for ENS to go high. */
    while (BUS_RegBitRead(&IEC_P_INST(CMU)->STATUS, ensBitPos) == 0UL) {
    }
#endif

    /* Wait for the clock to become ready after enable. */
    if (wait) {
      while (BUS_RegBitRead(&IEC_P_INST(CMU)->STATUS, rdyBitPos) == 0UL) {
      }
#if defined(_SILICON_LABS_32B_SERIES_1)
      if ((osc == cmuOsc_HFXO) && firstHfxoEnable) {
        if ((IEC_P_INST(CMU)->HFXOCTRL & _CMU_HFXOCTRL_MODE_MASK)
    ...
    IEC_P_INST(CMU)->OSCENCMD = disBit;

#if defined(_SILICON_LABS_32B_SERIES_1)
    /* Always wait for ENS to go low. */
    while ((IEC_P_INST(CMU)->STATUS & (0x1 << ensBitPos)) != 0U) {
    }

    if (wait) {
      /* Wait for RDY to go low as well. */
      while ((IEC_P_INST(CMU)->STATUS & (0x1 << rdyBitPos)) != 0U) {
      }
    }
#endif
  }
...
uint32_t CMU_OscillatorTuningGet(CMU_Osc_TypeDef osc)
{
  ...
  switch (osc) {
    case cmuOsc_LFRCO:
      ret = (IEC_P_INST(CMU)->LFRCOCTRL & _CMU_LFRCOCTRL_TUNING_MASK)
            >> _CMU_LFRCOCTRL_TUNING_SHIFT;
      break;

    case cmuOsc_HFRCO:
      ret = (IEC_P_INST(CMU)->HFRCOCTRL & _CMU_HFRCOCTRL_TUNING_MASK)
            >> _CMU_HFRCOCTRL_TUNING_SHIFT;
      break;

#if defined (_CMU_USHFRCOCTRL_TUNING_MASK)
    case cmuOsc_USHFRCO:
      ret = (IEC_P_INST(CMU)->USHFRCOCTRL & _CMU_USHFRCOCTRL_TUNING_MASK)
            >> _CMU_USHFRCOCTRL_TUNING_SHIFT;
      break;
#endif

    case cmuOsc_AUXHFRCO:
      ret = (IEC_P_INST(CMU)->AUXHFRCOCTRL & _CMU_AUXHFRCOCTRL_TUNING_MASK)
            >> _CMU_AUXHFRCOCTRL_TUNING_SHIFT;
      break;

#if defined(_CMU_HFXOCTRL_PEAKDETSHUNTOPTMODE_MASK)
    case cmuOsc_HFXO:
      ret = IEC_P_INST(CMU)->HFXOTRIMSTATUS & (_CMU_HFXOTRIMSTATUS_IBTRIMXOCORE_MASK
...
void CMU_OscillatorTuningSet(CMU_Osc_TypeDef osc, uint32_t val)
{
  ...
  switch (osc) {
...
#endif
      IEC_P_INST(CMU)->LFRCOCTRL = (IEC_P_INST(CMU)->LFRCOCTRL & ~(_CMU_LFRCOCTRL_TUNING_MASK))
...
#endif
      IEC_P_INST(CMU)->HFRCOCTRL = (IEC_P_INST(CMU)->HFRCOCTRL & ~(_CMU_HFRCOCTRL_TUNING_MASK))
...
#endif
      IEC_P_INST(CMU)->USHFRCOCTRL = (IEC_P_INST(CMU)->USHFRCOCTRL & ~(_CMU_USHFRCOCTRL_TUNING_MASK))
...
#endif
      IEC_P_INST(CMU)->AUXHFRCOCTRL = (IEC_P_INST(CMU)->AUXHFRCOCTRL & ~(_CMU_AUXHFRCOCTRL_TUNING_MASK))
    ...
    case cmuOsc_HFXO:
      ...
      EFM_ASSERT((IEC_P_INST(CMU)->STATUS & CMU_STATUS_HFXOENS) == 0UL);
                              ...
                              CMU_HFXOTuningMode_TypeDef mode)
{
  ...
  while ((IEC_P_INST(CMU)->STATUS & waitFlags) != waitFlags) {
  }

#if defined(CMU_IF_HFXOPEAKDETERR)
  /* Check error flags. */
  if ((waitFlags & CMU_STATUS_HFXOPEAKDETRDY) != 0UL) {
    return (IEC_P_INST(CMU)->IF & CMU_IF_HFXOPEAKDETERR) != 0UL ? true : false;
  }
#endif
  return true;
}
  ...
  switch (osc) {
    case cmuOsc_HFXO:
      if ((unsigned)mode != 0U) {
        ...
        IEC_P_INST(CMU)->IFC = CMU_IFC_HFXOPEAKDETERR;
#endif
        IEC_P_INST(CMU)->CMD = (uint32_t)mode;
      }
      if (wait) {
...
bool CMU_PCNTClockExternalGet(unsigned int instance)
{
  ...
  switch (instance) {
    ...
    case 0:
      setting = IEC_P_INST(CMU)->PCNTCTRL & CMU_PCNTCTRL_PCNT0CLKSEL_PCNT0S0;
      break;

#if defined(_CMU_PCNTCTRL_PCNT1CLKEN_MASK)
    case 1:
      setting = IEC_P_INST(CMU)->PCNTCTRL & CMU_PCNTCTRL_PCNT1CLKSEL_PCNT1S0;
      break;

#if defined(_CMU_PCNTCTRL_PCNT2CLKEN_MASK)
    case 2:
      setting = IEC_P_INST(CMU)->PCNTCTRL & CMU_PCNTCTRL_PCNT2CLKSEL_PCNT2S0;
      break;
#endif
#endif
#endif
...
void CMU_PCNTClockExternalSet(unsigned int instance, bool external)
{
  ...
  BUS_RegBitWrite(&(IEC_P_INST(CMU)->PCNTCTRL), (instance * 2U) + 1U, setting);
...
{
  return (CMU_USHFRCOBand_TypeDef)((IEC_P_INST(CMU)->USHFRCOCONF
...
void CMU_USHFRCOBandSet(CMU_USHFRCOBand_TypeDef band)
{
  ...
  switch (band) {
    ...
    case cmuUSHFRCOBand_48MHz:
      ...
      BUS_RegBitWrite(&IEC_P_INST(CMU)->USHFRCOCONF, _CMU_USHFRCOCONF_USHFRCODIV2DIS_SHIFT, 0);
  ...
  /* Set band and tuning. */
  IEC_P_INST(CMU)->USHFRCOCONF = (IEC_P_INST(CMU)->USHFRCOCONF & ~_CMU_USHFRCOCONF_BAND_MASK)
                     | (band << _CMU_USHFRCOCONF_BAND_SHIFT);
  IEC_P_INST(CMU)->USHFRCOCTRL = (IEC_P_INST(CMU)->USHFRCOCTRL & ~_CMU_USHFRCOCTRL_TUNING_MASK)
                     | (tuning << _CMU_USHFRCOCTRL_TUNING_SHIFT);
  IEC_P_INST(CMU)->USHFRCOTUNE = (IEC_P_INST(CMU)->USHFRCOTUNE & ~_CMU_USHFRCOTUNE_FINETUNING_MASK)
  ...
  if (band == cmuUSHFRCOBand_24MHz) {
    BUS_RegBitWrite(&IEC_P_INST(CMU)->USHFRCOCONF, _CMU_USHFRCOCONF_USHFRCODIV2DIS_SHIFT, 1);
  }
}
#endif
