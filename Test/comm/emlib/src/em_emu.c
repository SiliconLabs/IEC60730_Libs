...
static void emState(emState_TypeDef action)
{
  ...
  if (action == emState_Save) {
    ...
    /* Save vscale. */
    EMU_VScaleWait();
    vScaleStatus = (uint8_t)((IEC_P_INST(EMU)->STATUS & _EMU_STATUS_VSCALE_MASK)
...
#endif
    {
      ...
      IEC_P_INST(EMU)->CMD = vScaleEM01Cmd((EMU_VScaleEM01_TypeDef)vScaleStatus);
    }
#endif
    ...
    EMU_VScaleWait();
    if ((IEC_P_INST(EMU)->CTRL & _EMU_CTRL_EM23VSCALEAUTOWSEN_MASK) != 0U) {
...
static void dcdcFetCntSet(bool lpModeSet)
{
  ...
  if (lpModeSet) {
    emuDcdcMiscCtrlReg = IEC_P_INST(EMU)->DCDCMISCCTRL;
    tmp  = IEC_P_INST(EMU)->DCDCMISCCTRL
    ...
    IEC_P_INST(EMU)->DCDCMISCCTRL = tmp;
    currentLimitersUpdate();
  } else {
    IEC_P_INST(EMU)->DCDCMISCCTRL = emuDcdcMiscCtrlReg;
    currentLimitersUpdate();
  }
}
#endif
...
#define EMU_DCDCSTATUS  (*(volatile uint32_t *)(EMU_BASE + 0x7C))
  if ((errataFixDcdcHsState == errataFixDcdcHsTrimSet)
    ...
    /* Wait for LNRUNNING */
    if ((IEC_P_INST(EMU)->DCDCCTRL & _EMU_DCDCCTRL_DCDCMODE_MASK) == EMU_DCDCCTRL_DCDCMODE_LOWNOISE) {
...
static void vScaleDownEM23Setup(void)
{
  ...
  uint32_t em23vs = (IEC_P_INST(EMU)->CTRL & _EMU_CTRL_EM23VSCALE_MASK) >> _EMU_CTRL_EM23VSCALE_SHIFT;
  uint32_t em01vs = (IEC_P_INST(EMU)->STATUS & _EMU_STATUS_VSCALE_MASK) >> _EMU_STATUS_VSCALE_SHIFT;

  /* Inverse coding. */
  if (em23vs > em01vs) {
    IEC_P_INST(EMU)->CTRL |= EMU_CTRL_EM23VSCALEAUTOWSEN;
...
{
  if ((IEC_P_INST(EMU)->CTRL & EMU_CTRL_EM23VSCALEAUTOWSEN) != 0U) {
...
void EMU_EnterEM4(void)
{
  ...
  uint32_t em4seq2 = (IEC_P_INST(EMU)->EM4CTRL & ~_EMU_EM4CTRL_EM4ENTRY_MASK)
                     | (2U << _EMU_EM4CTRL_EM4ENTRY_SHIFT);
  uint32_t em4seq3 = (IEC_P_INST(EMU)->EM4CTRL & ~_EMU_EM4CTRL_EM4ENTRY_MASK)
                     | (3U << _EMU_EM4CTRL_EM4ENTRY_SHIFT);
#else
  uint32_t em4seq2 = (IEC_P_INST(EMU)->CTRL & ~_EMU_CTRL_EM4CTRL_MASK)
                     | (2U << _EMU_CTRL_EM4CTRL_SHIFT);
  uint32_t em4seq3 = (IEC_P_INST(EMU)->CTRL & ~_EMU_CTRL_EM4CTRL_MASK)
  ...
  if ((IEC_P_INST(EMU)->EM4CTRL & _EMU_EM4CTRL_EM4STATE_MASK) == EMU_EM4CTRL_EM4STATE_EM4S) {
    uint32_t dcdcMode = IEC_P_INST(EMU)->DCDCCTRL & _EMU_DCDCCTRL_DCDCMODE_MASK;
  ...
  if (IEC_P_INST(EMU)->EM4CTRL & EMU_EM4CTRL_EM4STATE_EM4H) {
    /* Fix for errata EMU_E208 - Occasional Full Reset After Exiting EM4H.
  ...
  *(volatile uint32_t *)0x400C80E4 = 0;
  ...
  if ((IEC_P_INST(EMU)->EM4CTRL & _EMU_EM4CTRL_EM4STATE_MASK) == EMU_EM4CTRL_EM4STATE_EM4S) {
    if ((IEC_P_INST(EMU)->PWRCTRL & _EMU_PWRCTRL_ANASW_MASK) == EMU_PWRCTRL_ANASW_DVDD) {
      BUS_RegMaskedClear(&IEC_P_INST(EMU)->PWRCTRL, _EMU_PWRCTRL_ANASW_MASK);
  ...
  for (i = 0; i < 4; i++) {
#if defined(_EMU_EM4CTRL_EM4ENTRY_SHIFT)
    IEC_P_INST(EMU)->EM4CTRL = em4seq2;
    IEC_P_INST(EMU)->EM4CTRL = em4seq3;
  }
  IEC_P_INST(EMU)->EM4CTRL = em4seq2;
#else
    IEC_P_INST(EMU)->CTRL = em4seq2;
    IEC_P_INST(EMU)->CTRL = em4seq3;
  }
  IEC_P_INST(EMU)->CTRL = em4seq2;
#endif
}
...
void EMU_EnterEM4H(void)
{
  ...
  BUS_RegBitWrite(&IEC_P_INST(EMU)->EM4CTRL, _EMU_EM4CTRL_EM4STATE_SHIFT, 1);
#endif
  EMU_EnterEM4();
}
...
{
#if defined(_EMU_EM4CTRL_EM4STATE_MASK)
  BUS_RegBitWrite(&IEC_P_INST(EMU)->EM4CTRL, _EMU_EM4CTRL_EM4STATE_SHIFT, 0);
#endif
  EMU_EnterEM4();
}
#endif
...
void EMU_MemPwrDown(uint32_t blocks)
{
  ...
  IEC_P_INST(EMU)->MEMCTRL = blocks & _EMU_MEMCTRL_MASK;
#elif defined(_EMU_RAM0CTRL_MASK)
  IEC_P_INST(EMU)->RAM0CTRL = blocks & _EMU_RAM0CTRL_MASK;
#else
  (void)blocks;
#endif
}
...
#if defined(_EMU_MEMCTRL_MASK)
  IEC_P_INST(EMU)->MEMCTRL = IEC_P_INST(EMU)->MEMCTRL   | mask;
#elif defined(_EMU_RAM0CTRL_MASK)
  IEC_P_INST(EMU)->RAM0CTRL = IEC_P_INST(EMU)->RAM0CTRL | mask;
...
void EMU_RamPowerUp(void)
{
  ...
  IEC_P_INST(EMU)->RAM0CTRL = 0x0UL;
#endif
#if defined(_EMU_RAM1CTRL_MASK)
  IEC_P_INST(EMU)->RAM1CTRL = 0x0UL;
#endif
#if defined(_EMU_RAM2CTRL_MASK)
  IEC_P_INST(EMU)->RAM2CTRL = 0x0UL;
#endif
...
{
  EFM_ASSERT(!enable);
  IEC_P_INST(EMU)->EM23PERNORETAINCTRL = (uint32_t)periMask
...
void EMU_VScaleEM01(EMU_VScaleEM01_TypeDef voltage, bool wait)
{
  ...
  CORE_ENTER_CRITICAL();
  IEC_P_INST(EMU)->IF_CLR = EMU_IF_VSCALEDONE;
  IEC_P_INST(EMU)->CMD = vScaleEM01Cmd(voltage);
  ...
  while (((IEC_P_INST(EMU)->IF & EMU_IF_VSCALEDONE) == 0U)
         && ((IEC_P_INST(EMU)->STATUS & EMU_STATUS_VSCALEFAILED) == 0U)) {
    EFM_ASSERT((IEC_P_INST(EMU)->STATUS & EMU_STATUS_VSCALEFAILED) == 0U);
...
void EMU_EM23Init(const EMU_EM23Init_TypeDef *em23Init)
{
  ...
  IEC_P_INST(EMU)->CTRL = em23Init->em23VregFullEn ? (IEC_P_INST(EMU)->CTRL | EMU_CTRL_EMVREG)
              : (IEC_P_INST(EMU)->CTRL & ~EMU_CTRL_EMVREG);
#elif defined(_EMU_CTRL_EM23VREG_MASK)
  IEC_P_INST(EMU)->CTRL = em23Init->em23VregFullEn ? (IEC_P_INST(EMU)->CTRL | EMU_CTRL_EM23VREG)
              : (IEC_P_INST(EMU)->CTRL & ~EMU_CTRL_EM23VREG);
#else
  (void)em23Init;
#endif

#if defined(EMU_VSCALE_PRESENT)
  IEC_P_INST(EMU)->CTRL = (IEC_P_INST(EMU)->CTRL & ~_EMU_CTRL_EM23VSCALE_MASK)
...
void EMU_EM4Init(const EMU_EM4Init_TypeDef *em4Init)
{
  ...
  /* Initialization for platforms with IEC_P_INST(EMU)->EM4CONF register. */
  uint32_t em4conf = IEC_P_INST(EMU)->EM4CONF;
  ...
  IEC_P_INST(EMU)->EM4CONF = em4conf;

#elif defined(_EMU_EM4CTRL_EM4STATE_MASK)
  /* Initialization for platforms with IEC_P_INST(EMU)->EM4CTRL register and EM4H and EM4S. */

  uint32_t em4ctrl = IEC_P_INST(EMU)->EM4CTRL;
  ...
  IEC_P_INST(EMU)->EM4CTRL = em4ctrl;
#elif defined(_EMU_EM4CTRL_MASK)
  IEC_P_INST(EMU)->EM4CTRL = (IEC_P_INST(EMU)->EM4CTRL & ~_EMU_EM4CTRL_EM4IORETMODE_MASK)
                 | (uint32_t)em4Init->pinRetentionMode;
#endif

#if defined(_EMU_CTRL_EM4HVSCALE_MASK)
  IEC_P_INST(EMU)->CTRL = (IEC_P_INST(EMU)->CTRL & ~_EMU_CTRL_EM4HVSCALE_MASK)
...
void EMU_BUPDInit(const EMU_BUPDInit_TypeDef *bupdInit)
{
  ...
  reg = IEC_P_INST(EMU)->PWRCONF & ~(_EMU_PWRCONF_PWRRES_MASK
  ...
  IEC_P_INST(EMU)->PWRCONF = reg;

  /* Set the backup domain inactive mode configuration. */
  reg = IEC_P_INST(EMU)->BUINACT & ~(_EMU_BUINACT_PWRCON_MASK);
  reg |= (bupdInit->inactivePower);
  IEC_P_INST(EMU)->BUINACT = reg;

  /* Set the backup domain active mode configuration. */
  reg = IEC_P_INST(EMU)->BUACT & ~(_EMU_BUACT_PWRCON_MASK);
  reg |= (bupdInit->activePower);
  IEC_P_INST(EMU)->BUACT = reg;

  /* Set the power control configuration */
  reg = IEC_P_INST(EMU)->BUCTRL & ~(_EMU_BUCTRL_PROBE_MASK
...
void EMU_BUThresholdSet(EMU_BODMode_TypeDef mode, uint32_t value)
{
  ...
  switch (mode) {
    case emuBODMode_Active:
      IEC_P_INST(EMU)->BUACT = (IEC_P_INST(EMU)->BUACT & ~_EMU_BUACT_BUEXTHRES_MASK)
                   | (value << _EMU_BUACT_BUEXTHRES_SHIFT);
      break;
    case emuBODMode_Inactive:
      IEC_P_INST(EMU)->BUINACT = (IEC_P_INST(EMU)->BUINACT & ~_EMU_BUINACT_BUENTHRES_MASK)
...
void EMU_BUThresRangeSet(EMU_BODMode_TypeDef mode, uint32_t value)
{
  ...
  switch (mode) {
    case emuBODMode_Active:
      IEC_P_INST(EMU)->BUACT = (IEC_P_INST(EMU)->BUACT & ~_EMU_BUACT_BUEXRANGE_MASK)
                   | (value << _EMU_BUACT_BUEXRANGE_SHIFT);
      break;
    case emuBODMode_Inactive:
      IEC_P_INST(EMU)->BUINACT = (IEC_P_INST(EMU)->BUINACT & ~_EMU_BUINACT_BUENRANGE_MASK)
...
void EMU_BUDisMaxCompSet(bool disableMainBuComparator)
{
  ...
  reg = IEC_P_INST(EMU)->BUCTRL & ~(_EMU_BUCTRL_DISMAXCOMP_MASK);
...
void EMU_BUBuInactPwrConSet(EMU_BUBuInactPwrCon_TypeDef inactPwrCon)
{
  ...
  reg = IEC_P_INST(EMU)->BUCTRL & ~(_EMU_BUCTRL_BUINACTPWRCON_MASK);
...
void EMU_BUBuActPwrConSet(EMU_BUBuActPwrCon_TypeDef actPwrCon)
{
  ...
  reg = IEC_P_INST(EMU)->BUCTRL & ~(_EMU_BUCTRL_BUACTPWRCON_MASK);
...
void EMU_BUPwrResSet(EMU_BUPwrRes_TypeDef pwrRes)
{
  ...
  reg = IEC_P_INST(EMU)->BUCTRL & ~(_EMU_BUCTRL_PWRRES_MASK);
  reg |= (uint32_t)(pwrRes);
  IEC_P_INST(EMU)->BUCTRL = reg;
}
#endif
...
void EMU_BUVoutResSet(EMU_BUVoutRes_TypeDef resistorSel)
{
  ...
  reg = IEC_P_INST(EMU)->BUCTRL & ~(_EMU_BUCTRL_VOUTRES_MASK);
...
void EMU_BUBuVinProbeEnSet(bool enable)
{
  ...
  reg = IEC_P_INST(EMU)->BUCTRL & ~(_EMU_BUCTRL_BUVINPROBEEN_MASK);
...
void EMU_BUStatEnSet(bool enable)
{
  ...
  reg = IEC_P_INST(EMU)->BUCTRL & ~(_EMU_BUCTRL_STATEN_MASK);
...
void EMU_BUEnableSet(bool enable)
{
  ...
  reg = IEC_P_INST(EMU)->BUCTRL & ~(_EMU_BUCTRL_EN_MASK);
...
#define EMU_DCDCSMCTRL  (*(volatile uint32_t *)(EMU_BASE + 0x44))
  ...
  IEC_P_INST(EMU)->DCDCLPCTRL |= EMU_DCDCLPCTRL_LPVREFDUTYEN;

  /* Set low-noise RCO for LNFORCECCM configuration.
   * LNFORCECCM is default 1 for EFR32
   * LNFORCECCM is default 0 for EFM32
   */
  lnForceCcm = BUS_RegBitRead(&IEC_P_INST(EMU)->DCDCMISCCTRL, _EMU_DCDCMISCCTRL_LNFORCECCM_SHIFT);
  if (lnForceCcm != 0U) {
...
#if defined(_SILICON_LABS_GECKO_INTERNAL_SDID_80)
  IEC_P_INST(EMU)->DCDCTIMING &= ~_EMU_DCDCTIMING_DUTYSCALE_MASK;
  IEC_P_INST(EMU)->DCDCMISCCTRL |= EMU_DCDCMISCCTRL_LPCMPHYSDIS
  ...
  if ((rev.major == 1)
    ...
    dcdcTiming = IEC_P_INST(EMU)->DCDCTIMING;
    ...
    IEC_P_INST(EMU)->DCDCTIMING = dcdcTiming;
...
static void currentLimitersUpdate(void)
{
  ...
  /* Get enabled PFETs. */
  pFetCnt = (IEC_P_INST(EMU)->DCDCMISCCTRL & _EMU_DCDCMISCCTRL_PFETCNT_MASK)
  ...
  IEC_P_INST(EMU)->DCDCMISCCTRL = (IEC_P_INST(EMU)->DCDCMISCCTRL & ~(_EMU_DCDCMISCCTRL_LNCLIMILIMSEL_MASK
  ...
  if (dcdcReverseCurrentControl >= 0) {
    ...
    IEC_P_INST(EMU)->DCDCZDETCTRL = (IEC_P_INST(EMU)->DCDCZDETCTRL & ~_EMU_DCDCZDETCTRL_ZDETILIMSEL_MASK)
                        | zdetLimSel;
  }
}
...
static void compCtrlSet(EMU_DcdcLnCompCtrl_TypeDef comp)
{
  switch (comp) {
    case emuDcdcLnCompCtrl_1u0F:
      IEC_P_INST(EMU)->DCDCLNCOMPCTRL = 0x57204077UL;
      break;

    case emuDcdcLnCompCtrl_4u7F:
      IEC_P_INST(EMU)->DCDCLNCOMPCTRL = 0xB7102137UL;
      break;
                                     ...
                                     dcdcTrimMode_TypeDef trimMode)
{
  ...
  if (trimMode == dcdcTrimMode_EM234H_LP) {
    ...
    IEC_P_INST(EMU)->DCDCLPCTRL = (IEC_P_INST(EMU)->DCDCLPCTRL & ~_GENERIC_DCDCLPCTRL_LPCMPHYSSELEM234H_MASK) | lpcmpHystSel;
  }
  ...
  if (trimMode == dcdcTrimMode_EM01_LP) {
    ...
    IEC_P_INST(EMU)->DCDCLPEM01CFG = (IEC_P_INST(EMU)->DCDCLPEM01CFG & ~_EMU_DCDCLPEM01CFG_LPCMPHYSSELEM01_MASK) | lpcmpHystSel;
  }
#endif
...
void EMU_DCDCModeSet(EMU_DcdcMode_TypeDef dcdcMode)
{
  ...
  dcdcLocked = (IEC_P_INST(EMU)->PWRLOCK == EMU_PWRLOCK_LOCKKEY_LOCKED);
  EMU_PowerUnlock();

  /* Wait for any previous write sync to complete and read DCDC mode. */
  while ((IEC_P_INST(EMU)->DCDCSYNC & EMU_DCDCSYNC_DCDCCTRLBUSY) != 0U) {
  }
  currentDcdcMode = (IEC_P_INST(EMU)->DCDCCTRL & _EMU_DCDCCTRL_DCDCMODE_MASK);
  ...
  if (currentDcdcMode != EMU_DCDCCTRL_DCDCMODE_BYPASS) {
    BUS_RegBitWrite(&IEC_P_INST(EMU)->DCDCCLIMCTRL, _EMU_DCDCCLIMCTRL_BYPLIMEN_SHIFT, 1);
  }
  ...
  if ((EMU_DcdcMode_TypeDef)currentDcdcMode == dcdcMode) {
    ...
    if (dcdcMode == emuDcdcMode_Bypass) {
      BUS_RegBitWrite(&IEC_P_INST(EMU)->DCDCCLIMCTRL, _EMU_DCDCCLIMCTRL_BYPLIMEN_SHIFT, 0);
    }
    return;
  }
  ...
  if (((currentDcdcMode == EMU_DCDCCTRL_DCDCMODE_OFF) || (currentDcdcMode == EMU_DCDCCTRL_DCDCMODE_BYPASS))
    ...
    IEC_P_INST(EMU)->DCDCCTRL = (IEC_P_INST(EMU)->DCDCCTRL & ~_EMU_DCDCCTRL_DCDCMODE_MASK) | EMU_DCDCCTRL_DCDCMODE_LOWNOISE;
  ...
  while ((IEC_P_INST(EMU)->DCDCSYNC & EMU_DCDCSYNC_DCDCCTRLBUSY) != 0UL) {
  }
  IEC_P_INST(EMU)->DCDCCTRL = (IEC_P_INST(EMU)->DCDCCTRL & ~_EMU_DCDCCTRL_DCDCMODE_MASK)
                  | (uint32_t)dcdcMode;
  ...
  BUS_RegBitWrite(&IEC_P_INST(EMU)->DCDCCLIMCTRL,
...
void EMU_DCDCModeEM23Set(EMU_DcdcModeEM23_TypeDef dcdcModeEM23)
{
  ...
  IEC_P_INST(EMU)->DCDCCTRL = (IEC_P_INST(EMU)->DCDCCTRL & ~_EMU_DCDCCTRL_DCDCMODEEM23_MASK)
                               ...
                               bool rcoDefaultSet)
{
  EMU_DcdcMode_TypeDef currentDcdcMode
    = (EMU_DcdcMode_TypeDef)((uint32_t)
                             (IEC_P_INST(EMU)->DCDCCTRL & _EMU_DCDCCTRL_DCDCMODE_MASK));
  EMU_DcdcLnRcoBand_TypeDef rcoBand
    = (EMU_DcdcLnRcoBand_TypeDef)((uint32_t)
                                  ((IEC_P_INST(EMU)->DCDCLNFREQCTRL
  ...
  if (conductionMode == emuDcdcConductionMode_DiscontinuousLN) {
    IEC_P_INST(EMU)->DCDCMISCCTRL &= ~EMU_DCDCMISCCTRL_LNFORCECCM;
    if (rcoDefaultSet) {
      EMU_DCDCLnRcoBandSet(emuDcdcLnRcoBand_3MHz);
    } else {
...
bool EMU_DCDCInit(const EMU_DCDCInit_TypeDef *dcdcInit)
{
  ...
  /* Set the external power configuration. This enables writing to the other
     DCDC registers. */
  IEC_P_INST(EMU)->PWRCFG = EMU_PWRCFG_PWRCFG_DCDCTODVDD;

  /* IEC_P_INST(EMU)->PWRCFG is write-once and POR reset only. Check that
     the desired power configuration was set. */
  if ((IEC_P_INST(EMU)->PWRCFG & _EMU_PWRCFG_PWRCFG_MASK) != EMU_PWRCFG_PWRCFG_DCDCTODVDD) {
  ...
  IEC_P_INST(EMU)->DCDCMISCCTRL = (IEC_P_INST(EMU)->DCDCMISCCTRL & ~(_GENERIC_DCDCMISCCTRL_LPCMPBIASEM234H_MASK
  ...
  IEC_P_INST(EMU)->DCDCLPEM01CFG = (IEC_P_INST(EMU)->DCDCLPEM01CFG & ~_EMU_DCDCLPEM01CFG_LPCMPBIASEM01_MASK)
  ...
  BUS_RegBitWrite(&IEC_P_INST(EMU)->PWRCTRL,
  ...
  IEC_P_INST(EMU)->PWRCTRL |= EMU_PWRCTRL_REGPWRSEL_DVDD;
#endif
...
#define DCDC_TRIM_MODES ((uint8_t)dcdcTrimMode_LN + 1)
  ...
  lpcmpBias[dcdcTrimMode_EM234H_LP] = (uint8_t)
                                      ((IEC_P_INST(EMU)->DCDCMISCCTRL & _GENERIC_DCDCMISCCTRL_LPCMPBIASEM234H_MASK)
  ...
  lpcmpBias[dcdcTrimMode_EM01_LP] = (uint8_t)
                                    ((IEC_P_INST(EMU)->DCDCLPEM01CFG & _EMU_DCDCLPEM01CFG_LPCMPBIASEM01_MASK)
  ...
  if (setLnVoltage) {
    IEC_P_INST(EMU)->DCDCLNVCTRL = (IEC_P_INST(EMU)->DCDCLNVCTRL & ~(_EMU_DCDCLNVCTRL_LNVREF_MASK | _EMU_DCDCLNVCTRL_LNATT_MASK))
  ...
  if (setLpVoltage) {
    ...
    IEC_P_INST(EMU)->DCDCLPVCTRL = (IEC_P_INST(EMU)->DCDCLPVCTRL & ~(_EMU_DCDCLPVCTRL_LPVREF_MASK | _EMU_DCDCLPVCTRL_LPATT_MASK))
...
void EMU_DCDCOptimizeSlice(uint32_t em0LoadCurrentmA)
{
  ...
  uint32_t rcoBand = (IEC_P_INST(EMU)->DCDCLNFREQCTRL & _EMU_DCDCLNFREQCTRL_RCOBAND_MASK)
                     >> _EMU_DCDCLNFREQCTRL_RCOBAND_SHIFT;

  /* Set the recommended slice count. */
  if (((IEC_P_INST(EMU)->DCDCMISCCTRL & _EMU_DCDCMISCCTRL_LNFORCECCM_MASK) != 0U)
  ...
  } else if (((IEC_P_INST(EMU)->DCDCMISCCTRL & _EMU_DCDCMISCCTRL_LNFORCECCM_MASK) == 0U)
  ...
  IEC_P_INST(EMU)->DCDCMISCCTRL = (IEC_P_INST(EMU)->DCDCMISCCTRL & ~(_EMU_DCDCMISCCTRL_PFETCNT_MASK
...
{
  uint32_t forcedCcm;
  forcedCcm = BUS_RegBitRead(&IEC_P_INST(EMU)->DCDCMISCCTRL, _EMU_DCDCMISCCTRL_LNFORCECCM_SHIFT);

  /* DCM mode supports up to 4 MHz LN RCO. */
  EFM_ASSERT(((forcedCcm == 0U) && band <= emuDcdcLnRcoBand_4MHz)
             || (forcedCcm != 0U));

  IEC_P_INST(EMU)->DCDCLNFREQCTRL = (IEC_P_INST(EMU)->DCDCLNFREQCTRL & ~_EMU_DCDCLNFREQCTRL_RCOBAND_MASK)
...
bool EMU_DCDCPowerOff(void)
{
...
#if defined(_EMU_PWRCFG_MASK)
  /* Set DCDCTODVDD only to enable write access to IEC_P_INST(EMU)->DCDCCTRL. */
  IEC_P_INST(EMU)->PWRCFG = EMU_PWRCFG_PWRCFG_DCDCTODVDD;
#endif

  /* Select DVDD as input to the digital regulator. */
#if defined(EMU_PWRCTRL_IMMEDIATEPWRSWITCH)
  IEC_P_INST(EMU)->PWRCTRL |= EMU_PWRCTRL_REGPWRSEL_DVDD | EMU_PWRCTRL_IMMEDIATEPWRSWITCH;
  ...
  IEC_P_INST(EMU)->DCDCCTRL = EMU_DCDCCTRL_DCDCMODE_OFF;

  dcdcModeSet = (IEC_P_INST(EMU)->DCDCCTRL == EMU_DCDCCTRL_DCDCMODE_OFF);
...
bool EMU_DCDCInit(const EMU_DCDCInit_TypeDef *dcdcInit)
{
  ...
  IEC_P_INST(EMU)->VREGVDDCMPCTRL = ((uint32_t)dcdcInit->cmpThreshold
...
void EMU_VmonInit(const EMU_VmonInit_TypeDef *vmonInit)
{
  ...
  switch (vmonInit->channel) {
    case emuVmonChannel_AVDD:
      IEC_P_INST(EMU)->VMONAVDDCTRL = (thresholdCoarse << _EMU_VMONAVDDCTRL_RISETHRESCOARSE_SHIFT)
      ...
      break;
    case emuVmonChannel_ALTAVDD:
      IEC_P_INST(EMU)->VMONALTAVDDCTRL = (thresholdCoarse << _EMU_VMONALTAVDDCTRL_THRESCOARSE_SHIFT)
      ...
      break;
    case emuVmonChannel_DVDD:
      IEC_P_INST(EMU)->VMONDVDDCTRL = (thresholdCoarse << _EMU_VMONDVDDCTRL_THRESCOARSE_SHIFT)
      ...
      break;
    case emuVmonChannel_IOVDD0:
      IEC_P_INST(EMU)->VMONIO0CTRL = (thresholdCoarse << _EMU_VMONIO0CTRL_THRESCOARSE_SHIFT)
...
#if defined(_EMU_VMONIO1CTRL_EN_MASK)
    case emuVmonChannel_IOVDD1:
      IEC_P_INST(EMU)->VMONIO1CTRL = (thresholdCoarse << _EMU_VMONIO1CTRL_THRESCOARSE_SHIFT)
...
#if defined(_EMU_VMONBUVDDCTRL_EN_MASK)
    case emuVmonChannel_BUVDD:
      IEC_P_INST(EMU)->VMONBUVDDCTRL = (thresholdCoarse << _EMU_VMONBUVDDCTRL_THRESCOARSE_SHIFT)
...
void EMU_VmonHystInit(const EMU_VmonHystInit_TypeDef *vmonInit)
{
  ...
  switch (vmonInit->channel) {
    case emuVmonChannel_AVDD:
      IEC_P_INST(EMU)->VMONAVDDCTRL = ((riseThreshold / 10U) << _EMU_VMONAVDDCTRL_RISETHRESCOARSE_SHIFT)
...
void EMU_VmonEnable(EMU_VmonChannel_TypeDef channel, bool enable)
{
  ...
  switch (channel) {
    case emuVmonChannel_AVDD:
      reg = &(IEC_P_INST(EMU)->VMONAVDDCTRL);
      bit = _EMU_VMONAVDDCTRL_EN_SHIFT;
      break;
    case emuVmonChannel_ALTAVDD:
      reg = &(IEC_P_INST(EMU)->VMONALTAVDDCTRL);
      bit = _EMU_VMONALTAVDDCTRL_EN_SHIFT;
      break;
    case emuVmonChannel_DVDD:
      reg = &(IEC_P_INST(EMU)->VMONDVDDCTRL);
      bit = _EMU_VMONDVDDCTRL_EN_SHIFT;
      break;
    case emuVmonChannel_IOVDD0:
      reg = &(IEC_P_INST(EMU)->VMONIO0CTRL);
      bit = _EMU_VMONIO0CTRL_EN_SHIFT;
      break;
#if defined(_EMU_VMONIO1CTRL_EN_MASK)
    case emuVmonChannel_IOVDD1:
      reg = &(IEC_P_INST(EMU)->VMONIO1CTRL);
      bit = _EMU_VMONIO1CTRL_EN_SHIFT;
      break;
#endif
#if defined(_EMU_VMONBUVDDCTRL_EN_MASK)
    case emuVmonChannel_BUVDD:
      reg = &(IEC_P_INST(EMU)->VMONBUVDDCTRL);
...
bool EMU_VmonChannelStatusGet(EMU_VmonChannel_TypeDef channel)
{
  ...
  return BUS_RegBitRead(&IEC_P_INST(EMU)->STATUS, bit) != 0U;
}
...
float EMU_TemperatureGet(void)
{
  ...
  return ((float) ((IEC_P_INST(EMU)->TEMP & (_EMU_TEMP_TEMP_MASK | _EMU_TEMP_TEMPLSB_MASK) )
  ...
  do {
    val1 = (IEC_P_INST(EMU)->TEMP & _EMU_TEMP_TEMP_MASK)
           >> _EMU_TEMP_TEMP_SHIFT;
    val2 = (IEC_P_INST(EMU)->TEMP & _EMU_TEMP_TEMP_MASK)
           >> _EMU_TEMP_TEMP_SHIFT;
  } while (val1 != val2);
