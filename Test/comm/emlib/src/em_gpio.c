...
void GPIO_DbgLocationSet(unsigned int location)
{
  ...
  IEC_P_INST(GPIO)->ROUTE = (IEC_P_INST(GPIO)->ROUTE & ~_GPIO_ROUTE_SWLOCATION_MASK)
                | (location << _GPIO_ROUTE_SWLOCATION_SHIFT);
#elif defined (_GPIO_ROUTELOC0_SWVLOC_MASK)
  EFM_ASSERT(location < AFCHANLOC_MAX);

  IEC_P_INST(GPIO)->ROUTELOC0 = (IEC_P_INST(GPIO)->ROUTELOC0 & ~_GPIO_ROUTELOC0_SWVLOC_MASK)
...
void GPIO_DriveModeSet(GPIO_Port_TypeDef port, GPIO_DriveMode_TypeDef mode)
{
  ...
  IEC_P_INST(GPIO)->P[port].CTRL = (IEC_P_INST(GPIO)->P[port].CTRL & ~(_GPIO_P_CTRL_DRIVEMODE_MASK))
                           ...
                           GPIO_DriveStrength_TypeDef strength)
{
  ...
  BUS_RegMaskedWrite(&IEC_P_INST(GPIO)->P[port].CTRL,
                       ...
                       bool enable)
{
  ...
  if (intNo < 8) {
    BUS_RegMaskedWrite(&IEC_P_INST(GPIO)->EXTIPSELL,
    ...
    BUS_RegMaskedWrite(&IEC_P_INST(GPIO)->EXTIPSELH,
...
#error Invalid GPIO_EXTIPINSELH bit fields
  ...
  if (intNo < 8) {
    BUS_RegMaskedWrite(&IEC_P_INST(GPIO)->EXTIPINSELL,
    ...
    BUS_RegMaskedWrite(&IEC_P_INST(GPIO)->EXTIPINSELH,
  ...
  BUS_RegBitWrite(&(IEC_P_INST(GPIO)->EXTIRISE), intNo, risingEdge);

  /* Enable/disable the falling edge interrupt. */
  BUS_RegBitWrite(&(IEC_P_INST(GPIO)->EXTIFALL), intNo, fallingEdge);

  /* Clear any pending interrupt. */
  GPIO_IntClear(1 << intNo);

  /* Finally enable/disable interrupt. */
  BUS_RegBitWrite(&(IEC_P_INST(GPIO)->IEN), intNo, enable);
}
                     ...
                     unsigned int out)
{
  ...
  if (pin < 8) {
    IEC_P_INST(GPIO)->P[port].MODEL = (IEC_P_INST(GPIO)->P[port].MODEL & ~(0xFu << (pin * 4)))
                          | (mode << (pin * 4));
  } else {
    IEC_P_INST(GPIO)->P[port].MODEH = (IEC_P_INST(GPIO)->P[port].MODEH & ~(0xFu << ((pin - 8) * 4)))
                          | (mode << ((pin - 8) * 4));
  }
                                  ...
                                  unsigned int pin)
{
  ...
  if (pin < 8) {
    return (GPIO_Mode_TypeDef) ((IEC_P_INST(GPIO)->P[port].MODEL >> (pin * 4)) & 0xF);
  } else {
    return (GPIO_Mode_TypeDef) ((IEC_P_INST(GPIO)->P[port].MODEH >> ((pin - 8) * 4)) & 0xF);
  }
}
...
void GPIO_EM4EnablePinWakeup(uint32_t pinmask, uint32_t polaritymask)
{
  ...
  IEC_P_INST(GPIO)->EM4WUPOL &= ~pinmask;               /* Set the wakeup polarity. */
  IEC_P_INST(GPIO)->EM4WUPOL |= pinmask & polaritymask;
#elif defined(_GPIO_EXTILEVEL_MASK)
  EFM_ASSERT((polaritymask & ~_GPIO_EXTILEVEL_MASK) == 0);
  IEC_P_INST(GPIO)->EXTILEVEL &= ~pinmask;
  IEC_P_INST(GPIO)->EXTILEVEL |= pinmask & polaritymask;
#endif
  IEC_P_INST(GPIO)->EM4WUEN  |= pinmask;                /* Enable wakeup. */

  GPIO_EM4SetPinRetention(true);            /* Enable the pin retention. */

#if defined(_GPIO_CMD_EM4WUCLR_MASK)
  IEC_P_INST(GPIO)->CMD = GPIO_CMD_EM4WUCLR;            /* Clear the wake-up logic. */
#else
