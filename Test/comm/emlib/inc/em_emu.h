...
typedef struct {
  ...
  /* Initialization parameters for platforms with IEC_P_INST(EMU)->EM4CONF register (Series 0). */
  ...
  /* Initialization parameters for platforms with IEC_P_INST(EMU)->EM4CTRL register (Series 1). */
...
__STATIC_INLINE void EMU_VScaleWait(void)
{
  ...
  while (BUS_RegBitRead(&IEC_P_INST(EMU)->STATUS, _EMU_STATUS_VSCALEBUSY_SHIFT) != 0U) {
  }
}
#endif
...
__STATIC_INLINE EMU_VScaleEM01_TypeDef EMU_VScaleGet(void)
{
  ...
  return (EMU_VScaleEM01_TypeDef)((uint32_t)
                                  ((IEC_P_INST(EMU)->STATUS & _EMU_STATUS_VSCALE_MASK)
...
{
  return BUS_RegBitRead(&IEC_P_INST(EMU)->STATUS, _EMU_STATUS_VMONRDY_SHIFT) != 0U;
}
...
__STATIC_INLINE void EMU_IntClear(uint32_t flags)
{
  ...
  IEC_P_INST(EMU)->IF_CLR = flags;
#else
  IEC_P_INST(EMU)->IFC = flags;
#endif
}
...
{
#if defined(EMU_HAS_SET_CLEAR)
  IEC_P_INST(EMU)->IEN_CLR = flags;
#else
  IEC_P_INST(EMU)->IEN &= ~flags;
#endif
}
...
__STATIC_INLINE uint32_t EMU_IntGetEnabled(void)
{
  ...
  ien = IEC_P_INST(EMU)->IEN;
  return IEC_P_INST(EMU)->IF & ien;
}
...
{
#if defined(EMU_HAS_SET_CLEAR)
  IEC_P_INST(EMU)->IF_SET = flags;
#else
  IEC_P_INST(EMU)->IFS = flags;
#endif
}
#endif /* _EMU_IF_MASK */
...
{
  BUS_RegBitWrite(&(IEC_P_INST(EMU)->EM4CONF), _EMU_EM4CONF_LOCKCONF_SHIFT, enable);
}
#endif
...
{
  while (!(IEC_P_INST(EMU)->STATUS & EMU_STATUS_BURDY))
    ;
}
#endif
...
{
  BUS_RegBitWrite(&(IEC_P_INST(EMU)->ROUTE), _EMU_ROUTE_BUVINPEN_SHIFT, enable);
}
#endif
...
{
  IEC_P_INST(EMU)->LOCK = EMU_LOCK_LOCKKEY_UNLOCK;
}
...
{
  IEC_P_INST(EMU)->PWRLOCK = EMU_PWRLOCK_LOCKKEY_LOCK;
}
...
{
  IEC_P_INST(EMU)->PWRLOCK = EMU_PWRLOCK_LOCKKEY_UNLOCK;
}
#endif
...
{
  BUS_RegBitWrite(&IEC_P_INST(EMU)->CTRL, _EMU_CTRL_EM2BLOCK_SHIFT, 1U);
}
...
{
  BUS_RegBitWrite(&IEC_P_INST(EMU)->CTRL, _EMU_CTRL_EM2BLOCK_SHIFT, 0U);
}
#endif
...
{
  IEC_P_INST(EMU)->CMD = EMU_CMD_EM4UNLATCH;
}
#endif
...
__STATIC_INLINE bool EMU_TemperatureReady(void)
{
  ...
  return IEC_P_INST(EMU)->STATUS & EMU_STATUS_FIRSTTEMPDONE;
#else
  return !((IEC_P_INST(EMU)->TEMP & _EMU_TEMP_TEMP_MASK) == 0u);
#endif
}
...
{
  return ((float) ((IEC_P_INST(EMU)->TEMP & _EMU_TEMP_TEMPAVG_MASK)
...
{
  BUS_RegBitWrite(&IEC_P_INST(EMU)->CTRL, _EMU_CTRL_TEMPAVGNUM_SHIFT, numSamples);
  IEC_P_INST(EMU)->CMD = 1u << _EMU_CMD_TEMPAVGREQ_SHIFT;
}
