...
{
  BUS_RegBitWrite(&IEC_P_INST(CMU)->CALCTRL, _CMU_CALCTRL_CONT_SHIFT, (uint32_t)enable);
}
...
{
  IEC_P_INST(CMU)->CALCMD = CMU_CALCMD_CALSTART;
}
...
{
  IEC_P_INST(CMU)->CALCMD = CMU_CALCMD_CALSTOP;
}
...
__STATIC_INLINE uint32_t CMU_IntGetEnabled(void)
{
  ...
  ien = IEC_P_INST(CMU)->IEN;
  return IEC_P_INST(CMU)->IF & ien;
}
...
{
  IEC_P_INST(CMU)->LOCK = ~CMU_LOCK_LOCKKEY_UNLOCK;
}
...
{
  IEC_P_INST(CMU)->WDOGLOCK = ~CMU_WDOGLOCK_LOCKKEY_UNLOCK;
}
...
__STATIC_INLINE void CMU_CalibrateStop(void)
{
  IEC_P_INST(CMU)->CMD = CMU_CMD_CALSTOP;
}
#endif
...
__STATIC_INLINE void CMU_Lock(void)
{
  IEC_P_INST(CMU)->LOCK = CMU_LOCK_LOCKKEY_LOCK;
}
