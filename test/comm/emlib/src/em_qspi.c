...
void QSPI_ReadConfig(QSPI_TypeDef *qspi, const QSPI_ReadConfig_TypeDef *config)
{
  ...
  qspi->DEVINSTRRDCONFIG =
      ...
      | (config->addrTransfer
         << _QSPI_DEVINSTRRDCONFIG_ADDRXFERTYPESTDMODE_SHIFT)
      | (config->dataTransfer
         << _QSPI_DEVINSTRRDCONFIG_DATAXFERTYPEEXTMODE_SHIFT)
 ...
 ******************************************************************************/
void QSPI_WriteConfig(QSPI_TypeDef *qspi,
                      const QSPI_WriteConfig_TypeDef *config)
{
  EFM_ASSERT(config->dummyCycles < 31);

  QSPI_WaitForIdle(qspi);
  qspi->DEVINSTRWRCONFIG =
      (config->opCode << _QSPI_DEVINSTRWRCONFIG_WROPCODE_SHIFT)
      | (config->dummyCycles << _QSPI_DEVINSTRWRCONFIG_DUMMYWRCLKCYCLES_SHIFT)
      | (config->addrTransfer
         << _QSPI_DEVINSTRWRCONFIG_ADDRXFERTYPESTDMODE_SHIFT)
      | (config->dataTransfer
         << _QSPI_DEVINSTRWRCONFIG_DATAXFERTYPEEXTMODE_SHIFT)
...
void QSPI_ExecStigCmd(QSPI_TypeDef *qspi, const QSPI_StigCmd_TypeDef *stigCmd)
{
  ...
  qspi->FLASHCMDCTRL =
      (stigCmd->cmdOpcode << _QSPI_FLASHCMDCTRL_CMDOPCODE_SHIFT)
  ...
  if (stigCmd->addrSize) {
    qspi->FLASHCMDCTRL |=
        QSPI_FLASHCMDCTRL_ENBCOMDADDR
        | ((stigCmd->addrSize - 1) << _QSPI_FLASHCMDCTRL_NUMADDRBYTES_SHIFT);
