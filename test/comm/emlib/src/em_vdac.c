...
static void VDAC_DisableModule(VDAC_TypeDef *vdac)
{
  ...
  if (vdac->EN & VDAC_EN_EN) {
    ...
    while (vdac->STATUS
           & (VDAC_STATUS_SYNCBUSY | VDAC_STATUS_CH0FIFOFLBUSY
              | VDAC_STATUS_CH1FIFOFLBUSY)) {}
#endif
...
void VDAC_Init(VDAC_TypeDef *vdac, const VDAC_Init_TypeDef *init)
{
  ...
  config =
      ((((uint32_t) init->warmupTime << _VDAC_CFG_WARMUPTIME_SHIFT)
        & _VDAC_CFG_WARMUPTIME_MASK)
                      ...
                      unsigned int ch)
{
  ...
  channelConfig =
      ((uint32_t) init->prsSel << _VDAC_CH0CTRL_PRSSEL_SHIFT)
  ...
  channelConfig =
      ((uint32_t) init->warmupKeepOn << _VDAC_CH0CFG_KEEPWARM_SHIFT)
      | ((uint32_t) init->highCapLoadEnable << _VDAC_CH0CFG_HIGHCAPLOADEN_SHIFT)
      | (((uint32_t) init->fifoLowDataThreshold << _VDAC_CH0CFG_FIFODVL_SHIFT)
         & _VDAC_CH0CFG_FIFODVL_MASK)
  ...
  if (ch == 0) {
    ...
    vdac->OUTTIMERCFG =
        ...
        | (((uint32_t) init->holdOutTime
            << _VDAC_OUTTIMERCFG_CH0OUTHOLDTIME_SHIFT)
           & _VDAC_OUTTIMERCFG_CH0OUTHOLDTIME_MASK);

    vdac->EN_SET = _VDAC_EN_EN_MASK;

    vdac->OUTCTRL =
        ((uint32_t) (vdac->OUTCTRL
                     & ~(_VDAC_OUTCTRL_ABUSPINSELCH0_MASK
                         | _VDAC_OUTCTRL_ABUSPORTSELCH0_MASK
                         | _VDAC_OUTCTRL_SHORTCH0_MASK
                         | _VDAC_OUTCTRL_AUXOUTENCH0_MASK
                         | _VDAC_OUTCTRL_MAINOUTENCH0_MASK)))
    ...
    vdac->OUTTIMERCFG =
        (vdac->OUTTIMERCFG & ~(_VDAC_OUTTIMERCFG_CH1OUTHOLDTIME_MASK))
        | (((uint32_t) init->holdOutTime
            << _VDAC_OUTTIMERCFG_CH1OUTHOLDTIME_SHIFT)
           & _VDAC_OUTTIMERCFG_CH1OUTHOLDTIME_MASK);

    vdac->EN_SET = _VDAC_EN_EN_MASK;

    vdac->OUTCTRL =
        ((uint32_t) (vdac->OUTCTRL
                     & ~(_VDAC_OUTCTRL_ABUSPINSELCH1_MASK
                         | _VDAC_OUTCTRL_ABUSPORTSELCH1_MASK
                         | _VDAC_OUTCTRL_SHORTCH1_MASK
                         | _VDAC_OUTCTRL_AUXOUTENCH1_MASK
                         | _VDAC_OUTCTRL_MAINOUTENCH1_MASK)))
...
uint32_t VDAC_PrescaleCalc(uint32_t vdacFreq, bool syncMode, uint32_t hfperFreq)
{
  ...
  /* frequency value. */
  for (ret = 0; ret <= (_VDAC_CTRL_PRESC_MASK >> _VDAC_CTRL_PRESC_SHIFT);
       ret++) {
...
void VDAC_Reset(VDAC_TypeDef *vdac)
{
  ...
  if (vdac->EN & VDAC_EN_EN) {
    ...
    while (vdac->STATUS
           & (VDAC_STATUS_CH0FIFOFLBUSY | VDAC_STATUS_CH1FIFOFLBUSY)) {}
