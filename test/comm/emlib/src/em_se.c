...
#define ROOT_MAILBOX_OUTPUT_BASE                                               \
  (RDMEM_FRCRAM_S_MEM_END + 1 - ROOT_MAILBOX_SIZE)
...
void SE_executeCommand(SE_Command_t *command)
{
  ...
  // (must be stored in a RAM area which is not used by the root code)
  root_InputMailbox_t *rootInMb =
      (root_InputMailbox_t *) ROOT_MAILBOX_INPUT_BASE;
  uint32_t *mbData;
  ...
  inDataLen = 0;
  for (inDataDesc = command->data_in; inDataDesc;
       inDataDesc = (SE_DataTransfer_t *) inDataDesc->next) {
    inData = (uint32_t *) inDataDesc->data;
    for (i = 0; i < (inDataDesc->length & SE_DATATRANSFER_LENGTH_MASK)
                        / sizeof(uint32_t);
         i++) {
...
SE_Response_t SE_ackCommand(SE_Command_t *command)
{
  ...
  while (outDataDesc && (outDataCnt < outDataLen)) {
    ...
    if (i == outDescLen) {
      outDataDesc = (SE_DataTransfer_t *) ((uint32_t) outDataDesc->next
                            ...
                            bool signature)
{
  ...
  EFM_ASSERT((key_type == SE_KEY_TYPE_BOOT) || (key_type == SE_KEY_TYPE_AUTH));
...
SE_Response_t SE_initOTP(SE_OTPInit_t *otp_init)
{
  ...
  volatile uint32_t parameters[2] = {parity, sizeof(mcuSettingsFlags)};
  ...
  static struct ReservedSettings {
    ...
    uint8_t reserved2[2];
    uint8_t reserved3[2];
  } reservedSettings = {{0x00}, {0xFF}, {0x00}};
  ...
  SE_DataTransfer_t mcuSettingsFlagsData =
      SE_DATATRANSFER_DEFAULT((volatile void *) &mcuSettingsFlags,
                              sizeof(mcuSettingsFlags));
  SE_addDataInput(&command, &mcuSettingsFlagsData);

  SE_DataTransfer_t reservedSettingsData =
 ...
 *    word size and contain a number of bytes that is divisible by four.
 *
 * @note
...
SE_Response_t SE_debugLockStatus(SE_DebugStatus_t *status)
{
  ...
  SE_DataTransfer_t statusData =
      SE_DATATRANSFER_DEFAULT((void *) &status_word, 4);
