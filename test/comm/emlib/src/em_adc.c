 ...
 ******************************************************************************/
static void
    ADC_LoadDevinfoCal(ADC_TypeDef *adc, ADC_Ref_TypeDef ref, bool setScanCal)
{
  uint32_t calReg;
  uint32_t newCal;
  uint32_t mask;
...
void ADC_Init(ADC_TypeDef *adc, const ADC_Init_TypeDef *init)
{
  ...
  tmp = ((uint32_t) (init->ovsRateSel) << _ADC_CTRL_OVSRSEL_SHIFT)
        ...
        | (((uint32_t) (presc) << _ADC_CTRL_PRESC_SHIFT) & _ADC_CTRL_PRESC_MASK)
                                     ...
                                     ADC_PosSel_TypeDef singleEndedSel)
{
  ...
  currentSel =
      (scanInit->scanInputConfig.scanInputSel >> ((unsigned) inputGroup * 8U))
      & 0xFFU;
  ...
  if (currentSel == ADC_SCANINPUTSEL_GROUP_NONE) {
    ...
    scanInit->scanInputConfig.scanInputSel |= newSel
                                              << ((unsigned) inputGroup * 8U);
...
void ADC_InitScan(ADC_TypeDef *adc, const ADC_InitScan_TypeDef *init)
{
  ...
  tmp = adc->SCANCTRLX
        & ~(_ADC_SCANCTRLX_VREFSEL_MASK | _ADC_SCANCTRLX_PRSSEL_MASK
...
void ADC_InitSingle(ADC_TypeDef *adc, const ADC_InitSingle_TypeDef *init)
{
  ...
  if ((init->posSel == adcPosSelTEMP) && (init->reference == adcRef1V25)
  ...
  tmp = adc->SINGLECTRLX
        & ~(_ADC_SINGLECTRLX_VREFSEL_MASK | _ADC_SINGLECTRLX_PRSSEL_MASK
...
#if defined(_ADC_BIASPROG_GPBIASACC_MASK)                                      \
...
uint32_t ADC_DataIdScanGet(ADC_TypeDef *adc, uint32_t *scanId)
{
  ...
  *scanId  = (scanData & _ADC_SCANDATAX_SCANINPUTID_MASK)
            >> _ADC_SCANDATAX_SCANINPUTID_SHIFT;
