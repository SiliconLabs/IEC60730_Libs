...
void PRS_GpioOutputLocation(unsigned int ch, unsigned int location)
{
  ...
  PRS->ROUTE |= (location << _PRS_ROUTE_LOCATION_SHIFT) | (1 << ch);
#else
...
void PRS_ConnectSignal(unsigned int ch, PRS_ChType_t type, PRS_Signal_t signal)
{
  ...
  uint32_t signalField =
      (uint32_t) signal
      & (_PRS_CH_CTRL_SOURCESEL_MASK | _PRS_CH_CTRL_SIGSEL_MASK);
  if (type == prsTypeAsync) {
#if defined(PRS_ASYNC_SUPPORTED)
    EFM_ASSERT(ch < PRS_ASYNC_CHAN_COUNT);
    PRS->CH[ch].CTRL = PRS_CH_CTRL_EDSEL_OFF | PRS_CH_CTRL_ASYNC | signalField;
#endif
  } else {
    EFM_ASSERT(ch < PRS_SYNC_CHAN_COUNT);
    PRS->CH[ch].CTRL = PRS_CH_CTRL_EDSEL_OFF | signalField;
  }
#endif
}
...
void PRS_Combine(unsigned int chA, unsigned int chB, PRS_Logic_t logic)
{
  ...
  PRS->ASYNC_CH[chA].CTRL =
      (PRS->ASYNC_CH[chA].CTRL & ~_PRS_ASYNC_CH_CTRL_FNSEL_MASK)
  ...
  PRS->ASYNC_CH[chB].CTRL =
      (PRS->ASYNC_CH[chB].CTRL & ~_PRS_ASYNC_CH_CTRL_FNSEL_MASK)
