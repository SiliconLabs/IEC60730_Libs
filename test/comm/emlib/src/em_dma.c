                      ...
                      unsigned int nMinus1)
{
  ...
  EFM_ASSERT(nMinus1
             <= (_DMA_CTRL_N_MINUS_1_MASK >> _DMA_CTRL_N_MINUS_1_SHIFT));
...
{
  EFM_ASSERT(channel < DMA_CHAN_COUNT);
  EFM_ASSERT(primNMinus1
             <= (_DMA_CTRL_N_MINUS_1_MASK >> _DMA_CTRL_N_MINUS_1_SHIFT));
  EFM_ASSERT(altNMinus1
             <= (_DMA_CTRL_N_MINUS_1_MASK >> _DMA_CTRL_N_MINUS_1_SHIFT));
                               ...
                               unsigned int count)
{
  ...
  descr->DSTEND =
      (uint32_t *) ((DMA_DESCRIPTOR_TypeDef *) (DMA->ALTCTRLBASE) + channel + 1)
      - 1;
...
#error "Unsupported DMA channel count (em_dma.c)."
  ...
  DMA->CONFIG =
      ((uint32_t) (init->hprot) << _DMA_CONFIG_CHPROT_SHIFT) | DMA_CONFIG_EN;
}
