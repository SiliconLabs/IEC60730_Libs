...
{
  CORE_CRITICAL_SECTION(*nvicState =
                            *(CORE_nvicMask_t *) ((uint32_t) &NVIC->ICER[0]);
                        *(CORE_nvicMask_t *) ((uint32_t) &NVIC->ICER[0]) =
                            *disable;)
}
...
{
  CORE_CRITICAL_SECTION(*(CORE_nvicMask_t *) ((uint32_t) &NVIC->ICER[0]) =
                            *disable;)
}
...
{
  CORE_CRITICAL_SECTION(*(CORE_nvicMask_t *) ((uint32_t) &NVIC->ISER[0]) =
                            *enable;)
}
...
void CORE_YieldNvicMask(const CORE_nvicMask_t *enable)
{
  ...
  CORE_CRITICAL_SECTION(nvicMask =
                            *(CORE_nvicMask_t *) ((uint32_t) &NVIC->ISER[0]);)
                              ...
                              bool overwriteActive)
{
  ...
  EFM_ASSERT(
      ((uint32_t) targetTable & ~(SCB_VTOR_TBLOFF_Msk | SCB_VTOR_TBLBASE_Msk))
      == 0U);
#else
