...
void CSEN_Init(CSEN_TypeDef *csen, const CSEN_Init_TypeDef *init)
{
  ...
  csen->SCANINPUTSEL0 =
      (init->input0To7 << _CSEN_SCANINPUTSEL0_INPUT0TO7SEL_SHIFT)
  ...
  csen->SCANINPUTSEL1 =
      (init->input32To39 << _CSEN_SCANINPUTSEL1_INPUT32TO39SEL_SHIFT)
...
void CSEN_InitMode(CSEN_TypeDef *csen, const CSEN_InitMode_TypeDef *init)
{
   ...
   * to be shifted. */
  tmp = ((uint32_t) init->sampleMode | (uint32_t) init->convSel
  ...
  tmp |= csen->CTRL
         & (_CSEN_CTRL_CPACCURACY_MASK | _CSEN_CTRL_LOCALSENS_MASK
