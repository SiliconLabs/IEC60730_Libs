...
#elif (ACMP_COUNT == 3)
#define ACMP_REF_VALID(ref)                                                    \
  (((ref) == ACMP0) || ((ref) == ACMP1) || ((ref) == ACMP2))
#elif (ACMP_COUNT == 4)
#define ACMP_REF_VALID(ref)                                                    \
  (((ref) == ACMP0) || ((ref) == ACMP1) || ((ref) == ACMP2) || ((ref) == ACMP3))
#else
...
#define ACMP_DEVICE_ID(acmp) ((acmp) == ACMP0 ? 0 : (acmp) == ACMP1 ? 1 : 0)
                       ...
                       const ACMP_CapsenseInit_TypeDef *init)
{
  ...
  EFM_ASSERT(init->biasProg <= (_ACMP_CFG_BIAS_MASK >> _ACMP_CFG_BIAS_SHIFT));
  ...
  acmp->HYSTERESIS0 =
      (init->vddLevelHigh << _ACMP_HYSTERESIS0_DIVVA_SHIFT)
      | (init->hysteresisLevel_0 << _ACMP_HYSTERESIS0_HYST_SHIFT);
  acmp->HYSTERESIS1 =
      (init->vddLevelLow << _ACMP_HYSTERESIS1_DIVVA_SHIFT)
  ...
  acmp->INPUTSEL = (init->resistor << _ACMP_INPUTSEL_CSRESSEL_SHIFT)
                   | ACMP_INPUTSEL_CSRESEN | ACMP_INPUTSEL_VASEL_VDD
  ...
  /* Select capacitive sensing mode by selecting a resistor and enabling it. */
  acmp->INPUTSEL =
      (init->resistor << _ACMP_INPUTSEL_CSRESSEL_SHIFT) | ACMP_INPUTSEL_CSRESEN
...
void ACMP_CapsenseChannelSet(ACMP_TypeDef *acmp, ACMP_Channel_TypeDef channel)
{
  ...
  BUS_RegMaskedWrite(&acmp->INPUTCTRL,
                     _ACMP_INPUTCTRL_POSSEL_MASK,
                     channel << _ACMP_INPUTCTRL_POSSEL_SHIFT);
#else
  /* Set channel as a positive channel in ACMP. */
  BUS_RegMaskedWrite(&acmp->INPUTSEL,
...
{
  acmp->EXTIFCTRL =
      (aport << _ACMP_EXTIFCTRL_APORTSEL_SHIFT) | ACMP_EXTIFCTRL_EN;
                    ...
                    bool invert)
{
  ...
  GPIO->ACMPROUTE[acmpIndex].ACMPOUTROUTE =
      (port << _GPIO_ACMP_ACMPOUTROUTE_PORT_SHIFT)
      | (pin << _GPIO_ACMP_ACMPOUTROUTE_PIN_SHIFT);
  GPIO->ACMPROUTE[acmpIndex].ROUTEEN =
      enable ? GPIO_ACMP_ROUTEEN_ACMPOUTPEN : 0;
}
#else
/***************************************************************************/ /**
 * @brief
                     ...
                     ACMP_Channel_TypeDef posSel)
{
  ...
  acmp->INPUTCTRL =
      (acmp->INPUTCTRL
       & ~(_ACMP_INPUTCTRL_POSSEL_MASK | _ACMP_INPUTCTRL_NEGSEL_MASK))
