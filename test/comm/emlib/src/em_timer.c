 ...
 */
static void timerPrsConfig(TIMER_TypeDef *timer,
                           unsigned int cc,
                           unsigned int prsCh,
                           bool async)
{
  ...
  if (async) {
    base->TIMER_CONSUMER[i].CONSUMER_CH[cc] =
        prsCh << _PRS_CONSUMER_TIMER0_CC0_PRSSEL_SHIFT;
  } else {
    base->TIMER_CONSUMER[i].CONSUMER_CH[cc] =
        prsCh << _PRS_CONSUMER_TIMER0_CC0_SPRSSEL_SHIFT;
  }
}
#endif
...
void TIMER_InitDTI(TIMER_TypeDef *timer, const TIMER_InitDTI_TypeDef *init)
{
  ...
  if (init->enableFaultSourcePrsSel0) {
    timerPrsConfig(timer,
                   TIMER_INPUT_CHANNEL_DTIFS1,
                   init->faultSourcePrsSel0,
                   true);
  }
  if (init->enableFaultSourcePrsSel1) {
    timerPrsConfig(timer,
                   TIMER_INPUT_CHANNEL_DTIFS2,
