...
#else
#error "The actual number of CAN buses is not supported."
#endif
...
bool CAN_MessageLost(CAN_TypeDef *can, uint8_t interface, uint8_t msgNum)
{
  ...
  mir->CMDMASK = CAN_MIR_CMDMASK_WRRD_READ | CAN_MIR_CMDMASK_CONTROL
                  ...
                  uint16_t pinTxLoc)
{
  if (active) {
    ...
    can->ROUTE = CAN_ROUTE_TXPEN | (pinRxLoc << _CAN_ROUTE_RXLOC_SHIFT)
                      ...
                      uint16_t synchronisationJumpWidth)
{
  ...
  can->BITTIMING =
      (brpLow << _CAN_BITTIMING_BRP_SHIFT)
...
void CAN_SetMode(CAN_TypeDef *can, CAN_Mode_TypeDef mode)
{
  switch (mode) {
    case canModeNormal:
      ...
      can->CTRL = _CAN_CTRL_EIE_MASK | _CAN_CTRL_SIE_MASK | _CAN_CTRL_IE_MASK;
      break;
                        ...
                        bool wait)
{
  ...
  /* Set which registers to read from RAM. */
  mir->CMDMASK = CAN_MIR_CMDMASK_WRRD_READ | CAN_MIR_CMDMASK_ARBACC
  ...
  if (message->extended) {
    ...
    mir->ARB = (mir->ARB & ~_CAN_MIR_ARB_ID_MASK)
               | (message->id << _CAN_MIR_ARB_ID_SHIFT) | CAN_MIR_ARB_MSGVAL
               | CAN_MIR_ARB_XTD_EXT;
  } else {
  ...
  if (message->extendedMask) {
    mir->MASK =
        (message->mask << _CAN_MIR_MASK_MASK_SHIFT) & _CAN_MIR_MASK_MASK_MASK;
  } else {
    mir->MASK =
        (message->mask << _CAN_MIR_MASK_STD_SHIFT) & _CAN_MIR_ARB_STD_ID_MASK;
  }
                                ...
                                bool wait)
{
  ...
  mir->CTRL = (endOfBuffer << _CAN_MIR_CTRL_EOB_SHIFT) | _CAN_MIR_CTRL_TXIE_MASK
                     ...
                     bool wait)
{
  ...
  if (((mir->CTRL & _CAN_MIR_CTRL_RMTEN_MASK) == 0)
    ...
    mir->CMDMASK |= CAN_MIR_CMDMASK_DATAA | CAN_MIR_CMDMASK_DATAB;
  ...
  mir->CTRL = (mir->CTRL & ~_CAN_MIR_CTRL_DLC_MASK) | message->dlc;
  ...
  if (message->extended) {
    ...
    mir->ARB = (mir->ARB & ~(_CAN_MIR_ARB_ID_MASK | _CAN_MIR_ARB_XTD_MASK))
               | CAN_MIR_ARB_MSGVAL | (message->id << _CAN_MIR_ARB_STD_ID_SHIFT)
               | CAN_MIR_ARB_XTD_STD;
  }
                     ...
                     CAN_MessageObject_TypeDef *message)
{
  ...
  /* Set which registers to read from RAM. */
  mir->CMDMASK = CAN_MIR_CMDMASK_WRRD_READ | CAN_MIR_CMDMASK_MASKACC
                 | CAN_MIR_CMDMASK_ARBACC | CAN_MIR_CMDMASK_CONTROL
                 | CAN_MIR_CMDMASK_CLRINTPND | CAN_MIR_CMDMASK_TXRQSTNEWDAT
                 | CAN_MIR_CMDMASK_DATAA | CAN_MIR_CMDMASK_DATAB;
  ...
  message->dlc =
      ((mir->CTRL & _CAN_MIR_CTRL_DLC_MASK) >> _CAN_MIR_CTRL_DLC_SHIFT);
  ...
  if (message->extended) {
    ...
    message->id =
        ((mir->ARB & _CAN_MIR_ARB_STD_ID_MASK) >> _CAN_MIR_ARB_STD_ID_SHIFT);
  }
...
void CAN_ResetMessages(CAN_TypeDef *can, uint8_t interface)
{
  ...
  /* Set which registers to read from RAM. */
  mir->CMDMASK = CAN_MIR_CMDMASK_WRRD | CAN_MIR_CMDMASK_MASKACC
                 | CAN_MIR_CMDMASK_ARBACC | CAN_MIR_CMDMASK_CONTROL
                 | CAN_MIR_CMDMASK_DATAA | CAN_MIR_CMDMASK_DATAB;
