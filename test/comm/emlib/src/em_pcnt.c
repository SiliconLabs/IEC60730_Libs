...
#elif (PCNT_COUNT == 3)
#define PCNT_REF_VALID(ref)                                                    \
  (((ref) == PCNT0) || ((ref) == PCNT1) || ((ref) == PCNT2))
#else
 ...
 *   done as part of initialization or if requiring to disable/re-enable pulse
 *   counter.
 *
 * @note
 ...
 *
 * @details
 *   The pulse counter is disabled while changing these values and re-enabled
...
void PCNT_Init(PCNT_TypeDef *pcnt, const PCNT_Init_TypeDef *init)
{
  ...
  pcnt->CFG &=
      ~(_PCNT_CFG_DEBUGHALT_MASK | _PCNT_CFG_FILTEN_MASK | _PCNT_CFG_HYST_MASK);
  ...
  if (init->mode != pcntModeDisable) {
    pcnt->CFG = ((pcnt->CFG & (~_PCNT_CFG_MODE_MASK))
                              ...
                              bool enable)
{
  ...
  ovscfg = ((config->filtLen & _PCNT_OVSCFG_FILTLEN_MASK)
            << _PCNT_OVSCFG_FILTLEN_SHIFT)
...
#endif
  /* Construct the new filter setting value. */
  ovscfg = ((config->filtLen & _PCNT_OVSCTRL_FILTLEN_MASK)
            << _PCNT_OVSCTRL_FILTLEN_SHIFT)
...
{
  uint32_t ctrl = 0;
  uint32_t mask = _PCNT_CTRL_TCCMODE_MASK | _PCNT_CTRL_TCCPRESC_MASK
                  | _PCNT_CTRL_TCCCOMP_MASK | _PCNT_CTRL_PRSGATEEN_MASK
                  | _PCNT_CTRL_TCCPRSPOL_MASK | _PCNT_CTRL_TCCPRSSEL_MASK;

  EFM_ASSERT(PCNT_REF_VALID(pcnt));

  /* Construct the TCC part of the configuration register. */
  ctrl |= (config->mode << _PCNT_CTRL_TCCMODE_SHIFT) & _PCNT_CTRL_TCCMODE_MASK;
  ctrl |= (config->prescaler << _PCNT_CTRL_TCCPRESC_SHIFT)
          & _PCNT_CTRL_TCCPRESC_MASK;
  ctrl |=
      (config->compare << _PCNT_CTRL_TCCCOMP_SHIFT) & _PCNT_CTRL_TCCCOMP_MASK;
  ctrl |= (config->tccPRS << _PCNT_CTRL_TCCPRSSEL_SHIFT)
          & _PCNT_CTRL_TCCPRSSEL_MASK;
  ctrl |= (config->prsPolarity << _PCNT_CTRL_TCCPRSPOL_SHIFT)
          & _PCNT_CTRL_TCCPRSPOL_MASK;
  ctrl |= (config->prsGateEnable << _PCNT_CTRL_PRSGATEEN_SHIFT)
