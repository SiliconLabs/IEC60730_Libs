...
// Typical frequency for HFXO as recommended in the datasheets.
...
#define HFRCOCALTABLE_ENTRIES                                                  \
  (sizeof(hfrcoCalTable) / sizeof(struct hfrcoCalTableElement))
...
static int8_t ctuneDelta =
    40; // Recommendation from analog team to counter the internal chip imbalance.
#else
 ...
 *
 * @details
 *   Run a calibration of a selectable reference clock against HCLK. Please
                         ...
                         CMU_Select_TypeDef upSel)
{
  ...
  EFM_ASSERT(downCycles
             <= (_CMU_CALCTRL_CALTOP_MASK >> _CMU_CALCTRL_CALTOP_SHIFT));
  calCtrl |= downCycles;
...
#elif (_SILICON_LABS_32B_SERIES_2_CONFIG > 1)
      EFM_ASSERT((div == 1U) || (div == 2U) || (div == 4U) || (div == 8U)
                 || (div == 16U));
#endif
...
void CMU_ClockSelectSet(CMU_Clock_TypeDef clock, CMU_Select_TypeDef ref)
{
  ...
  switch (clock) {
    ...
    case cmuClock_IADCCLK:
      ...
      CMU->IADCCLKCTRL =
          (CMU->IADCCLKCTRL & ~_CMU_IADCCLKCTRL_CLKSEL_MASK) | tmp;
      break;
    ...
    case cmuClock_EM01GRPACLK:
      ...
      CMU->EM01GRPACLKCTRL =
          (CMU->EM01GRPACLKCTRL & ~_CMU_EM01GRPACLKCTRL_CLKSEL_MASK) | tmp;
      break;
...
#if defined(_SILICON_LABS_32B_SERIES_2_CONFIG_1)
          SYSCFG->CFGSYSTIC =
              (SYSCFG->CFGSYSTIC | _SYSCFG_CFGSYSTIC_SYSTICEXTCLKEN_MASK);
#else
    ...
    case cmuClock_LESENSECLK:
      ...
      CMU->EM23GRPACLKCTRL =
          (CMU->EM23GRPACLKCTRL & ~_CMU_EM23GRPACLKCTRL_CLKSEL_MASK) | tmp;
      break;
    ...
    case cmuClock_EM4GRPACLK:
      ...
      CMU->EM4GRPACLKCTRL =
          (CMU->EM4GRPACLKCTRL & ~_CMU_EM4GRPACLKCTRL_CLKSEL_MASK) | tmp;
      break;
    ...
    case cmuClock_EM01GRPBCLK:
      ...
      CMU->EM01GRPBCLKCTRL =
          (CMU->EM01GRPBCLKCTRL & ~_CMU_EM01GRPBCLKCTRL_CLKSEL_MASK) | tmp;
      break;
#endif
    ...
    case cmuClock_WDOG0CLK:
      ...
      CMU->WDOG0CLKCTRL =
          (CMU->WDOG0CLKCTRL & ~_CMU_WDOG0CLKCTRL_CLKSEL_MASK) | tmp;
      break;
    ...
    case cmuClock_WDOG1CLK:
      ...
      CMU->WDOG1CLKCTRL =
          (CMU->WDOG1CLKCTRL & ~_CMU_WDOG1CLKCTRL_CLKSEL_MASK) | tmp;
      break;
#endif
    ...
    case cmuClock_DPLLREFCLK:
      ...
      CMU->DPLLREFCLKCTRL =
          (CMU->DPLLREFCLKCTRL & ~_CMU_DPLLREFCLKCTRL_CLKSEL_MASK) | tmp;
      break;
    ...
    case cmuClock_TRACECLK:
     ...
     && defined(CoreDebug_DEMCR_TRCENA_Msk))
      ...
      CMU->TRACECLKCTRL =
          (CMU->TRACECLKCTRL & ~_CMU_TRACECLKCTRL_CLKSEL_MASK) | tmp;
#if (defined(_SILICON_LABS_32B_SERIES_2_CONFIG_4)                              \
     && defined(CoreDebug_DEMCR_TRCENA_Msk))
      if (restoreTrace) {
        CoreDebug->DEMCR |= CoreDebug_DEMCR_TRCENA_Msk;
    ...
    case cmuClock_EUART0CLK:
      ...
      CMU->EUART0CLKCTRL =
          (CMU->EUART0CLKCTRL & ~_CMU_EUART0CLKCTRL_CLKSEL_MASK) | tmp;
      break;
#elif defined(EUSART_PRESENT)
    case cmuClock_EUSART0:
    case cmuClock_EUSART0CLK:
      ...
      CMU->EUSART0CLKCTRL =
          (CMU->EUSART0CLKCTRL & ~_CMU_EUSART0CLKCTRL_CLKSEL_MASK) | tmp;
      break;
    ...
    case cmuClock_EM01GRPCCLK:
      ...
      CMU->EM01GRPCCLKCTRL =
          (CMU->EM01GRPCCLKCTRL & ~_CMU_EM01GRPCCLKCTRL_CLKSEL_MASK) | tmp;
      break;
#endif
...
// -----------------------------------------------------------------------------
    ...
    case cmuClock_RTCCCLK:
      ...
      CMU->RTCCCLKCTRL =
          (CMU->RTCCCLKCTRL & ~_CMU_RTCCCLKCTRL_CLKSEL_MASK) | tmp;
      break;
#endif
// -----------------------------------------------------------------------------
#if defined(SYSRTC_PRESENT)
    ...
    case cmuClock_SYSRTCCLK:
      ...
      CMU->SYSRTC0CLKCTRL =
          (CMU->SYSRTC0CLKCTRL & ~_CMU_SYSRTC0CLKCTRL_CLKSEL_MASK) | tmp;
      break;
#endif
// -----------------------------------------------------------------------------
#if defined(LCD_PRESENT)
...
// -----------------------------------------------------------------------------
    ...
    case cmuClock_VDAC0CLK:
      ...
      CMU->VDAC0CLKCTRL =
          (CMU->VDAC0CLKCTRL & ~_CMU_VDAC0CLKCTRL_CLKSEL_MASK) | tmp;
      break;
#if (VDAC_COUNT > 1)
    case cmuClock_VDAC1:
    case cmuClock_VDAC1CLK:
      ...
      CMU->VDAC1CLKCTRL =
          (CMU->VDAC1CLKCTRL & ~_CMU_VDAC1CLKCTRL_CLKSEL_MASK) | tmp;
      break;
#endif
#endif /* VDAC_PRESENT */
// -----------------------------------------------------------------------------
    ...
    case cmuClock_PCNT0CLK:
      ...
      CMU->PCNT0CLKCTRL =
          (CMU->PCNT0CLKCTRL & ~_CMU_PCNT0CLKCTRL_CLKSEL_MASK) | tmp;
      break;
#endif
// -----------------------------------------------------------------------------
#if defined(LESENSE_PRESENT)
    case cmuClock_LESENSEHFCLK:
      ...
      CMU->LESENSEHFCLKCTRL =
          (CMU->LESENSEHFCLKCTRL & ~_CMU_LESENSEHFCLKCTRL_CLKSEL_MASK) | tmp;
      break;
#endif
    default:
      EFM_ASSERT(false);
...
bool CMU_DPLLLock(const CMU_DPLLInit_TypeDef *init)
{
  ...
  if (hfrcoFreqRangeExpected == hfrcoFreqRangeActual) {
    ...
    DPLL0->CFG =
        ((init->autoRecover ? 1UL : 0UL) << _DPLL_CFG_AUTORECOVER_SHIFT)
...
void CMU_HFXOInit(const CMU_HFXOInit_TypeDef *hfxoInit)
{
  ...
  // bitfield.
  EFM_ASSERT(hfxoInit->timeoutCbLsb <= (_HFXO_XTALCFG_TIMEOUTCBLSB_MASK
  ...
  EFM_ASSERT(hfxoInit->timeoutSteady <= (_HFXO_XTALCFG_TIMEOUTSTEADY_MASK
                                         >> _HFXO_XTALCFG_TIMEOUTSTEADY_SHIFT));
  ...
  EFM_ASSERT(hfxoInit->coreDegenAna <= (_HFXO_XTALCTRL_COREDGENANA_MASK
                                        >> _HFXO_XTALCTRL_COREDGENANA_SHIFT));
  EFM_ASSERT(hfxoInit->ctuneFixAna <= (_HFXO_XTALCTRL_CTUNEFIXANA_MASK
...
#if defined(_DEVINFO_HFXOCAL_VTRTRIMANA_MASK) && defined(_HFXO_BUFOUTTRIM_MASK)
  {
    ...
    tmp =
        BUS_RegMaskedRead(&DEVINFO->HFXOCAL, _DEVINFO_HFXOCAL_VTRTRIMANA_MASK);
...
#if defined(_DEVINFO_HFXOCAL_SHUNTBIASANA_MASK)                                \
    && defined(_HFXO_LOWPWRCTRL_MASK)
  {
    uint32_t tmp;
  ...
  HFXO0->XTALCTRL =
      (HFXO0->XTALCTRL & _HFXO_XTALCTRL_SKIPCOREBIASOPT_MASK)
  ...
  HFXO0->CFG =
      (HFXO0->CFG
       & ~(_HFXO_CFG_SQBUFSCHTRGANA_MASK | _HFXO_CFG_ENXIDCBIASANA_MASK
           | _HFXO_CFG_MODE_MASK))
      | ((hfxoInit->mode == cmuHfxoOscMode_Crystal) ? 0
                                                    : HFXO_CFG_SQBUFSCHTRGANA)
    ...
    unsigned int pin)
{
  ...
  GPIO->SYXOROUTE[0].BUFOUTREQINASYNCROUTE =
      pin << _GPIO_SYXO_BUFOUTREQINASYNCROUTE_PIN_SHIFT
  ...
  HFXO0->CTRL_CLR =
      _HFXO_CTRL_BUFOUTFREEZE_MASK | _HFXO_CTRL_DISONDEMANDBUFOUT_MASK;
    ...
    unsigned int pin)
{
  ...
  (&(GPIO->PRSROUTE[0].ASYNCH0ROUTE))[prsAsyncCh] =
      pin << _GPIO_PRS_ASYNCH0ROUTE_PIN_SHIFT
      | port << _GPIO_PRS_ASYNCH0ROUTE_PORT_SHIFT;
  GPIO->PRSROUTE[0].ROUTEEN =
      1U << (_GPIO_PRS_ROUTEEN_ASYNCH0PEN_SHIFT + prsAsyncCh);
}
#endif
...
void CMU_OscillatorTuningSet(CMU_Osc_TypeDef osc, uint32_t val)
{
  ...
  switch (osc) {
...
#endif
      EFM_ASSERT(val
                 <= (_LFRCO_CAL_FREQTRIM_MASK >> _LFRCO_CAL_FREQTRIM_SHIFT));
    ...
    case cmuOsc_HFXO:
      ...
      CMU->CLKEN0_SET = CMU_CLKEN0_HFXO0;
#endif
      EFM_ASSERT(val <= (_HFXO_XTALCTRL_COREBIASANA_MASK
                         >> _HFXO_XTALCTRL_COREBIASANA_SHIFT));
 ...
 *   never attempts to set core frequency above 40MHz at VSCALE1 (1.0V).
...
static void flashWaitStateControl(uint32_t coreFreq, int vscale)
{
  ...
  mscLocked =
      (MSC->STATUS & _MSC_STATUS_REGLOCK_MASK) == MSC_STATUS_REGLOCK_LOCKED;
...
#define HFXO_TUNING_READY_FLAGS                                                \
...
#define HFRCOCTRLTABLE_ENTRIES                                                 \
  (sizeof(hfrcoCtrlTable) / sizeof(struct hfrcoCtrlTableElement))
...
static const struct flashWsTableElement {
  ...
  uint8_t ws;
     ...
     || _SILICON_LABS_GECKO_INTERNAL_SDID == 106)
    {CMU_MAX_FREQ_0WS_1V2,
     0,
     0}, /* 0 wait states at max frequency 18 MHz and 1.2V */
    {CMU_MAX_FREQ_1WS_1V2,
     0,
     1}, /* 1 wait states at max frequency 36 MHz and 1.2V */
    {CMU_MAX_FREQ_2WS_1V2,
     0,
     2}, /* 2 wait states at max frequency 54 MHz and 1.2V */
    {CMU_MAX_FREQ_3WS_1V2,
     0,
     3}, /* 3 wait states at max frequency 72 MHz and 1.2V */
    {CMU_MAX_FREQ_0WS_1V0,
     2,
     0}, /* 0 wait states at max frequency 7 MHz and 1.0V */
    {CMU_MAX_FREQ_1WS_1V0,
     2,
     1}, /* 1 wait states at max frequency 14 MHz and 1.0V */
    {CMU_MAX_FREQ_2WS_1V0,
     2,
     2}, /* 2 wait states at max frequency 21 MHz and 1.0V */
#else
...
#if defined(_CMU_USHFRCOCTRL_FREQRANGE_MASK) || defined(_CMU_USHFRCOTUNE_MASK)
...
static uint32_t lfClkGet(CMU_Clock_TypeDef lfClkBranch)
{
  ...
  switch (lfClkBranch) {
    ...
    case cmuClock_LFC:
      ...
      sel = (CMU->LFCCLKSEL & _CMU_LFCCLKSEL_LFC_MASK)
            >> _CMU_LFCCLKSEL_LFC_SHIFT;
#else
      EFM_ASSERT(false);
#endif
      break;
 ...
 *   frequency while still being within spec.
 *
 * @note
 *   This function applies to EFM32GG11B. There are 3 HFPER clock trees with
 *   these frequency limits:
                         ...
                         CMU_Osc_TypeDef upSel)
{
  ...
  uint32_t calCtrl =
      CMU->CALCTRL & ~(_CMU_CALCTRL_UPSEL_MASK | _CMU_CALCTRL_DOWNSEL_MASK);
    ...
    case CMU_LFAPRESC0_REG:
      switch (clock) {
        ...
        case cmuClock_LCDpre:
          ret =
              (CMU_ClkDiv_TypeDef) (((CMU->LFAPRESC0 & _CMU_LFAPRESC0_LCD_MASK)
...
uint32_t CMU_ClockFreqGet(CMU_Clock_TypeDef clock)
{
  ...
  switch ((unsigned) clock & (CMU_CLK_BRANCH_MASK << CMU_CLK_BRANCH_POS)) {
    ...
    case (CMU_HFPERB_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ...
      ret /= 1U
             + ((CMU->HFPERPRESCB & _CMU_HFPERPRESCB_PRESC_MASK)
    ...
    case (CMU_HFPERC_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ...
      ret /= 1U
             + ((CMU->HFPERPRESCC & _CMU_HFPERPRESCC_PRESC_MASK)
                >> _CMU_HFPERPRESCC_PRESC_SHIFT);
      break;
#endif

#if defined(_SILICON_LABS_32B_SERIES_1)
#if defined(CRYPTO_PRESENT) || defined(LDMA_PRESENT) || defined(GPCRC_PRESENT) \
    || defined(PRS_PRESENT) || defined(GPIO_PRESENT)
    case (CMU_HFBUS_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ret = SystemHFClockGet();
#if defined(_CMU_HFBUSPRESC_MASK)
      ret /= 1U
             + ((CMU->HFBUSPRESC & _CMU_HFBUSPRESC_MASK)
...
#if defined(_SILICON_LABS_32B_SERIES_0)
#if defined(AES_PRESENT) || defined(DMA_PRESENT) || defined(EBI_PRESENT)       \
    || defined(USB_PRESENT)
    case (CMU_HFCORE_CLK_BRANCH << CMU_CLK_BRANCH_POS):
    {
    ...
    case (CMU_ADC0ASYNC_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ...
      ret /= 1U
             + ((CMU->ADCCTRL & _CMU_ADCCTRL_ADC0CLKDIV_MASK)
    ...
    case (CMU_ADC1ASYNC_CLK_BRANCH << CMU_CLK_BRANCH_POS):
      ...
      ret /= 1U
             + ((CMU->ADCCTRL & _CMU_ADCCTRL_ADC1CLKDIV_MASK)
  ...
  switch (selRegId) {
    ...
    case CMU_LFBCLKSEL_REG:
      ...
      switch (ref) {
        ...
        case cmuSelect_LFXO:
          /* Ensure that these elected oscillator is enabled, waiting for it to stabilize. */
    ...
    case CMU_DBGCLKSEL_REG:
      switch (ref) {
        ...
        case cmuSelect_AUXHFRCO:
          /* Select AUXHFRCO as a debug clock. */
          CMU->CTRL =
              (CMU->CTRL & ~(_CMU_CTRL_DBGCLK_MASK)) | CMU_CTRL_DBGCLK_AUXHFRCO;
          break;

        case cmuSelect_HFCLK:
          /* Select divided HFCLK as a debug clock. */
          CMU->CTRL =
              (CMU->CTRL & ~(_CMU_CTRL_DBGCLK_MASK)) | CMU_CTRL_DBGCLK_HFCLK;
          break;
#endif
...
bool CMU_DPLLLock(const CMU_DPLLInit_TypeDef *init)
{
  ...
  EFM_ASSERT(init->ssInterval
             <= (_CMU_HFRCOSS_SSINV_MASK >> _CMU_HFRCOSS_SSINV_SHIFT));
  EFM_ASSERT(init->ssAmplitude
             <= (_CMU_HFRCOSS_SSAMP_MASK >> _CMU_HFRCOSS_SSAMP_SHIFT));
  ...
  CMU->IFC =
      CMU_IFC_DPLLRDY | CMU_IFC_DPLLLOCKFAILLOW | CMU_IFC_DPLLLOCKFAILHIGH;
  CMU->DPLLCTRL1 = ((uint32_t) init->n << _CMU_DPLLCTRL1_N_SHIFT)
                   | ((uint32_t) init->m << _CMU_DPLLCTRL1_M_SHIFT);
  CMU->HFRCOCTRL = hfrcoCtrlVal;
  CMU->DPLLCTRL =
      ((uint32_t) init->refClk << _CMU_DPLLCTRL_REFSEL_SHIFT)
      | ((init->autoRecover ? 1UL : 0UL) << _CMU_DPLLCTRL_AUTORECOVER_SHIFT)
  ...
  while ((lockStatus = (CMU->IF
                        & (CMU_IF_DPLLRDY | CMU_IF_DPLLLOCKFAILLOW
                           | CMU_IF_DPLLLOCKFAILHIGH)))
         == 0UL) {}
...
void CMU_HFRCOBandSet(CMU_HFRCOFreq_TypeDef setFreq)
{
  ...
  switch (setFreq) {
    case cmuHFRCOFreq_1M0Hz:
      freqCal =
          (freqCal & ~_CMU_HFRCOCTRL_CLKDIV_MASK) | CMU_HFRCOCTRL_CLKDIV_DIV4;
      break;

    case cmuHFRCOFreq_2M0Hz:
      freqCal =
          (freqCal & ~_CMU_HFRCOCTRL_CLKDIV_MASK) | CMU_HFRCOCTRL_CLKDIV_DIV2;
      break;

    case cmuHFRCOFreq_4M0Hz:
      freqCal =
          (freqCal & ~_CMU_HFRCOCTRL_CLKDIV_MASK) | CMU_HFRCOCTRL_CLKDIV_DIV1;
      break;
                             ...
                             bool enEM0EM1StartSel)
{
  ...
  if (enEM0EM1StartSel) {
    ...
    EFM_ASSERT((EMU->CTRL & _EMU_CTRL_EM23VSCALE_MASK)
...
void CMU_HFXOInit(const CMU_HFXOInit_TypeDef *hfxoInit)
{
  ...
  /* Set timeouts */
  CMU->HFXOTIMEOUTCTRL =
      (hfxoInit->timeoutPeakDetect << _CMU_HFXOTIMEOUTCTRL_PEAKDETTIMEOUT_SHIFT)
  ...
  EFM_ASSERT(!(hfxoInit->autoStartEm01 || hfxoInit->autoSelEm01
  ...
  CMU->HFXOCTRL1 =
      (CMU->HFXOCTRL1 & ~_CMU_HFXOCTRL1_PEAKDETTHR_MASK)
      | (hfxoInit->thresholdPeakDetect << _CMU_HFXOCTRL1_PEAKDETTHR_SHIFT);
#endif
  /* Set tuning for startup and steady state. */
  CMU->HFXOSTARTUPCTRL =
      ((uint32_t) hfxoInit->ctuneStartup << _CMU_HFXOSTARTUPCTRL_CTUNE_SHIFT)
...
#else
  CMU->CTRL = (CMU->CTRL
               & ~(_CMU_CTRL_HFXOTIMEOUT_MASK | _CMU_CTRL_HFXOBOOST_MASK
                   | _CMU_CTRL_HFXOMODE_MASK | _CMU_CTRL_HFXOGLITCHDETEN_MASK))
...
void CMU_LFXOInit(const CMU_LFXOInit_TypeDef *lfxoInit)
{
  ...
  BUS_RegMaskedWrite(
      &CMU->CTRL,
      _CMU_CTRL_LFXOTIMEOUT_MASK | _CMU_CTRL_LFXOBOOST_MASK
          | _CMU_CTRL_LFXOMODE_MASK,
      ((uint32_t) lfxoInit->timeout << _CMU_CTRL_LFXOTIMEOUT_SHIFT)
  ...
  BUS_RegBitWrite(&EMU->AUXCTRL,
                  _EMU_AUXCTRL_REDLFXOBOOST_SHIFT,
                  emuReduce ? 1 : 0);
#endif
#endif
}
...
#if defined(_CMU_HFXOSTEADYSTATECTRL_REGISH_MASK)
      regIshUpper =
          getRegIshUpperVal((val & _CMU_HFXOSTEADYSTATECTRL_REGISH_MASK)
                            >> _CMU_HFXOSTEADYSTATECTRL_REGISH_SHIFT);
      CMU->HFXOSTEADYSTATECTRL =
          (CMU->HFXOSTEADYSTATECTRL
           & ~(_CMU_HFXOSTEADYSTATECTRL_IBTRIMXOCORE_MASK
...
{
  return (
      CMU_USHFRCOBand_TypeDef) ((CMU->USHFRCOCONF & _CMU_USHFRCOCONF_BAND_MASK)
...
void CMU_USHFRCOBandSet(CMU_USHFRCOBand_TypeDef band)
{
  ...
  switch (band) {
    case cmuUSHFRCOBand_24MHz:
      ...
      fineTuning =
          (DEVINFO->USHFRCOCAL0 & _DEVINFO_USHFRCOCAL0_BAND24_FINETUNING_MASK)
    ...
    case cmuUSHFRCOBand_48MHz:
      ...
      fineTuning =
          (DEVINFO->USHFRCOCAL0 & _DEVINFO_USHFRCOCAL0_BAND48_FINETUNING_MASK)
