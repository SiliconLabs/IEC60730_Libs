...
void LESENSE_Init(const LESENSE_Init_TypeDef *init, bool reqReset)
{
  ...
  LESENSE->PERCTRL =
...
#endif
      | ((uint32_t) init->perCtrl.dacCh0Data
      ...
      | ((uint32_t) init->perCtrl.dacStartupHalf
         << _LESENSE_PERCTRL_DACSTARTUP_SHIFT)
...
uint32_t LESENSE_ScanFreqSet(uint32_t refFreq, uint32_t scanFreq)
{
  ...
  EFM_ASSERT(pcTop
             <= (_LESENSE_TIMCTRL_PCTOP_MASK >> _LESENSE_TIMCTRL_PCTOP_SHIFT));
...
void LESENSE_ChannelConfig(const LESENSE_ChDesc_TypeDef *confCh, uint32_t chIdx)
{
  ...
  EFM_ASSERT(confCh->exTime <= (_LESENSE_CH_TIMING_EXTIME_MASK
                                >> _LESENSE_CH_TIMING_EXTIME_SHIFT));
  EFM_ASSERT(confCh->measDelay <= (_LESENSE_CH_TIMING_MEASUREDLY_MASK
                                   >> _LESENSE_CH_TIMING_MEASUREDLY_SHIFT));
#if defined(_SILICON_LABS_32B_SERIES_0)
  // Sample delay on other devices are 8 bits which fits perfectly in uint8_t.
  EFM_ASSERT(confCh->sampleDelay <= (_LESENSE_CH_TIMING_SAMPLEDLY_MASK
  ...
  LESENSE_ChannelThresSet(chIdx, confCh->acmpThres, confCh->cntThres);
...
void LESENSE_AltExConfig(const LESENSE_ConfAltEx_TypeDef *confAltEx)
{
  ...
  for (i = 0U; i < 16U; ++i) {
    ...
    /* An atomic read-modify-write using BUS_RegBitWrite function to
     * support reconfiguration during the LESENSE operation. */
    BUS_RegBitWrite(&GENERIC_LESENSE_ROUTE, i, confAltEx->AltEx[i].enablePin);
  }
#endif
}
                              ...
                              uint16_t measDelay)
{
  ...
  EFM_ASSERT(exTime <= (_LESENSE_CH_TIMING_EXTIME_MASK
                        >> _LESENSE_CH_TIMING_EXTIME_SHIFT));
  EFM_ASSERT(measDelay <= (_LESENSE_CH_TIMING_MEASUREDLY_MASK
                           >> _LESENSE_CH_TIMING_MEASUREDLY_SHIFT));
#if defined(_SILICON_LABS_32B_SERIES_0)
  // A sample delay on other devices is 8 bits which fits perfectly in uint8_t.
  EFM_ASSERT(sampleDelay <= (_LESENSE_CH_TIMING_SAMPLEDLY_MASK
                             >> _LESENSE_CH_TIMING_SAMPLEDLY_SHIFT));
#endif
                             ...
                             uint16_t cntThres)
{
  ...
  LESENSE->CH[chIdx].EVALTHRES = (uint32_t) cntThres
                                 << _LESENSE_CH_EVALTHRES_EVALTHRES_SHIFT;
...
#if defined(_SILICON_LABS_32B_SERIES_0) || defined(_SILICON_LABS_32B_SERIES_1)
  ch->EVAL = (ch->EVAL
              & ~(_LESENSE_CH_EVAL_COMPTHRES_MASK | _LESENSE_CH_EVAL_MODE_MASK))
...
void LESENSE_WindowSizeSet(uint32_t windowSize)
{
  ...
  LESENSE->EVALCTRL =
      (LESENSE->EVALCTRL & ~_LESENSE_EVALCTRL_WINSIZE_MASK) | windowSize;
                                ...
                                uint32_t decSt)
{
  ...
  EFM_ASSERT((uint32_t) confDecSt->confA.nextState
  ...
  EFM_ASSERT((uint32_t) confDecSt->confB.nextState
             < LESENSE_NUM_DECODER_STATES);
  ...
  LESENSE->ST[decSt].ARC =
      (uint32_t) confDecSt->prsAct
