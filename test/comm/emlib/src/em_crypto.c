...
#define CRYPTO_INSTRUCTION_REGS                                                \
  (CRYPTO_INSTRUCTIONS_MAX / CRYPTO_INSTRUCTIONS_PER_REG)

#define CRYPTO_SHA1_BLOCK_SIZE_IN_BITS  (512UL)
#define CRYPTO_SHA1_BLOCK_SIZE_IN_BYTES (CRYPTO_SHA1_BLOCK_SIZE_IN_BITS / 8UL)
#define CRYPTO_SHA1_BLOCK_SIZE_IN_32BIT_WORDS                                  \
  (CRYPTO_SHA1_BLOCK_SIZE_IN_BYTES / sizeof(uint32_t))
#define CRYPTO_SHA1_DIGEST_SIZE_IN_32BIT_WORDS                                 \
  (CRYPTO_SHA1_DIGEST_SIZE_IN_BYTES / sizeof(uint32_t))

#define CRYPTO_SHA256_BLOCK_SIZE_IN_BITS (512UL)
#define CRYPTO_SHA256_BLOCK_SIZE_IN_BYTES                                      \
  (CRYPTO_SHA256_BLOCK_SIZE_IN_BITS / 8UL)
#define CRYPTO_SHA256_BLOCK_SIZE_IN_32BIT_WORDS                                \
  (CRYPTO_SHA256_BLOCK_SIZE_IN_BYTES / sizeof(uint32_t))

#define CRYPTO_SHA256_DIGEST_SIZE_IN_32BIT_WORDS                               \
  (CRYPTO_SHA256_DIGEST_SIZE_IN_BYTES / sizeof(uint32_t))
...
#define PARTIAL_OPERAND_WIDTH_IN_32BIT_WORDS                                   \
  (PARTIAL_OPERAND_WIDTH_IN_BYTES / sizeof(uint32_t))
...
{
  uint32_t temp =
      crypto->WAC & (~(_CRYPTO_WAC_MODULUS_MASK | _CRYPTO_WAC_MODOP_MASK));
                    ...
                    CRYPTO_SHA256_Digest_TypeDef msgDigest)
{
  ...
  /* Copy data of DDATA0 and select DDATA0 and DDATA1 for SHA operation. */
  CRYPTO_EXECUTE_2(crypto,
                   CRYPTO_CMD_INSTR_DDATA1TODDATA0,
                   CRYPTO_CMD_INSTR_SELDDATA0DDATA1);
  len = (uint32_t) msgLen;
                ...
                int rSize)
{
...
#ifdef USE_VARIABLE_SIZED_DATA_LOADS
  unsigned numWordsLastOperandA =
      ((unsigned) aSize & PARTIAL_OPERAND_WIDTH_MASK) >> 5;
  unsigned numPartialOperandsA =
      numWordsLastOperandA
          ? ((unsigned) aSize >> PARTIAL_OPERAND_WIDTH_LOG2) + 1
          : (unsigned) aSize >> PARTIAL_OPERAND_WIDTH_LOG2;
  unsigned numWordsLastOperandB =
      ((unsigned) bSize & PARTIAL_OPERAND_WIDTH_MASK) >> 5;
  unsigned numPartialOperandsB =
      numWordsLastOperandB
          ? ((unsigned) bSize >> PARTIAL_OPERAND_WIDTH_LOG2) + 1
          : (unsigned) bSize >> PARTIAL_OPERAND_WIDTH_LOG2;
  unsigned numWordsLastOperandR =
      ((unsigned) rSize & PARTIAL_OPERAND_WIDTH_MASK) >> 5;
  unsigned numPartialOperandsR =
  ...
  /* Set up DMA request signaling for MCU to run in parallel with
     ...
     sequence. */
  crypto->CTRL = CRYPTO_CTRL_DMA0RSEL_DATA0 | CRYPTO_CTRL_DMA0MODE_FULL
...
#ifdef USE_VARIABLE_SIZED_DATA_LOADS
      if ((numWordsLastOperandR != 0)
          && ((i + j + 1U) == numPartialOperandsR - 1U)) {
        CRYPTO_DataWriteVariableSize(
            &crypto->DATA1,
                            ...
                            CRYPTO_KeyWidth_TypeDef keyWidth)
{
  ...
  CRYPTO_AES_ProcessLoop(crypto, len, &crypto->DATA1, in, &crypto->DATA0, out);
}
                             ...
                             CRYPTO_KeyWidth_TypeDef keyWidth)
{
  ...
  CRYPTO_AES_ProcessLoop(crypto, len, &crypto->DATA1, in, &crypto->DATA2, out);
}
                            ...
                            CRYPTO_KeyWidth_TypeDef keyWidth)
{
  ...
  CRYPTO_AES_ProcessLoop(crypto, len, &crypto->DATA2, in, &crypto->DATA0, out);
                            ...
                            CRYPTO_KeyWidth_TypeDef keyWidth)
{
  ...
  CRYPTO_AES_ProcessLoop(crypto, len, &crypto->DATA0, in, &crypto->DATA1, out);
}
