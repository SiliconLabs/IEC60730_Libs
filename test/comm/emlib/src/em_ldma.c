...
void LDMA_Init(const LDMA_Init_t *init)
{
  ...
  EFM_ASSERT(
      !(((uint32_t) init->ldmaInitCtrlNumFixed << _LDMA_CTRL_NUMFIXED_SHIFT)
        & ~_LDMA_CTRL_NUMFIXED_MASK));

#if defined(_LDMA_CTRL_SYNCPRSCLREN_SHIFT)                                     \
    && defined(_LDMA_CTRL_SYNCPRSSETEN_SHIFT)
  EFM_ASSERT(!(((uint32_t) init->ldmaInitCtrlSyncPrsClrEn
                << _LDMA_CTRL_SYNCPRSCLREN_SHIFT)
               & ~_LDMA_CTRL_SYNCPRSCLREN_MASK));
  EFM_ASSERT(!(((uint32_t) init->ldmaInitCtrlSyncPrsSetEn
                << _LDMA_CTRL_SYNCPRSSETEN_SHIFT)
               & ~_LDMA_CTRL_SYNCPRSSETEN_MASK));
#endif

#if defined(_LDMA_SYNCHWEN_SYNCCLREN_SHIFT)                                    \
    && defined(_LDMA_SYNCHWEN_SYNCSETEN_SHIFT)
  EFM_ASSERT(!(((uint32_t) init->ldmaInitCtrlSyncPrsClrEn
                        ...
                        const LDMA_Descriptor_t *descriptor)
{
...
#if defined(_LDMA_SYNCHWEN_SYNCCLREN_SHIFT)                                    \
    && defined(_LDMA_SYNCHWEN_SYNCSETEN_SHIFT)
  EFM_ASSERT(!(((uint32_t) transfer->ldmaCtrlSyncPrsClrOff
                << _LDMA_SYNCHWEN_SYNCCLREN_SHIFT)
               & ~_LDMA_SYNCHWEN_SYNCCLREN_MASK));
  EFM_ASSERT(!(((uint32_t) transfer->ldmaCtrlSyncPrsClrOn
                << _LDMA_SYNCHWEN_SYNCCLREN_SHIFT)
               & ~_LDMA_SYNCHWEN_SYNCCLREN_MASK));
  EFM_ASSERT(!(((uint32_t) transfer->ldmaCtrlSyncPrsSetOff
                << _LDMA_SYNCHWEN_SYNCSETEN_SHIFT)
               & ~_LDMA_SYNCHWEN_SYNCSETEN_MASK));
  EFM_ASSERT(!(((uint32_t) transfer->ldmaCtrlSyncPrsSetOn
                << _LDMA_SYNCHWEN_SYNCSETEN_SHIFT)
               & ~_LDMA_SYNCHWEN_SYNCSETEN_MASK));
#elif defined(_LDMA_CTRL_SYNCPRSCLREN_SHIFT)                                   \
    && defined(_LDMA_CTRL_SYNCPRSSETEN_SHIFT)
  EFM_ASSERT(!(((uint32_t) transfer->ldmaCtrlSyncPrsClrOff
                << _LDMA_CTRL_SYNCPRSCLREN_SHIFT)
               & ~_LDMA_CTRL_SYNCPRSCLREN_MASK));
  ...
  EFM_ASSERT(
      !(((uint32_t) transfer->ldmaCfgArbSlots << _LDMA_CH_CFG_ARBSLOTS_SHIFT)
...
#endif
  LDMA->CH[ch].LOOP = (uint32_t) transfer->ldmaLoopCnt
                      << _LDMA_CH_LOOP_LOOPCNT_SHIFT;
  LDMA->CH[ch].CFG =
      ((uint32_t) transfer->ldmaCfgArbSlots << _LDMA_CH_CFG_ARBSLOTS_SHIFT)
      | ((uint32_t) transfer->ldmaCfgSrcIncSign
         << _LDMA_CH_CFG_SRCINCSIGN_SHIFT)
      | ((uint32_t) transfer->ldmaCfgDstIncSign
...
#else
  CORE_ATOMIC_SECTION(LDMA->IEN &= ~chMask;
...
bool LDMA_TransferDone(int ch)
{
  ...
  CORE_ATOMIC_SECTION(
      if (((LDMA->CHSTATUS & chMask) == 0)
          && ((LDMA->CHDONE & chMask) == chMask)) { retVal = true; })
#else
  CORE_ATOMIC_SECTION(
...
uint32_t LDMA_TransferRemainingCount(int ch)
{
  ...
  CORE_ATOMIC_SECTION(iflag = LDMA->IF; done = LDMA->CHDONE;
                      remaining              = LDMA->CH[ch].CTRL;)

  iflag &= chMask;
  done &= chMask;
  remaining =
      (remaining & _LDMA_CH_CTRL_XFERCNT_MASK) >> _LDMA_CH_CTRL_XFERCNT_SHIFT;
