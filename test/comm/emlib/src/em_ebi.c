...
void EBI_Init(const EBI_Init_TypeDef *ebiInit)
{
...
#if defined(_EFM32_GECKO_FAMILY)
  ctrl &=
      ~(_EBI_CTRL_MODE_MASK | _EBI_CTRL_ARDYEN_MASK | _EBI_CTRL_ARDYTODIS_MASK
        | _EBI_CTRL_BANK0EN_MASK | _EBI_CTRL_BANK1EN_MASK
        | _EBI_CTRL_BANK2EN_MASK | _EBI_CTRL_BANK3EN_MASK);
  if (ebiInit->enable) {
  ...
  if (ebiInit->banks & EBI_BANK0) {
    ctrl &=
        ~(_EBI_CTRL_MODE_MASK | _EBI_CTRL_ARDYEN_MASK | _EBI_CTRL_ARDYTODIS_MASK
          | _EBI_CTRL_BL_MASK | _EBI_CTRL_NOIDLE_MASK | _EBI_CTRL_BANK0EN_MASK);
  ...
  if (ebiInit->banks & EBI_BANK1) {
    ctrl &= ~(_EBI_CTRL_BL1_MASK | _EBI_CTRL_MODE1_MASK | _EBI_CTRL_ARDY1EN_MASK
              | _EBI_CTRL_ARDYTO1DIS_MASK | _EBI_CTRL_NOIDLE1_MASK
  ...
  if (ebiInit->banks & EBI_BANK2) {
    ctrl &= ~(_EBI_CTRL_BL2_MASK | _EBI_CTRL_MODE2_MASK | _EBI_CTRL_ARDY2EN_MASK
              | _EBI_CTRL_ARDYTO2DIS_MASK | _EBI_CTRL_NOIDLE2_MASK
  ...
  if (ebiInit->banks & EBI_BANK3) {
    ctrl &= ~(_EBI_CTRL_BL3_MASK | _EBI_CTRL_MODE3_MASK | _EBI_CTRL_ARDY3EN_MASK
              | _EBI_CTRL_ARDYTO3DIS_MASK | _EBI_CTRL_NOIDLE3_MASK
  ...
  EBI_GENERIC_ROUTE_REG =
      (EBI_GENERIC_ROUTE_REG & ~_EBI_GENERIC_ALB_MASK) | ebiInit->allow;
  EBI_GENERIC_ROUTE_REG =
      (EBI_GENERIC_ROUTE_REG & ~_EBI_GENERIC_APEN_MASK) | ebiInit->aHigh;
                       ...
                       uint32_t holdCycles)
{
  ...
  EFM_ASSERT(setupCycles <= _EBI_RDTIMING_RDSETUP_MASK
             >> _EBI_RDTIMING_RDSETUP_SHIFT);
  EFM_ASSERT(strobeCycles <= _EBI_RDTIMING_RDSTRB_MASK
             >> _EBI_RDTIMING_RDSTRB_SHIFT);
  EFM_ASSERT(holdCycles <= _EBI_RDTIMING_RDHOLD_MASK
                        ...
                        uint32_t holdCycles)
{
  ...
  /* Check that timings are within limits */
  EFM_ASSERT(setupCycles <= _EBI_WRTIMING_WRSETUP_MASK
             >> _EBI_WRTIMING_WRSETUP_SHIFT);
  EFM_ASSERT(strobeCycles <= _EBI_WRTIMING_WRSTRB_MASK
             >> _EBI_WRTIMING_WRSTRB_SHIFT);
  EFM_ASSERT(holdCycles <= _EBI_WRTIMING_WRHOLD_MASK
...
void EBI_AddressTimingSet(uint32_t setupCycles, uint32_t holdCycles)
{
  ...
  /* Check that timings are within limits */
  EFM_ASSERT(setupCycles <= _EBI_ADDRTIMING_ADDRSETUP_MASK
             >> _EBI_ADDRTIMING_ADDRSETUP_SHIFT);
  EFM_ASSERT(holdCycles <= _EBI_ADDRTIMING_ADDRHOLD_MASK
...
{
  EFM_ASSERT((horizontal - 1) <= _EBI_TFTSIZE_HSZ_MASK
...
{
  EFM_ASSERT(front <= _EBI_TFTHPORCH_HFPORCH_MASK
             >> _EBI_TFTHPORCH_HFPORCH_SHIFT);
  EFM_ASSERT(back <= _EBI_TFTHPORCH_HBPORCH_MASK
             >> _EBI_TFTHPORCH_HBPORCH_SHIFT);
  EFM_ASSERT((pulseWidth - 1) <= _EBI_TFTHPORCH_HSYNC_MASK
...
{
  EFM_ASSERT(front <= _EBI_TFTVPORCH_VFPORCH_MASK
             >> _EBI_TFTVPORCH_VFPORCH_SHIFT);
  EFM_ASSERT(back <= _EBI_TFTVPORCH_VBPORCH_MASK
             >> _EBI_TFTVPORCH_VBPORCH_SHIFT);
  EFM_ASSERT((pulseWidth - 1) <= _EBI_TFTVPORCH_VSYNC_MASK
...
{
  /* Check that timings are within limits */
  EFM_ASSERT(dclkPeriod <= _EBI_TFTTIMING_DCLKPERIOD_MASK
             >> _EBI_TFTTIMING_DCLKPERIOD_SHIFT);
  EFM_ASSERT(start <= _EBI_TFTTIMING_TFTSTART_MASK
             >> _EBI_TFTTIMING_TFTSTART_SHIFT);
  EFM_ASSERT(setup <= _EBI_TFTTIMING_TFTSETUP_MASK
             >> _EBI_TFTTIMING_TFTSETUP_SHIFT);
  EFM_ASSERT(hold <= _EBI_TFTTIMING_TFTHOLD_MASK
