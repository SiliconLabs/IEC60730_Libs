...
void BURTC_Enable(bool enable)
{
  ...
  EFM_ASSERT(
      ((enable == true)
       && ((BURTC->CTRL & _BURTC_CTRL_MODE_MASK) != BURTC_CTRL_MODE_DISABLE))
      || (enable == false));
...
uint32_t BURTC_ClockFreqGet(void)
{
  ...
  switch (clkSel) {
    ...
    case BURTC_CTRL_CLKSEL_LFRCO:
      frequency =
          SystemLFRCOClockGet() / (1 << clkDiv); /* freq=32768/2^clkDiv */
      break;

    /** Low-frequency crystal oscillator. */
    case BURTC_CTRL_CLKSEL_LFXO:
      frequency =
          SystemLFXOClockGet() / (1 << clkDiv); /* freq=32768/2^clkDiv */
      break;
