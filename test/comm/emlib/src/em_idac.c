...
void IDAC_Reset(IDAC_TypeDef *idac)
{
  ...
  idac->CURPROG =
      IDAC_CURPROG_RANGESEL_RANGE0 | (0x0 << _IDAC_CURPROG_STEPSEL_SHIFT);
...
void IDAC_RangeSet(IDAC_TypeDef *idac, const IDAC_Range_TypeDef range)
{
  ...
  tmp = idac->CURPROG
        & ~(_IDAC_CURPROG_TUNING_MASK | _IDAC_CURPROG_RANGESEL_MASK);
...
void IDAC_StepSet(IDAC_TypeDef *idac, const uint32_t step)
{
  ...
  EFM_ASSERT(IDAC_REF_VALID(idac));
  EFM_ASSERT(step
             <= (_IDAC_CURPROG_STEPSEL_MASK >> _IDAC_CURPROG_STEPSEL_SHIFT));
