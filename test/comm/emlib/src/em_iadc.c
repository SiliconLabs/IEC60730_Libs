...
#define IADC_SCANENTRIES(iadc) ((iadc) == IADC0 ? IADC0_SCANENTRIES : 0UL)
#define IADC_CONFIGNUM(iadc)   ((iadc) == IADC0 ? IADC0_CONFIGNUM : 0UL)
#define IADC_FIFOENTRIES(iadc) ((iadc) == IADC0 ? IADC0_FIFOENTRIES : 0UL)
#define IADC_CMU_CLOCK(iadc)   ((iadc) == IADC0 ? cmuClock_IADC0 : cmuClock_IADC0)
               ...
               const IADC_AllConfigs_t *allConfigs)
{
  ...
  iadc->CMPTHR =
      (((uint32_t) (init->greaterThanEqualThres) << _IADC_CMPTHR_ADGT_SHIFT)
  ...
  for (config = 0; config < IADC_CONFIGNUM(iadc); config++) {
    ...
    switch (adcMode) {
      ...
      case iadcCfgModeHighSpeed:
        ...
        if ((uiAnaGain == _IADC_CFG_ANALOGGAIN_ANAGAIN0P5)
          ...
          if (adcMode == iadcCfgModeNormal) {
            offset =
                (int16_t) (DEVINFO->IADC0NORMALOFFSETCAL0
                           >> _DEVINFO_IADC0NORMALOFFSETCAL0_OFFSETANA2NORM_SHIFT);
          } else {
            offset =
                (int16_t) (DEVINFO->IADC0HISPDOFFSETCAL0
        ...
        if (iOsr == 2) {
          ...
          if (adcMode == iadcCfgModeNormal) {
            offset =
                (int16_t) (DEVINFO->IADC0NORMALOFFSETCAL1
                           & _DEVINFO_IADC0NORMALOFFSETCAL1_OFFSETANA3NORM_MASK)
                - offset;
          } else {
            offset +=
                (int16_t) (DEVINFO->IADC0HISPDOFFSETCAL1
      ...
      case iadcCfgModeHighAccuracy:
        ...
        offsetAna1HiAccInt =
            (uint16_t) (DEVINFO->IADC0OFFSETCAL0
        ...
        offsetAnaBase =
            (int16_t) (DEVINFO->IADC0OFFSETCAL0
                       & _DEVINFO_IADC0OFFSETCAL0_OFFSETANABASE_MASK)
        ...
        offset2 = 262144.0f / osrHiAcc[osrValue] / (osrHiAcc[osrValue] + 1)
        ...
        IADC0->CFG[config].SCALE |=
            (uint32_t) (offsetLong & _IADC_SCALE_OFFSET_MASK);
        break;
#endif
      default:
        // Mode not supported.
                   ...
                   const IADC_ScanTable_t *scanTable)
{
  ...
  iadc->TRIGGER =
      (iadc->TRIGGER
       & ~(_IADC_TRIGGER_SCANTRIGSEL_MASK | _IADC_TRIGGER_SCANTRIGACTION_MASK))
  ...
  for (i = 0; i < IADC_SCANENTRIES(iadc); i++) {
    iadc->SCANTABLE[i].SCAN =
        (((uint32_t) (scanTable->entries[i].negInput)
...
void IADC_setScanMask(IADC_TypeDef *iadc, uint32_t mask)
{
  ...
  iadc->MASKREQ =
      (mask << _IADC_MASKREQ_MASKREQ_SHIFT) & _IADC_MASKREQ_MASKREQ_MASK;
                          ...
                          IADC_ScanTableEntry_t *entry)
{
  ...
  iadc->SCANTABLE[id].SCAN =
      (((uint32_t) (entry->negInput) << _IADC_SCAN_PINNEG_SHIFT)
       & (_IADC_SCAN_PORTNEG_MASK | _IADC_SCAN_PINNEG_MASK))
...
{
  uint32_t alignment =
      (iadc->SINGLEFIFOCFG & _IADC_SINGLEFIFOCFG_ALIGNMENT_MASK)
