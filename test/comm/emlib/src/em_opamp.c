    ...
    const OPAMP_Init_TypeDef *init)
{
  ...
  EFM_ASSERT(DAC_REF_VALID(dac));
  EFM_ASSERT(DAC_OPA_VALID(opa));
  EFM_ASSERT(init->bias
             <= (_DAC_BIASPROG_BIASPROG_MASK >> _DAC_BIASPROG_BIASPROG_SHIFT));

  if (opa == OPA0) {
    EFM_ASSERT((init->outPen & ~_DAC_OPA0MUX_OUTPEN_MASK) == 0);

    dac->BIASPROG =
        (dac->BIASPROG & ~(_DAC_BIASPROG_BIASPROG_MASK | DAC_BIASPROG_HALFBIAS))
    ...
    if (init->defaultOffset) {
      ...
      EFM_ASSERT(init->offset
                 <= (_DAC_CAL_CH0OFFSET_MASK >> _DAC_CAL_CH0OFFSET_SHIFT));
    ...
    dac->OPA0MUX = (uint32_t) init->resSel | (uint32_t) init->outMode
                   | init->outPen | (uint32_t) init->resInMux
                   | (uint32_t) init->negSel | (uint32_t) init->posSel
    ...
    dac->OPACTRL =
        ...
        | (init->hcmDisable ? DAC_OPACTRL_OPA0HCMDIS : 0) | DAC_OPACTRL_OPA0EN;
  } else if (opa == OPA1) {
    ...
    if (init->defaultOffset) {
      ...
      EFM_ASSERT(init->offset
                 <= (_DAC_CAL_CH1OFFSET_MASK >> _DAC_CAL_CH1OFFSET_SHIFT));
    ...
    dac->OPACTRL =
        ...
        | (init->hcmDisable ? DAC_OPACTRL_OPA1HCMDIS : 0) | DAC_OPACTRL_OPA1EN;
  } else { /* OPA2 */
    ...
    dac->OPACTRL =
        ...
        | (init->hcmDisable ? DAC_OPACTRL_OPA2HCMDIS : 0) | DAC_OPACTRL_OPA2EN;
  }
  ...
  if (!init->defaultOffsetN) {
    EFM_ASSERT(init->offsetN
               <= (_VDAC_OPA_CAL_OFFSETN_MASK >> _VDAC_OPA_CAL_OFFSETN_SHIFT));
    calData = (calData & ~_VDAC_OPA_CAL_OFFSETN_MASK)
              | (init->offsetN << _VDAC_OPA_CAL_OFFSETN_SHIFT);
  }
  if (!init->defaultOffsetP) {
    EFM_ASSERT(init->offset
               <= (_VDAC_OPA_CAL_OFFSETP_MASK >> _VDAC_OPA_CAL_OFFSETP_SHIFT));
    calData = (calData & ~_VDAC_OPA_CAL_OFFSETP_MASK)
              | (init->offset << _VDAC_OPA_CAL_OFFSETP_SHIFT);
  }
  ...
  dac->OPA[opa].MUX = (uint32_t) init->resSel
                      ...
                      | (uint32_t) init->resInMux | (uint32_t) init->negSel
                      | (uint32_t) init->posSel;

  dac->OPA[opa].OUT = (uint32_t) init->outMode | (uint32_t) init->outPen;
  ...
  dac->OPA[opa].TIMER =
      (uint32_t) (init->settleTime << _VDAC_OPA_TIMER_SETTLETIME_SHIFT)
      | (uint32_t) (warmupTime << _VDAC_OPA_TIMER_WARMUPTIME_SHIFT)
      | (uint32_t) (init->startupDly << _VDAC_OPA_TIMER_STARTUPDLY_SHIFT);

  dac->OPA[opa].CTRL =
      (init->aportYMasterDisable ? VDAC_OPA_CTRL_APORTYMASTERDIS : 0)
      | (init->aportXMasterDisable ? VDAC_OPA_CTRL_APORTXMASTERDIS : 0)
      | (uint32_t) init->prsOutSel | (uint32_t) init->prsSel
      | (uint32_t) init->prsMode | (init->prsEn ? VDAC_OPA_CTRL_PRSEN : 0)
      | (init->halfDrvStr ? VDAC_OPA_CTRL_OUTSCALE_HALF
                          : VDAC_OPA_CTRL_OUTSCALE_FULL)
      | (init->hcmDisable ? VDAC_OPA_CTRL_HCMDIS : 0)
      | (init->ugBwScale ? VDAC_OPA_CTRL_INCBW : 0) | (uint32_t) init->drvStr;
