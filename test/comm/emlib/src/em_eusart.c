                       ...
                       const EUSART_UartInit_TypeDef *init)
{
  ...
  EFM_ASSERT(EUSART_REF_VALID(eusart)
             && EUSART_EM2_CAPABLE(EUSART_NUM(eusart)));
  // Init structure must be provided.
  EFM_ASSERT(init);
...
#endif
    EFM_ASSERT((clock_source == cmuSelect_LFRCO)
  ...
  EFM_ASSERT((init->stopbits == eusartStopbits1)
                     ...
                     const EUSART_DaliInit_TypeDef *daliInit)
{
  ...
  if (daliInit->init.loopbackEnable) {
    ...
    EFM_ASSERT(
        (daliInit->TXdatabits >> _EUSART_DALICFG_DALITXDATABITS_SHIFT)
  ...
  if (daliInit->daliLowFrequencyEnable) {
    ...
    EFM_ASSERT(daliInit->init.baudrate >= 1200
...
{
  while (!(eusart->STATUS & EUSART_STATUS_RXFL)) {} // Wait for incoming data.
                        ...
                        uint32_t baudrate)
{
  ...
  if (eusart->CFG0 & _EUSART_CFG0_SYNC_MASK) {
    ...
    eusart->CFG2 =
        ...
        | ((clkdiv << _EUSART_CFG2_SDIV_SHIFT) & _EUSART_CFG2_SDIV_MASK);
...
#if defined(EUART_PRESENT)
  PRS->CONSUMER_EUART0_TRIGGER =
      (init->prs_trigger_channel & _PRS_CONSUMER_EUART0_TRIGGER_MASK);
#else
#if defined(EUSART0)
  if (eusart == EUSART0) {
    PRS->CONSUMER_EUSART0_TRIGGER =
        (init->prs_trigger_channel & _PRS_CONSUMER_EUSART0_TRIGGER_MASK);
  }
#endif
#if defined(EUSART1)
  if (eusart == EUSART1) {
    PRS->CONSUMER_EUSART1_TRIGGER =
        (init->prs_trigger_channel & _PRS_CONSUMER_EUSART1_TRIGGER_MASK);
  }
#endif
#if defined(EUSART2)
  if (eusart == EUSART2) {
    PRS->CONSUMER_EUSART2_TRIGGER =
        (init->prs_trigger_channel & _PRS_CONSUMER_EUSART2_TRIGGER_MASK);
  }
#endif
                                   ...
                                   const EUSART_DaliInit_TypeDef *daliInit)
{
...
#if defined(EUART_PRESENT)
      PRS->CONSUMER_EUART0_RX_SET =
          (init->advancedSettings->prsRxChannel & _PRS_CONSUMER_EUART0_RX_MASK);
#elif defined(EUSART_PRESENT)
      if (eusart == EUSART0) {
        PRS->CONSUMER_EUSART0_RX_SET = (init->advancedSettings->prsRxChannel
                                        & _PRS_CONSUMER_EUSART0_RX_MASK);
      }
#if defined(EUSART1)
      if (eusart == EUSART1) {}
      PRS->CONSUMER_EUSART1_RX_SET = (init->advancedSettings->prsRxChannel
                                      & _PRS_CONSUMER_EUSART1_RX_MASK);
#endif
#if defined(EUSART2)
      if (eusart == EUSART2) {
        PRS->CONSUMER_EUSART2_RX_SET = (init->advancedSettings->prsRxChannel
                                        & _PRS_CONSUMER_EUSART2_RX_MASK);
      }
#endif
#endif
    }
  ...
  if (irdaInit) {
    if (irdaInit->irDALowFrequencyEnable) {
      ...
      eusart->IRHFCFG_SET =
          (eusart->IRHFCFG
           & ~(_EUSART_IRHFCFG_IRHFEN_MASK | _EUSART_IRHFCFG_IRHFEN_MASK
...
{
  void *advanceSetting_ptr =
      (void *) init->advancedSettings; // Used to avoid GCC over optimization.
  ...
  if (advanceSetting_ptr) {
    // Configure global configuration register 2.
    eusart->CFG2 = (eusart->CFG2
                    & ~(_EUSART_CFG2_AUTOCS_MASK | _EUSART_CFG2_AUTOTX_MASK
                        | _EUSART_CFG2_CSINV_MASK | _EUSART_CFG2_CLKPRSEN_MASK))
  ...
  eusart->CFG0 =
      (eusart->CFG0 & ~(_EUSART_CFG0_SYNC_MASK | _EUSART_CFG0_LOOPBK_MASK))
      | (uint32_t) (_EUSART_CFG0_SYNC_SYNC) | (uint32_t) (init->loopbackEnable);
  ...
  if (advanceSetting_ptr) {
    ...
    eusart->TIMINGCFG =
        (eusart->TIMINGCFG
         & ~(_EUSART_TIMINGCFG_CSSETUP_MASK | _EUSART_TIMINGCFG_CSHOLD_MASK
  ...
  if (advanceSetting_ptr) {
    eusart->DTXDATCFG =
        (init->advancedSettings->defaultTxData & _EUSART_DTXDATCFG_MASK);
    ...
    if (init->advancedSettings->prsClockEnable) {
      ...
      if (eusart == EUSART0) {
        PRS->CONSUMER_EUSART0_CLK_SET = (init->advancedSettings->prsClockChannel
                                         & _PRS_CONSUMER_EUSART0_CLK_MASK);
      }
#if defined(EUSART1)
      if (eusart == EUSART1) {
        PRS->CONSUMER_EUSART1_CLK_SET = (init->advancedSettings->prsClockChannel
                                         & _PRS_CONSUMER_EUSART1_CLK_MASK);
      }
#endif
#if defined(EUSART2)
      if (eusart == EUSART2) {
        PRS->CONSUMER_EUSART2_CLK_SET = (init->advancedSettings->prsClockChannel
                                         & _PRS_CONSUMER_EUSART2_CLK_MASK);
      }
#endif
    }
  }
  ...
  eusart_sync(
      eusart,
      _EUSART_SYNCBUSY_RXEN_MASK
          & _EUSART_SYNCBUSY_TXEN_MASK); // Wait for low frequency register synchronization.
