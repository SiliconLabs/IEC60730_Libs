...
#elif (I2C_COUNT == 3)
#define I2C_REF_VALID(ref)                                                     \
  (((ref) == I2C0) || ((ref) == I2C1) || ((ref) == I2C2))
#endif
...
uint32_t I2C_BusFreqGet(I2C_TypeDef *i2c)
{
  ...
  /* n = Nlow + Nhigh */
  n = (uint32_t)
      i2cNSum[(i2c->CTRL & _I2C_CTRL_CLHR_MASK) >> _I2C_CTRL_CLHR_SHIFT];
    ...
    switch (transfer->state) {
      ...
      case i2cStateWFData:
        if (pending & I2C_IF_RXDATAV) {
          ...
          if (((status & I2C_IF_RXDATAV) == 0)
              & ((status & I2C_IF_RXFULL) != 0)) {
