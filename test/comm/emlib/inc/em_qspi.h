...
typedef struct {
  ...
  /**
   * @brief
   *   Enable/disable automatic issuing of WELL (Write Enable Latch)
   *   command before a write operation.
   *
   * @details
   *   When writing to a flash device, the WELL within the flash device must be
   *   high before a write sequence can be issued. The QSPI peripheral can
   *   automatically issue the WELL command before triggering a write sequence.
   *   The command used for enabling the WELL is WREN (0x06) and is common between
   *   devices. */
  bool autoWEL;
...
#define QSPI_WRITECONFIG_DEFAULT                                               \
  {                                                                            \
      ...
      qspiTransferSingle, /* Single I/O mode. */                               \
      qspiTransferSingle, /* Single I/O mode. */                               \
      true,               /* Send WELL command automatically. */               \
  }
