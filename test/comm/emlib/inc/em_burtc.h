...
typedef struct {
  ...
  uint32_t
      lowPowerComp; /**< Number of least significantt clock bits to ignore in low power mode. */
  bool timeStamp; /**< Enable time stamp on entering backup power domain. */

  bool
      compare0Top; /**< Set if Compare Value 0 is also top value (counter restart). */

  BURTC_LP_TypeDef
      lowPowerMode; /**< Low power operation mode, requires LFXO or LFRCO. */
} BURTC_Init_TypeDef;
