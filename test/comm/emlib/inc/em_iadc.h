...
typedef enum {
  ...
  iadcNegInputNegRef =
      (_IADC_SCAN_PORTNEG_PADREFNEG
       << (_IADC_SCAN_PORTNEG_SHIFT - _IADC_SCAN_PINNEG_SHIFT)),
#endif

  /** GPIO port A pin 0 */
  iadcNegInputPortAPin0 =
      (_IADC_SCAN_PORTNEG_PORTA
       << (_IADC_SCAN_PORTNEG_SHIFT - _IADC_SCAN_PINNEG_SHIFT)),
...
typedef struct {
  ...
  uint8_t
      srcClkPrescale; /**< User requested source clock divider (prescale+1) which
                                                          will be used if the calculated prescaler value is less. */
  uint16_t timerCycles; /**< Number of ADC_CLK cycles per TIMER event. */
  uint16_t
      greaterThanEqualThres; /**< Digital window comparator greater-than or equal threshold. */
  uint16_t
      lessThanEqualThres; /**< Digital window comparator less-than or equal threshold. */
} IADC_Init_t;
...
typedef struct {
  IADC_CfgAdcMode_t
      adcMode; /**< IADC mode; Normal, High speed or High Accuracy. */
  IADC_CfgOsrHighSpeed_t
      osrHighSpeed; /**< Over sampling ratio for High Speed and Normal modes. */
#if defined(_IADC_CFG_ADCMODE_HIGHACCURACY)
  IADC_CfgOsrHighAccuracy_t
      osrHighAccuracy; /**< Over sampling ratio for High Accuracy mode. */
#endif
...
#define IADC_ALLCONFIGS_DEFAULT                                                \
  {                                                                            \
    {                                                                          \
      IADC_CONFIG_DEFAULT, IADC_CONFIG_DEFAULT                                 \
    }                                                                          \
  }
...
typedef struct {
  ...
  IADC_FifoCfgDvl_t
      dataValidLevel; /**< Data valid level before requesting DMA transfer. */
...
typedef struct {
  IADC_NegInput_t
      negInput; /**< Port/pin input for the negative side of the ADC.  */
  IADC_PosInput_t
      posInput;     /**< Port/pin input for the positive side of the ADC.  */
  uint8_t configId; /**< Configuration id. */
  bool
      compare; /**< Perform digital window comparison on the result from this entry. */
} IADC_SingleInput_t;
...
#define IADC_SCANTABLE_DEFAULT                                                 \
  {                                                                            \
    {                                                                          \
      IADC_SCANTABLEENTRY_DEFAULT, IADC_SCANTABLEENTRY_DEFAULT,                \
          IADC_SCANTABLEENTRY_DEFAULT, IADC_SCANTABLEENTRY_DEFAULT,            \
          IADC_SCANTABLEENTRY_DEFAULT, IADC_SCANTABLEENTRY_DEFAULT,            \
          IADC_SCANTABLEENTRY_DEFAULT, IADC_SCANTABLEENTRY_DEFAULT,            \
          IADC_SCANTABLEENTRY_DEFAULT, IADC_SCANTABLEENTRY_DEFAULT,            \
...
typedef struct {
  ...
  uint8_t
      id; /**< Id of FIFO entry; Scan table entry id or single indicator (0x20). */
} IADC_Result_t;
...
{
  return (
      uint8_t) ((iadc->SINGLEFIFOSTAT & _IADC_SINGLEFIFOSTAT_FIFOREADCNT_MASK)
