...
typedef enum {
  ...
  mscReturnInvalidAddr =
      -1, /**< Invalid address. Write to an address that is not Flash. */
...
typedef enum {
  mscBusStrategyCPU =
      MSC_READCTRL_BUSSTRATEGY_CPU, /**< Prioritize CPU bus accesses. */
  mscBusStrategyDMA =
      MSC_READCTRL_BUSSTRATEGY_DMA, /**< Prioritize DMA bus accesses. */
  mscBusStrategyDMAEM1 =
      MSC_READCTRL_BUSSTRATEGY_DMAEM1, /**< Prioritize DMAEM1 for bus accesses. */
  mscBusStrategyNone =
      MSC_READCTRL_BUSSTRATEGY_NONE /**< No unit has bus priority. */
...
typedef struct {
  bool doutBufEn; /**< Flash doubt pipeline buffer enable */
...
typedef struct {
  ...
  bool
      iccDis; /**< Disable automatic caching of fetches in interrupt context. */
  bool
      useHprot; /**< Use ahb_hprot to determine if the instruction is cacheable or not. */
} MSC_ExecConfig_TypeDef;
...
MSC_RAMFUNC_DECLARATOR MSC_Status_TypeDef MSC_WriteWord(uint32_t *address,
                                                        void const *data,
                                                        uint32_t numBytes);
...
MSC_RAMFUNC_DECLARATOR MSC_Status_TypeDef MSC_WriteWordFast(uint32_t *address,
                                                            void const *data,
                                                            uint32_t numBytes);
#endif
