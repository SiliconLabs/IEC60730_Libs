...
#if defined(CMU_STATUS_AUXHFRCOENS)                                            \
    && (CMU_STATUS_AUXHFRCOENS != CMU_OSCENCMD_AUXHFRCOEN)
...
#endif
#if defined(CMU_STATUS_LFRCOENS)                                               \
    && (CMU_STATUS_LFRCOENS != CMU_OSCENCMD_LFRCOEN)
...
#define HFXO_STATUS_READY_FLAGS                                                \
  (CMU_STATUS_HFXOPEAKDETRDY | CMU_STATUS_HFXOSHUNTOPTRDY)
...
#define _GENERIC_DCDCMISCCTRL_LPCMPBIASEM234H_MASK                             \
  _EMU_DCDCMISCCTRL_LPCMPBIAS_MASK
#define _GENERIC_DCDCMISCCTRL_LPCMPBIASEM234H_SHIFT                            \
...
#define _GENERIC_DCDCLPCTRL_LPCMPHYSSELEM234H_MASK                             \
...
static void __attribute__((noinline)) ramWFI(void)
{
  ...
  if (CoreDebug->DHCSR & CoreDebug_DHCSR_C_DEBUGEN_Msk) {
    for (volatile int i = 0; i < 6; i++) {} // Dummy wait loop ...
  }
...
static void emState(emState_TypeDef action)
{
  ...
  if (action == emState_Save) {
    ...
    oscEnCmd = 0;
    oscEnCmd |=
        (cmuStatus & CMU_STATUS_HFRCOENS) != 0U ? CMU_OSCENCMD_HFRCOEN : 0U;
    oscEnCmd |= (cmuStatus & CMU_STATUS_AUXHFRCOENS) != 0U
                    ? CMU_OSCENCMD_AUXHFRCOEN
                    : 0U;
    oscEnCmd |=
        (cmuStatus & CMU_STATUS_LFRCOENS) != 0U ? CMU_OSCENCMD_LFRCOEN : 0U;
    oscEnCmd |=
        (cmuStatus & CMU_STATUS_HFXOENS) != 0U ? CMU_OSCENCMD_HFXOEN : 0U;
    oscEnCmd |=
        (cmuStatus & CMU_STATUS_LFXOENS) != 0U ? CMU_OSCENCMD_LFXOEN : 0U;
#if defined(_CMU_STATUS_USHFRCOENS_MASK)
    oscEnCmd |=
        (cmuStatus & CMU_STATUS_USHFRCOENS) != 0U ? CMU_OSCENCMD_USHFRCOEN : 0U;
#endif
...
{
#if defined(_EFM32_HAPPY_FAMILY) || defined(_EFM32_TINY_FAMILY)                \
    || defined(_EFM32_WONDER_FAMILY) || defined(_EFM32_ZERO_FAMILY)
...
static void vScaleDownEM23Setup(void)
{
  ...
  uint32_t em23vs =
      (EMU->CTRL & _EMU_CTRL_EM23VSCALE_MASK) >> _EMU_CTRL_EM23VSCALE_SHIFT;
  uint32_t em01vs =
      (EMU->STATUS & _EMU_STATUS_VSCALE_MASK) >> _EMU_STATUS_VSCALE_SHIFT;
  ...
  if (em23vs > em01vs) {
    ...
    BUS_RegMaskedSet(
        &MSC->RAMCTRL,
        (MSC_RAMCTRL_RAMWSEN | MSC_RAMCTRL_RAM1WSEN | MSC_RAMCTRL_RAM2WSEN));
#endif
  } else {
    EMU->CTRL &= ~EMU_CTRL_EM23VSCALEAUTOWSEN;
  }
...
void EMU_EM4Init(const EMU_EM4Init_TypeDef *em4Init)
{
  ...
  em4ctrl |=
      (em4Init->retainLfxo ? EMU_EM4CTRL_RETAINLFXO : 0U)
      | (em4Init->retainLfrco ? EMU_EM4CTRL_RETAINLFRCO : 0U)
      | (em4Init->retainUlfrco ? EMU_EM4CTRL_RETAINULFRCO : 0U)
      | (em4Init->em4State == emuEM4Hibernate ? EMU_EM4CTRL_EM4STATE_EM4H : 0U)
...
void EMU_EnterEM4(void)
{
...
#else
  uint32_t em4seq2 =
      (EMU->CTRL & ~_EMU_CTRL_EM4CTRL_MASK) | (2U << _EMU_CTRL_EM4CTRL_SHIFT);
  uint32_t em4seq3 =
      (EMU->CTRL & ~_EMU_CTRL_EM4CTRL_MASK) | (3U << _EMU_CTRL_EM4CTRL_SHIFT);
#endif
  ...
  if ((EMU->EM4CTRL & _EMU_EM4CTRL_EM4STATE_MASK)
...
void EMU_RamPowerDown(uint32_t start, uint32_t end)
{
  ...
  if (end > RAM1_MEM_END) {
    for (unsigned i = 0; i < RAM1_BLOCKS; i++) {
      mask |= ADDRESS_NOT_IN_BLOCK(start, RAM1_MEM_BASE + (i * RAM1_BLOCK_SIZE))
              << i;
    }
  }
  EMU->RAM1CTRL |= mask;
  ...
  if (end > RAM2_MEM_END) {
    for (unsigned i = 0; i < RAM2_BLOCKS; i++) {
      mask |= ADDRESS_NOT_IN_BLOCK(start, RAM2_MEM_BASE + (i * RAM2_BLOCK_SIZE))
              << i;
    }
  }
  EMU->RAM2CTRL |= mask;
...
{
  EFM_ASSERT(!enable);
  EMU->EM23PERNORETAINCTRL =
      (uint32_t) periMask & (uint32_t) emuPeripheralRetention_ALL;
}
#endif
...
void EMU_VScaleEM01(EMU_VScaleEM01_TypeDef voltage, bool wait)
{
  ...
  if (voltage == emuVScaleEM01_LowPower) {
    EFM_ASSERT(hfSrcClockFrequency
  ...
  uint32_t hfPresc =
      1U
      + ((CMU->HFPRESC & _CMU_HFPRESC_PRESC_MASK) >> _CMU_HFPRESC_PRESC_SHIFT);
...
static bool dcdcConstCalibrationLoad(void)
{
  ...
  volatile uint32_t *const diCal_EMU_DCDCLNFREQCTRL =
      (volatile uint32_t *) (0x0FE08038);
  volatile uint32_t *const diCal_EMU_DCDCLNVCTRL =
      (volatile uint32_t *) (0x0FE08040);
  volatile uint32_t *const diCal_EMU_DCDCLPCTRL =
      (volatile uint32_t *) (0x0FE08048);
  volatile uint32_t *const diCal_EMU_DCDCLPVCTRL =
      (volatile uint32_t *) (0x0FE08050);
  volatile uint32_t *const diCal_EMU_DCDCTRIM0 =
      (volatile uint32_t *) (0x0FE08058);
  volatile uint32_t *const diCal_EMU_DCDCTRIM1 =
...
static void dcdcValidatedConfigSet(void)
{
   ...
   */
  lnForceCcm =
      BUS_RegBitRead(&EMU->DCDCMISCCTRL, _EMU_DCDCMISCCTRL_LNFORCECCM_SHIFT);
  if (lnForceCcm != 0U) {
  ...
  EMU->DCDCMISCCTRL |=
      EMU_DCDCMISCCTRL_LPCMPHYSDIS | EMU_DCDCMISCCTRL_LPCMPHYSHI;
  ...
  if ((rev.major == 1) && (rev.minor < 3)
    ...
    dcdcTiming &=
        ~(_EMU_DCDCTIMING_LPINITWAIT_MASK | _EMU_DCDCTIMING_LNWAIT_MASK
...
static void currentLimitersUpdate(void)
{
  ...
  const uint32_t lpcLim =
      (((14 + 40) + ((14 + 40) / 2)) / (5 * (DCDC_LP_PFET_CNT + 1))) - 1;
  ...
  if (dcdcReverseCurrentControl >= 0) {
    ...
    EMU->DCDCZDETCTRL =
        (EMU->DCDCZDETCTRL & ~_EMU_DCDCZDETCTRL_ZDETILIMSEL_MASK) | zdetLimSel;
  }
}
 ...
 ******************************************************************************/
static void userCurrentLimitsSet(
                                     ...
                                     dcdcTrimMode_TypeDef trimMode)
{
  ...
  if (devinfoRev < 4) {
    ...
    if (lpAttenuation) {
      lpcmpHystSel =
          (lpcmpHystSel & _DEVINFO_DCDCLPCMPHYSSEL0_LPCMPHYSSELLPATT1_MASK)
          >> _DEVINFO_DCDCLPCMPHYSSEL0_LPCMPHYSSELLPATT1_SHIFT;
    } else {
      lpcmpHystSel =
          (lpcmpHystSel & _DEVINFO_DCDCLPCMPHYSSEL0_LPCMPHYSSELLPATT0_MASK)
  ...
  if (trimMode == dcdcTrimMode_EM234H_LP) {
    ...
    EMU->DCDCLPCTRL =
        (EMU->DCDCLPCTRL & ~_GENERIC_DCDCLPCTRL_LPCMPHYSSELEM234H_MASK)
        | lpcmpHystSel;
  }
  ...
  if (trimMode == dcdcTrimMode_EM01_LP) {
    ...
    EMU->DCDCLPEM01CFG =
        (EMU->DCDCLPEM01CFG & ~_EMU_DCDCLPEM01CFG_LPCMPHYSSELEM01_MASK)
        | lpcmpHystSel;
  }
#endif
...
bool EMU_DCDCInit(const EMU_DCDCInit_TypeDef *dcdcInit)
{
  ...
  EMU->DCDCMISCCTRL =
      (EMU->DCDCMISCCTRL
       & ~(_GENERIC_DCDCMISCCTRL_LPCMPBIASEM234H_MASK
           | _EMU_DCDCMISCCTRL_LNFORCECCM_MASK))
      | ((uint32_t) lpCmpBiasSelEM234H
         | (dcdcInit->reverseCurrentControl >= 0 ? EMU_DCDCMISCCTRL_LNFORCECCM
                                                 : 0U));
#if defined(_EMU_DCDCLPEM01CFG_LPCMPBIASEM01_MASK)
  ...
  BUS_RegBitWrite(
      &EMU->PWRCTRL,
      _EMU_PWRCTRL_ANASW_SHIFT,
      dcdcInit->anaPeripheralPower == emuDcdcAnaPeripheralPower_DCDC ? 1U : 0U);
#endif
...
void EMU_DCDCModeSet(EMU_DcdcMode_TypeDef dcdcMode)
{
  ...
  if (((currentDcdcMode == EMU_DCDCCTRL_DCDCMODE_OFF)
       || (currentDcdcMode == EMU_DCDCCTRL_DCDCMODE_BYPASS))
  ...
  EMU->DCDCCTRL =
      (EMU->DCDCCTRL & ~_EMU_DCDCCTRL_DCDCMODE_MASK) | (uint32_t) dcdcMode;
...
void EMU_DCDCConductionModeSet(EMU_DcdcConductionMode_TypeDef conductionMode,
                               bool rcoDefaultSet)
{
  EMU_DcdcMode_TypeDef currentDcdcMode = (EMU_DcdcMode_TypeDef) ((
      uint32_t) (EMU->DCDCCTRL & _EMU_DCDCCTRL_DCDCMODE_MASK));
  EMU_DcdcLnRcoBand_TypeDef rcoBand    = (EMU_DcdcLnRcoBand_TypeDef) ((
      uint32_t) ((EMU->DCDCLNFREQCTRL & _EMU_DCDCLNFREQCTRL_RCOBAND_MASK)
...
bool EMU_DCDCOutputVoltageSet(uint32_t mV, bool setLpVoltage, bool setLnVoltage)
{
  ...
  validOutVoltage =
      (mV >= PWRCFG_DCDCTODVDD_VMIN) && (mV <= PWRCFG_DCDCTODVDD_VMAX);
  ...
  if (attenuationSet) {
    ...
    vrefHigh[dcdcTrimMode_LN] =
        (vrefLow[dcdcTrimMode_LN] & _DEVINFO_DCDCLNVCTRL0_3V0LNATT1_MASK)
        >> _DEVINFO_DCDCLNVCTRL0_3V0LNATT1_SHIFT;
    vrefLow[dcdcTrimMode_LN] =
        (vrefLow[dcdcTrimMode_LN] & _DEVINFO_DCDCLNVCTRL0_1V8LNATT1_MASK)
        >> _DEVINFO_DCDCLNVCTRL0_1V8LNATT1_SHIFT;
  } else {
    vrefLow[dcdcTrimMode_LN] = DEVINFO->DCDCLNVCTRL0;
    vrefHigh[dcdcTrimMode_LN] =
        (vrefLow[dcdcTrimMode_LN] & _DEVINFO_DCDCLNVCTRL0_1V8LNATT0_MASK)
        >> _DEVINFO_DCDCLNVCTRL0_1V8LNATT0_SHIFT;
    vrefLow[dcdcTrimMode_LN] =
        (vrefLow[dcdcTrimMode_LN] & _DEVINFO_DCDCLNVCTRL0_1V2LNATT0_MASK)
  ...
  lpcmpBias[dcdcTrimMode_EM01_LP] =
      (uint8_t) ((EMU->DCDCLPEM01CFG & _EMU_DCDCLPEM01CFG_LPCMPBIASEM01_MASK)
  ...
  vrefVal[dcdcTrimMode_LN] =
      ((mV - mVlow) * (vrefHigh[dcdcTrimMode_LN] - vrefLow[dcdcTrimMode_LN]))
      / mVdiff;
  vrefVal[dcdcTrimMode_LN] += vrefLow[dcdcTrimMode_LN];

  vrefVal[dcdcTrimMode_EM234H_LP] =
      ((mV - mVlow)
       * (vrefHigh[dcdcTrimMode_EM234H_LP] - vrefLow[dcdcTrimMode_EM234H_LP]))
      / mVdiff;
  vrefVal[dcdcTrimMode_EM234H_LP] += vrefLow[dcdcTrimMode_EM234H_LP];

#if defined(_EMU_DCDCLPEM01CFG_LPCMPBIASEM01_MASK)
  vrefVal[dcdcTrimMode_EM01_LP] =
      ((mV - mVlow)
       * (vrefHigh[dcdcTrimMode_EM01_LP] - vrefLow[dcdcTrimMode_EM01_LP]))
      / mVdiff;
  ...
  if (setLnVoltage) {
    EMU->DCDCLNVCTRL =
        (EMU->DCDCLNVCTRL
         & ~(_EMU_DCDCLNVCTRL_LNVREF_MASK | _EMU_DCDCLNVCTRL_LNATT_MASK))
  ...
  if (setLpVoltage) {
    ...
    if (!(lpCmpHystCalibrationLoad(attenuationSet,
                                   lpcmpBias[dcdcTrimMode_EM234H_LP],
                                   dcdcTrimMode_EM234H_LP))) {
      EFM_ASSERT(false);
      /* Return when assertions are disabled. */
      return false;
    }
    ...
    EMU->DCDCLPVCTRL =
        (EMU->DCDCLPVCTRL
         & ~(_EMU_DCDCLPVCTRL_LPVREF_MASK | _EMU_DCDCLPVCTRL_LPATT_MASK))
...
bool EMU_DCDCBoostInit(const EMU_DCDCBoostInit_TypeDef *dcdcBoostInit)
{
  ...
  DCDC->BSTCTRL =
      (DCDC->BSTCTRL & ~(_DCDC_BSTCTRL_IPKTMAXCTRL_MASK))
      | ((uint32_t) dcdcBoostInit->tonMax << _DCDC_BSTCTRL_IPKTMAXCTRL_SHIFT);
  DCDC->BSTEM01CTRL = ((uint32_t) dcdcBoostInit->driveSpeedEM01
                       << _DCDC_BSTEM01CTRL_DRVSPEED_SHIFT)
                      | ((uint32_t) dcdcBoostInit->peakCurrentEM01
                         << _DCDC_BSTEM01CTRL_IPKVAL_SHIFT);
  DCDC->BSTEM23CTRL = ((uint32_t) dcdcBoostInit->driveSpeedEM23
                       << _DCDC_BSTEM23CTRL_DRVSPEED_SHIFT)
                      | ((uint32_t) dcdcBoostInit->peakCurrentEM23
                         << _DCDC_BSTEM23CTRL_IPKVAL_SHIFT);
...
#endif
    currentDcdcMode =
        (DCDC->CTRL & _DCDC_CTRL_MODE_MASK) >> _DCDC_CTRL_MODE_SHIFT;
...
bool EMU_DCDCInit(const EMU_DCDCInit_TypeDef *dcdcInit)
{
  ...
  EMU->VREGVDDCMPCTRL =
      ((uint32_t) dcdcInit->cmpThreshold << _EMU_VREGVDDCMPCTRL_THRESSEL_SHIFT)
      | EMU_VREGVDDCMPCTRL_VREGINCMPEN;

#if defined(_DCDC_SYNCBUSY_MASK)
#if defined(_DCDC_SYNCBUSY_CTRL_MASK)
  EMU_DCDCSync(DCDC_SYNCBUSY_CTRL | DCDC_SYNCBUSY_EM01CTRL0
               | DCDC_SYNCBUSY_EM23CTRL0);
#else
  EMU_DCDCSync(_DCDC_SYNCBUSY_MASK);
#endif
#endif
  ...
  DCDC->CTRL =
      (DCDC->CTRL & ~(_DCDC_CTRL_IPKTMAXCTRL_MASK | _DCDC_CTRL_DCMONLYEN_MASK))
      | ((uint32_t) dcdcInit->tonMax << _DCDC_CTRL_IPKTMAXCTRL_SHIFT)
      | ((uint32_t) (dcdcInit->dcmOnlyEn ? 1U : 0U)
         << _DCDC_CTRL_DCMONLYEN_SHIFT);
#else
...
void EMU_DCDCSetPFMXModePeakCurrent(uint32_t value)
{
  ...
  EFM_ASSERT(value
             <= (_DCDC_PFMXCTRL_IPKVAL_MASK >> _DCDC_PFMXCTRL_IPKVAL_SHIFT));
                                        ...
                                        int threshold)
{
  ...
  switch (channel) {
    case emuVmonChannel_AVDD:
      ...
      tLow   = (10U
              * ((calReg & _DEVINFO_VMONCAL0_AVDD1V86THRESCOARSE_MASK)
      ...
      tHigh = (10U
               * ((calReg & _DEVINFO_VMONCAL0_AVDD2V98THRESCOARSE_MASK)
    ...
    case emuVmonChannel_ALTAVDD:
      calReg = DEVINFO->VMONCAL0;
      tLow   = (10U
              * ((calReg & _DEVINFO_VMONCAL0_ALTAVDD1V86THRESCOARSE_MASK)
      ...
      tHigh = (10U
               * ((calReg & _DEVINFO_VMONCAL0_ALTAVDD2V98THRESCOARSE_MASK)
    ...
    case emuVmonChannel_DVDD:
      ...
      tLow   = (10U
              * ((calReg & _DEVINFO_VMONCAL1_DVDD1V86THRESCOARSE_MASK)
      ...
      tHigh = (10U
               * ((calReg & _DEVINFO_VMONCAL1_DVDD2V98THRESCOARSE_MASK)
    ...
    case emuVmonChannel_IOVDD0:
      calReg = DEVINFO->VMONCAL1;
      tLow   = (10U
              * ((calReg & _DEVINFO_VMONCAL1_IO01V86THRESCOARSE_MASK)
      ...
      tHigh = (10U
               * ((calReg & _DEVINFO_VMONCAL1_IO02V98THRESCOARSE_MASK)
    ...
    case emuVmonChannel_IOVDD1:
      ...
      tLow   = (10U
              * ((calReg & _DEVINFO_VMONCAL2_IO11V86THRESCOARSE_MASK)
      ...
      tHigh = (10U
               * ((calReg & _DEVINFO_VMONCAL2_IO12V98THRESCOARSE_MASK)
    ...
    case emuVmonChannel_BUVDD:
      calReg = DEVINFO->VMONCAL2;
      tLow   = (10U
              * ((calReg & _DEVINFO_VMONCAL2_BUVDD1V86THRESCOARSE_MASK)
      ...
      tHigh = (10U
               * ((calReg & _DEVINFO_VMONCAL2_BUVDD2V98THRESCOARSE_MASK)
...
void EMU_VmonInit(const EMU_VmonInit_TypeDef *vmonInit)
{
  ...
  switch (vmonInit->channel) {
    case emuVmonChannel_AVDD:
      EMU->VMONAVDDCTRL =
          (thresholdCoarse << _EMU_VMONAVDDCTRL_RISETHRESCOARSE_SHIFT)
    ...
    case emuVmonChannel_ALTAVDD:
      EMU->VMONALTAVDDCTRL =
          (thresholdCoarse << _EMU_VMONALTAVDDCTRL_THRESCOARSE_SHIFT)
    ...
    case emuVmonChannel_DVDD:
      EMU->VMONDVDDCTRL =
          (thresholdCoarse << _EMU_VMONDVDDCTRL_THRESCOARSE_SHIFT)
    ...
    case emuVmonChannel_BUVDD:
      EMU->VMONBUVDDCTRL =
          (thresholdCoarse << _EMU_VMONBUVDDCTRL_THRESCOARSE_SHIFT)
...
void EMU_VmonHystInit(const EMU_VmonHystInit_TypeDef *vmonInit)
{
  ...
  EFM_ASSERT((vmonInit->riseThreshold >= 1620)
             && (vmonInit->riseThreshold <= 3400));
  EFM_ASSERT((vmonInit->fallThreshold >= 1620)
             && (vmonInit->fallThreshold <= 3400));
  /* The fall threshold has to be lower than rise threshold. */
  EFM_ASSERT(vmonInit->fallThreshold <= vmonInit->riseThreshold);

  riseThreshold =
      vmonCalibratedThreshold(vmonInit->channel, vmonInit->riseThreshold);
  fallThreshold =
      vmonCalibratedThreshold(vmonInit->channel, vmonInit->fallThreshold);

  switch (vmonInit->channel) {
    case emuVmonChannel_AVDD:
      EMU->VMONAVDDCTRL =
          ((riseThreshold / 10U) << _EMU_VMONAVDDCTRL_RISETHRESCOARSE_SHIFT)
...
void EMU_SetBiasMode(EMU_BiasMode_TypeDef mode)
{
  ...
  CMU_ULFRCOCTRL = (CMU_ULFRCOCTRL & ~0xC00u) | ((freq & 0x3u) << 10u);
  EMU_BIASTESTCTRL &= ~0x8u;
...
float EMU_TemperatureGet(void)
{
  ...
  do {
    val1 = (EMU->TEMP & _EMU_TEMP_TEMP_MASK) >> _EMU_TEMP_TEMP_SHIFT;
