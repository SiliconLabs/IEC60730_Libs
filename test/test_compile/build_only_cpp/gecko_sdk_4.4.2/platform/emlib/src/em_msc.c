...
#error                                                                         \
    "Running Flash write/erase operations from Flash is not supported on EFM32G."
#endif
...
#define ECC_IFC_MASK                                                           \
  (MSC_IFC_RAMERR1B | MSC_IFC_RAMERR2B | MSC_IFC_RAM1ERR1B | MSC_IFC_RAM1ERR2B)
...
#define ECC_IF_1BIT_ERROR                                                      \
  (MPAHBRAM_IF_AHB0ERR1B | MPAHBRAM_IF_AHB1ERR1B | MPAHBRAM_IF_AHB2ERR1B       \
   | MPAHBRAM_IF_AHB3ERR1B)
...
#define ECC_DMA_MAX_XFERCNT                                                    \
  (_LDMA_CH_CTRL_XFERCNT_MASK >> _LDMA_CH_CTRL_XFERCNT_SHIFT)
...
static const MSC_EccBank_Typedef eccBankTbl[MSC_ECC_BANKS] = {
    {ECC_RAM0_SYNDROMES_INIT,
     ECC_RAM0_CORRECTION_EN,
     ECC_RAM0_MEM_BASE,
     ECC_RAM0_MEM_SIZE},
#if MSC_ECC_BANKS > 1
    {ECC_RAM1_SYNDROMES_INIT,
     ECC_RAM1_CORRECTION_EN,
     ECC_RAM1_MEM_BASE,
     ECC_RAM1_MEM_SIZE},
#if MSC_ECC_BANKS > 2
    {ECC_RAM2_SYNDROMES_INIT,
     ECC_RAM2_CORRECTION_EN,
     ECC_RAM2_MEM_BASE,
     ECC_RAM2_MEM_SIZE},
#endif
#endif
 ...
 ******************************     FUNCTIONS     ******************************
 ******************************************************************************/
MSC_RAMFUNC_DECLARATOR MSC_Status_TypeDef MSC_WriteWordI(uint32_t *address,
                                                         void const *data,
                                                         uint32_t numBytes);

MSC_RAMFUNC_DECLARATOR MSC_Status_TypeDef MSC_LoadWriteData(uint32_t *data,
                                                            uint32_t numWords);
 ...
 * @param[in] numBytes
 *   Number of bytes to write to flash. NB: Must be divisible by four.
 * @return
    ...
    MSC_WriteWord(uint32_t *address, void const *data, uint32_t numBytes)
{
  ...
  while (numBytes) {
    ...
    burstLen =
        SL_MIN(numBytes, ((addr + FLASH_PAGE_SIZE) & FLASH_PAGE_MASK) - addr);
...
MSC_Status_TypeDef MSC_MassErase(void)
{
  ...
  MSC->MISCLOCKWORD_CLR =
      MSC_MISCLOCKWORD_MELOCKBIT;          // Enable Write ctrl access
  MSC->WRITECMD = MSC_WRITECMD_ERASEMAIN0; // Start Mass erase procedure
  retVal        = mscStatusWait(
      MSC_STATUS_BUSY,
      0); // Wait for end of busy flag or a problem (INVADDR, LOCK, REGLOCK, TIMEOUT)
  MSC->MISCLOCKWORD_SET =
      MSC_MISCLOCKWORD_MELOCKBIT;          // Re-enable mass erase lock bit
                                    ...
                                    uint32_t numBytes)
{
  ...
  LDMA->EN_SET = 0x1;
  LDMAXBAR->CH[ch].REQSEL =
      LDMAXBAR_CH_REQSEL_SOURCESEL_MSC | LDMAXBAR_CH_REQSEL_SIGSEL_MSCWDATA;
  ...
  while (numBytes) {
    // Max burst length is up to next flash page boundary
    burstLen =
        SL_MIN(numBytes, ((dst + FLASH_PAGE_SIZE) & FLASH_PAGE_MASK) - dst);
    words = burstLen / 4;
...
void MSC_Init(void)
{
  ...
  EFM_ASSERT((EMU->STATUS & _EMU_STATUS_VSCALE_MASK)
  ...
  if (freq > 1200000) {
    ...
    MSC->TIMEBASE =
        (MSC->TIMEBASE & ~(_MSC_TIMEBASE_BASE_MASK | _MSC_TIMEBASE_PERIOD_MASK))
...
void MSC_ExecConfigSet(MSC_ExecConfig_TypeDef *execConfig)
{
  ...
  } else if ((mscReadCtrl == MSC_READCTRL_MODE_WS0SCBTP)
             && (!execConfig->scbtEn)) {
    mscReadCtrl |= MSC_READCTRL_MODE_WS0;
  } else if ((mscReadCtrl == MSC_READCTRL_MODE_WS1SCBTP)
             && (!execConfig->scbtEn)) {
 ...
 ******************************************************************************/
MSC_RAMFUNC_DEFINITION_BEGIN
MSC_Status_TypeDef MSC_LoadWriteData(uint32_t *data, uint32_t numWords)
{
  uint32_t timeOut;
  uint32_t wordIndex;
                                    ...
                                    uint32_t numBytes)
{
  ...
  if (((uint32_t) data < SRAM_BASE)
      || ((uint32_t) data > (SRAM_BASE + SRAM_SIZE))) {
    EFM_ASSERT(false);
    return mscReturnInvalidAddr;
  }

  LDMA->CH[ch].REQSEL =
      LDMA_CH_REQSEL_SOURCESEL_MSC | LDMA_CH_REQSEL_SIGSEL_MSCWDATA;
  LDMA->CH[ch].CFG  = _LDMA_CH_CFG_RESETVALUE;
  LDMA->CH[ch].LOOP = _LDMA_CH_LOOP_RESETVALUE;
  LDMA->CH[ch].LINK = _LDMA_CH_LINK_RESETVALUE;
...
#define __asm__ asm
  ...
  __asm__ volatile(
      ...
      : [ramptr] "+r"(ramptr)
      : [endptr] "r"(endptr), [ctrlreg] "r"(ctrlreg), [enableEcc] "r"(enableEcc)
      : "r0",
        "r1",    /* R0  and R1 used as temporary registers */
                                       ...
                                       uint32_t channels[2])
{
  ...
  do {
    dmaDesc[descCnt][0] = /* DMA desc CTRL word */
        LDMA_CH_CTRL_STRUCTTYPE_TRANSFER | LDMA_CH_CTRL_STRUCTREQ
        | _LDMA_CH_CTRL_XFERCNT_MASK | LDMA_CH_CTRL_BLOCKSIZE_ALL
        | LDMA_CH_CTRL_REQMODE_ALL | LDMA_CH_CTRL_SRCINC_ONE
        | LDMA_CH_CTRL_SIZE_WORD | LDMA_CH_CTRL_DSTINC_ONE;
    ...
    dmaDesc[descCnt][3] =
        LDMA_CH_LINK_LINK
        | (((uint32_t) &dmaDesc[descCnt + 1][0]) & _LDMA_CH_LINK_LINKADDR_MASK);
  ...
  LDMA->CH[channels[0]].LINK =
      ((uint32_t) &dmaDesc[0][0]) & _LDMA_CH_LINK_LINKADDR_MASK;
  /* Set descriptor address for second channel. */
  LDMA->CH[channels[1]].LINK =
      ((uint32_t) &dmaDesc[descCnt / 2][0]) & _LDMA_CH_LINK_LINKADDR_MASK;
...
{
#if defined(DMEM_COUNT) && (DMEM_COUNT == 1)
  EFM_ASSERT(portPriority
             < ((DMEM_NUM_PORTS + 1) << _MPAHBRAM_CTRL_AHBPORTPRIORITY_SHIFT));

  BUS_RegMaskedWrite(&DMEM->CTRL,
                     _MPAHBRAM_CTRL_AHBPORTPRIORITY_MASK,
                     (uint32_t) portPriority
                         << _MPAHBRAM_CTRL_AHBPORTPRIORITY_SHIFT);
#elif defined(DMEM_COUNT) && (DMEM_COUNT == 2)
  EFM_ASSERT(portPriority
             < ((DMEM0_NUM_PORTS + 1) << _MPAHBRAM_CTRL_AHBPORTPRIORITY_SHIFT));
...
#if defined(DMEM_COUNT) && (DMEM_COUNT == 1)
  port = BUS_RegMaskedRead(&DMEM->CTRL, _MPAHBRAM_CTRL_AHBPORTPRIORITY_MASK);
#elif defined(DMEM_COUNT) && (DMEM_COUNT == 2)
  port = BUS_RegMaskedRead(&DMEM0->CTRL, _MPAHBRAM_CTRL_AHBPORTPRIORITY_MASK);
#endif
