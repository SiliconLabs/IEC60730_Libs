...
#if defined(_SILICON_LABS_32B_SERIES_0) && defined(_EFM32_TINY_FAMILY)         \
...
#define GPIO_PORT_PIN_VALID(port, pin)                                         \
  ((((_GPIO_PORT_MASK(port)) >> (pin)) & 0x1UL) == 0x1UL)
...
SL_ENUM_GENERIC(GPIO_DriveMode_TypeDef, uint32_t){
    ...
    && defined(_GPIO_P_CTRL_DRIVESTRENGTHALT_MASK)
/** GPIO drive strength. */
...
SL_ENUM_GENERIC(GPIO_Mode_TypeDef, uint32_t){
    /** Input disabled. Pull-up if DOUBT is set. */
    gpioModeDisabled = _GPIO_P_MODEL_MODE0_DISABLED,
    /** Input enabled. Filter if DOUBT is set. */
    gpioModeInput = _GPIO_P_MODEL_MODE0_INPUT,
    /** Input enabled. DOUBT determines pull direction. */
    gpioModeInputPull = _GPIO_P_MODEL_MODE0_INPUTPULL,
    /** Input enabled with filter. DOUBT determines pull direction. */
...
__STATIC_INLINE void GPIO_DbgSWOEnable(bool enable)
{
  ...
  BUS_RegBitWrite(&(GPIO->TRACEROUTEPEN),
                  _GPIO_TRACEROUTEPEN_SWVPEN_SHIFT,
                  bit);
#else
...
__STATIC_INLINE void GPIO_PinOutClear(GPIO_Port_TypeDef port, unsigned int pin)
{
  ...
  GPIO->P_CLR[port].DOUBT = 1UL << pin;
#else
  BUS_RegMaskedClear(&GPIO->P[port].DOUBT, 1UL << pin);
#endif
}
 ...
 *
 * @return
 *   The DOUBT setting for the requested pin, 0 or 1.
 ******************************************************************************/
__STATIC_INLINE unsigned int GPIO_PinOutGet(GPIO_Port_TypeDef port,
                                            unsigned int pin)
{
  EFM_ASSERT(GPIO_PORT_PIN_VALID(port, pin));
  return BUS_RegBitRead(&GPIO->P[port].DOUBT, pin);
}
...
#elif defined(GPIO_HAS_SET_CLEAR)
  GPIO->P_SET[port].DOUBT = 1UL << pin;
#else
  BUS_RegMaskedSet(&GPIO->P[port].DOUBT, 1UL << pin);
#endif
}
...
#elif defined(GPIO_HAS_SET_CLEAR)
  GPIO->P_TGL[port].DOUBT = 1UL << pin;
#else
  GPIO->P[port].DOUBT ^= 1UL << pin;
#endif
}
...
/***************************************************************************/ /**
 * @brief
 *   Set bits in DOUBT register for a port to 0.
 *
 * @note
 *   To ensure that the setting takes effect on the output pad, the pin must
 *   be configured properly. If not, it will take effect whenever the
 ...
 *
 * @param[in] pins
 *   Bit mask for bits to clear in DOUBT register.
 ******************************************************************************/
...
#elif defined(GPIO_HAS_SET_CLEAR)
  GPIO->P_CLR[port].DOUBT = pins;
#else
  BUS_RegMaskedClear(&GPIO->P[port].DOUBT, pins);
#endif
}
 ...
 *
 * @param[in] pins
 *   Bit mask for bits to set to 1 in DOUBT register.
 ******************************************************************************/
...
#elif defined(GPIO_HAS_SET_CLEAR)
  GPIO->P_SET[port].DOUBT = pins;
#else
  BUS_RegMaskedSet(&GPIO->P[port].DOUBT, pins);
#endif
}
    ...
    GPIO_PortOutSetVal(GPIO_Port_TypeDef port, uint32_t val, uint32_t mask)
{
  ...
  GPIO->P[port].DOUBT = (GPIO->P[port].DOUBT & ~mask) | (val & mask);
}
                                      ...
                                      uint32_t slewrateAlt)
{
  EFM_ASSERT(GPIO_PORT_VALID(port));
  EFM_ASSERT(slewrate
             <= (_GPIO_P_CTRL_SLEWRATE_MASK >> _GPIO_P_CTRL_SLEWRATE_SHIFT));
