...
sl_status_t sl_debug_swo_init(void)
{
  ...
  GPIO_PinModeSet((GPIO_Port_TypeDef) GPIO_SWV_PORT,
                  GPIO_SWV_PIN,
                  gpioModePushPull,
                  1);
#endif
  ...
  sl_gpio_set_pin_mode((sl_gpio_port_t) GPIO_SWV_PORT,
                       GPIO_SWV_PIN,
  ...
  DWT->CTRL =
      ((4UL << DWT_CTRL_NUMCOMP_Pos) // Number of comparators. Hardwired to 4.
       ...
       | (cyctap
          << DWT_CTRL_CYCTAP_Pos) // Tap cycle counter at bit 10 (vs bit 6 if 0)
       | (0xFUL << DWT_CTRL_POSTINIT_Pos) // Post-tap counter
       | (postpreset
          << DWT_CTRL_POSTPRESET_Pos)      // Post-tap counter reload value
