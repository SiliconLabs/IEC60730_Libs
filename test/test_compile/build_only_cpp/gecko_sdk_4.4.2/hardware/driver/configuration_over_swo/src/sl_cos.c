...
static uint32_t sli_cos_vcom_config(uint32_t baudrate, uint8_t flow_control)
{
  ...
  config |=
      (((baudrate) & (COS_CONFIG_BAUDRATE_MASK)) << COS_CONFIG_BAUDRATE_POS);
  config |=
      (((flow_control) & (COS_CONFIG_UART_FC_MASK)) << COS_CONFIG_UART_FC_POS);
                                   ...
                                   uint8_t interface)
{
  ...
  config |=
      (((baudrate) & (COS_CONFIG_BAUDRATE_MASK)) << COS_CONFIG_BAUDRATE_POS);
  config |=
      (((mode_Val) & (COS_CONFIG_PTI_MODE_MASK)) << COS_CONFIG_PTI_MODE_POS);
...
static void sli_cos_vcom_write(void)
{
  ...
  Cos_Uart_Config.optionValue =
      sli_cos_vcom_config(baudrate, uartdrv_handle->fcType);
...
static void sli_cos_pti_write(void)
{
  ...
  Cos_Pti_Config.optionValue = sli_cos_pti_config(SL_RAIL_UTIL_PTI_BAUD_RATE_HZ,
